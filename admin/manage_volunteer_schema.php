...
<div class="search-box-container" style="max-width: 1100px; margin: 0 auto;">
    <h3><?php echo htmlspecialchars(__('volunteer_schema.heading')); ?></h3>
    <p><?php echo htmlspecialchars(__('volunteer_schema.subheading')); ?></p>

    <div style="margin-bottom: 1.5rem;">
        <a href="volunteer_dashboard.php" class="btn btn-secondary" style="text-decoration: none;">← <?php echo htmlspecialchars(__('volunteer_schema.back_to_dashboard')); ?></a>
    </div>
    ...
    <details class="search-box-container" style="margin-bottom: 2rem; background: rgba(0,0,0,0.01);">
        <summary style="cursor: pointer; font-weight: bold; font-size: 1.15rem; color: #333; padding: 0.25rem 0;">
            ✏️ <?php echo htmlspecialchars(__('volunteer_schema.settings_summary')); ?>
        </summary>
    ...
    <details class="search-box-container" id="create-column-details" <?php echo $edit_col ? 'open' : ''; ?> style="margin-bottom: 2rem; background: rgba(0,0,0,0.01);">
        <summary style="cursor: pointer; font-weight: bold; font-size: 1.15rem; color: #333; padding: 0.25rem 0;">
            <?php echo $edit_col ? htmlspecialchars(__('volunteer_schema.edit_field_title')) . ' ' . htmlspecialchars($edit_col['column_name']) : htmlspecialchars(__('volunteer_schema.add_field_title')); ?>
        </summary>
        ...
        <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border-color);">
            <form method="POST" action="actions/save_volunteer_schema.php">
                ...
                <button type="submit" class="btn"><?php echo $edit_col ? htmlspecialchars(__('feedback_schema.save_field_btn')) : htmlspecialchars(__('volunteer_schema.create_field_btn')); ?></button>
                <?php if ($edit_col): ?>
                    <a href="manage_volunteer_schema.php" class="btn btn-secondary" style="margin-left: 0.5rem; text-decoration: none;"><?php echo htmlspecialchars(__('btn.cancel')); ?></a>
                <?php endif; ?>
            </form>
        </div>
    </details>
    ...
    <h3><?php echo htmlspecialchars(__('volunteer_schema.existing_fields_heading')); ?></h3>
        ...
        <table class="data-table" role="table" style="width: 100%; border-collapse: collapse;">
            ...
            <tbody id="sortable-columns-body">
                <?php if (empty($columns)): ?>
                    <tr><td colspan="6" style="text-align: center; color: #666; padding: 1rem;"><?php echo htmlspecialchars(__('volunteer_schema.no_fields')); ?></td></tr>
                <?php else: ?>
                    <?php foreach ($columns as $col): ?>
                        <tr data-column-id="<?php echo $col['id']; ?>" style="cursor: grab;">
                            ...
                            <td style="white-space: nowrap;">
                                <a href="manage_volunteer_schema.php?edit_column=<?php echo $col['id']; ?>#create-column-details" class="btn btn-secondary" style="padding: 0.25rem 0.5rem; font-size: 0.85rem; text-decoration: none; margin-right: 4px;"><?php echo htmlspecialchars(__('feedback_schema.edit_btn')); ?></a>
                                <form method="POST" action="actions/save_volunteer_schema.php" style="display:inline;" onsubmit="return confirm('<?php echo htmlspecialchars(__('volunteer_schema.delete_confirm')); ?>');">
                                    <?php echo csrf_field(); ?>
