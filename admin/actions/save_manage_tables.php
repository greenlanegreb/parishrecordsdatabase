...
elseif ($action === 'create' || $action === 'update') {
    ...
    if (empty($column_name)) {
        ...
        if ($action === 'create') {
            // Automatically determine the next sort order so new columns play nicely at the end of existing order
            $order_stmt = $pdo->prepare("SELECT COALESCE(MAX(sort_order), 0) + 1 FROM table_columns WHERE table_id = ?");
            $order_stmt->execute([$table_id]);
            $next_sort_order = $order_stmt->fetchColumn();
            ...
            if ($column_id > 0) {
                // Update column properties without touching or overriding the existing sort_order
