...
// AUTO-BACKFILL: Ensure all existing dynamic tables have their view and moderation permissions registered in the database
try {
...
// Check if we are editing an existing role
$edit_role = null;
...
<div class="search-box-container" role="region" aria-label="Site Settings Form" style="max-width: 900px; margin: 0 auto;">
    ...
    <div role="tabpanel" id="panel-permissions" aria-labelledby="tab-permissions" class="tab-panel" style="display: none;">
        ...
        <p style="font-size: 0.9rem; color: #666; margin-bottom: 1.5rem;">Configure which capabilities are assigned to each system role. Table view permissions, moderation permissions, and code definitions automatically populate here.</p>
        
        <!-- Add / Edit Custom Role Collapsible Section -->
        <details id="role-form-details" style="background: rgba(0,0,0,0.02); border: 1px solid var(--border-color); border-radius: 6px; margin-bottom: 2rem; padding: 1rem 1.25rem;" <?php echo $edit_role ? 'open' : ''; ?>>
            <summary style="cursor: pointer; font-weight: bold; color: #333; outline: none;">
                <?php echo $edit_role ? 'Edit Role: ' . htmlspecialchars($edit_role['role_name']) : '+ Create New Custom Role'; ?>
            </summary>
            
            <div style="margin-top: 1rem; border-top: 1px solid var(--border-color); padding-top: 1rem;">
                <p style="font-size: 0.85rem; color: #666; margin-bottom: 1rem;"><?php echo $edit_role ? 'Update the identifier name or description of this role.' : "Add a new role category (e.g., 'archivist', 'data_clerk') to assign capabilities below."; ?></p>
                
                <form method="POST" action="actions/save_role.php" style="display: flex; gap: 1rem; align-items: flex-end; flex-wrap: wrap;">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="action" value="<?php echo $edit_role ? 'update_role' : 'create_role'; ?>">
                    <?php if ($edit_role): ?>
                    ...
                    <div style="flex: 1; min-width: 200px;">
                        ...
                        <input type="text" id="role_name" name="role_name" value="<?php echo $edit_role ? htmlspecialchars($edit_role['role_name']) : ''; ?>" placeholder="e.g. archivist" required class="volunteer-input" style="width: 100%; padding: 0.35rem; margin-top: 0.2rem;">
                    </div>
                    <div style="flex: 2; min-width: 250px;">
                        <label for="role_description" style="font-size: 0.85rem;">Description / Purpose:</label><br>
                        <input type="text" id="role_description" name="description" value="<?php echo $edit_role ? htmlspecialchars($edit_role['description'] ?? '') : ''; ?>" placeholder="e.g. Specialized access for historical document archiving" class="volunteer-input" style="width: 100%; padding: 0.35rem; margin-top: 0.2rem;">
                    </div>
                    <div>
                        <button type="submit" class="btn" style="white-space: nowrap;"><?php echo $edit_role ? 'Save Role' : 'Create Role'; ?></button>
                        <?php if ($edit_role): ?>
                            <a href="settings.php#tab-permissions" class="btn btn-secondary" style="margin-left: 0.5rem; text-decoration: none; padding: 0.35rem 0.7rem; font-size: 0.9rem;" onclick="switchTab('permissions'); return true;">Cancel</a>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
        ...
        // Fetch dynamic table mapping lookup for friendly table name labeling
        $tables_lookup = [];
        $t_lookup_stmt = $pdo->query("SELECT id, table_name FROM dynamic_tables");
        while ($t_row = $t_lookup_stmt->fetch(PDO::FETCH_ASSOC)) {
            $tables_lookup[$t_row['id']] = $t_row['table_name'];
        }
                ...
                <table class="data-table" style="width: 100%; border-collapse: collapse; text-align: left;">
                    <thead>
                        <tr style="border-bottom: 2px solid var(--border-color);">
                            ...
                            <?php foreach ($roles_list as $r): ?>
                                <th style="padding: 0.75rem; text-align: center; text-transform: capitalize;">
                                    <?php echo htmlspecialchars($r['role_name']); ?><br>
                                    <div style="margin-top: 0.3rem; font-size: 0.75rem; font-weight: normal;">
                                        <a href="settings.php?edit_role=<?php echo $r['id']; ?>#tab-permissions" onclick="switchTab('permissions');" style="color: #007bff; text-decoration: underline; margin-right: 4px;">Edit</a>
                                        <?php if (!in_array($r['role_name'], ['admin', 'moderator', 'user', 'guest'])): ?>
                                            <button type="submit" name="delete_role_id" value="<?php echo $r['id']; ?>" formaction="actions/save_role.php" formnovalidate onclick="return confirm('Are you sure you want to delete role \'<?php echo htmlspecialchars($r['role_name']); ?>\'? Any users assigned to it will be reassigned to the default User role.');" style="background: none; border: none; color: #dc3545; cursor: pointer; padding: 0; font-size: 0.75rem; text-decoration: underline;">Delete</button>
                                        <?php endif; ?>
                                    </div>
                                </th>
                            <?php endforeach; ?>
                    ...
                    <tbody>
                        <?php foreach ($perms_list as $p): ?>
                            <?php 
                                $display_key = $p['permission_key'];
                                // Check if the permission is a table view key (e.g. view_table_1)
                                if (strpos($display_key, 'view_table_') === 0) {
                                    $t_id = intval(str_replace('view_table_', '', $display_key));
                                    if (isset($tables_lookup[$t_id])) {
                                        $display_key .= ' <span style="color: #007bff; font-weight: normal;">(' . htmlspecialchars($tables_lookup[$t_id]) . ')</span>';
                                    }
                                }
                                // Check if the permission is a table moderation key (e.g. moderate_table_1)
                                elseif (strpos($display_key, 'moderate_table_') === 0) {
                                    $t_id = intval(str_replace('moderate_table_', '', $display_key));
                                    if (isset($tables_lookup[$t_id])) {
                                        $display_key .= ' <span style="color: #28a745; font-weight: normal;">(Moderate: ' . htmlspecialchars($tables_lookup[$t_id]) . ')</span>';
                                    }
                                }
                            ?>
                            <tr style="border-bottom: 1px solid var(--border-color);">
                                <td style="padding: 0.75rem;"><strong><?php echo $display_key; ?></strong></td>
...
document.addEventListener('DOMContentLoaded', () => {
    // Automatically open the Roles & Permissions tab if editing a role or if the URL has the tab anchor
