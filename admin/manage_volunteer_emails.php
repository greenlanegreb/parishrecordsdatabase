<?php
// admin/manage_volunteer_emails.php - Manage volunteer notification and trigger email templates
...
$current_user = require_admin_page($pdo, 'manage_volunteers', 'Manage volunteer email templates and triggers');

// Fetch all templates
$templates = $pdo->query("SELECT * FROM volunteer_email_templates ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);

// Fetch custom schema columns so admins know what dynamic braces they can use
$columns = $pdo->query("SELECT column_name FROM volunteer_columns ORDER BY sort_order ASC")->fetchAll(PDO::FETCH_COLUMN);
?>
<?php require_once '../partials/header.php'; ?>

<div class="search-box-container" style="max-width: 1100px; margin: 0 auto;">
    <h3><?php echo htmlspecialchars(__('volunteer_emails.heading')); ?></h3>
    <p><?php echo htmlspecialchars(__('volunteer_emails.subheading')); ?></p>

    <div style="margin-bottom: 1.5rem;">
        <a href="volunteer_dashboard.php" class="btn btn-secondary" style="text-decoration: none;">← <?php echo htmlspecialchars(__('volunteer_emails.back_to_dashboard')); ?></a>
    </div>
    ...
    <div style="display: flex; gap: 2rem; flex-wrap: wrap;">
            ...
            <?php foreach ($templates as $tpl): ?>
                <details class="search-box-container" style="margin-bottom: 1.5rem; background: rgba(0,0,0,0.01);">
                    ...
                    <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border-color);">
                        <form method="POST" action="actions/save_volunteer_email_template.php">
                            <?php echo csrf_field(); ?>
                            ...
                            <input type="text" id="subject_<?php echo $tpl['id']; ?>" name="subject" value="<?php echo htmlspecialchars($tpl['subject']); ?>" required class="volunteer-input" style="width: 100%; padding: 0.4rem; margin-bottom: 1rem;"><br>

                            <label for="body_<?php echo $tpl['id']; ?>"><strong><?php echo htmlspecialchars(__('feedback_emails.email_body')); ?></strong></label><br>
                            <textarea id="body_<?php echo $tpl['id']; ?>" name="body" rows="8" required class="volunteer-input" style="width: 100%; padding: 0.4rem; margin-bottom: 1rem; font-family: monospace; resize: vertical;"><?php echo htmlspecialchars($tpl['body']); ?></textarea><br>
        ...
        <div style="flex: 1; min-width: 280px; background: #f8f9fa; padding: 1.2rem; border-radius: 8px; border: 1px solid #ddd; align-self: flex-start; overflow-wrap: break-word; word-break: break-word;">
            ...
            <?php if (!empty($columns)): ?>
                ...
                <p style="font-size: 0.8rem; color: #666; margin-bottom: 4px;"><?php echo htmlspecialchars(__('volunteer_emails.custom_tags_desc')); ?></p>
