...
class VolunteerSchemaController
{
    ...
    public function index(): void
    {
        ...
        $stmtColumns = $this->pdo->query("SELECT vc.*, u.username FROM volunteer_columns vc LEFT JOIN users u ON vc.created_by = u.id ORDER BY vc.sort_order ASC, vc.column_name ASC");
        ...
        if ($action === 'create' || $action === 'update') {
            ...
            if ($columnName !== '') {
                if ($action === 'create') {
                    ...
                    $stmt = $this->pdo->prepare("INSERT INTO volunteer_columns (column_name, data_type, field_subtype, field_options, allow_multiple, max_length, boolean_display_format, sort_order, is_required, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    ...
                    if ($colId > 0) {
                        $stmt = $this->pdo->prepare("UPDATE volunteer_columns SET column_name = ?, data_type = ?, field_subtype = ?, field_options = ?, allow_multiple = ?, max_length = ?, boolean_display_format = ?, is_required = ? WHERE id = ?");
            ...
            $formTitle = isset($post['form_title']) && is_string($post['form_title']) ? trim($post['form_title']) : 'Volunteer for Data Entry';
            $formIntro = isset($post['form_intro']) && is_string($post['form_intro']) ? trim($post['form_intro']) : '';

            $stmt = $this->pdo->prepare("INSERT INTO volunteer_form_settings (setting_key, setting_value) VALUES ('form_title', ?), ('form_intro', ?) ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)");
