            ...
            if ($columnName !== '') {
                if ($action === 'create') {
                    ...
                    $stmt = $this->pdo->prepare(
                        'INSERT INTO volunteer_columns (column_name, data_type, field_subtype, field_options, allow_multiple, max_length, min_value, max_value, boolean_display_format, sort_order, is_required, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
                    );
                    $stmt->execute([
                    ...
                    if ($colId > 0) {
                        $stmt = $this->pdo->prepare(
                            'UPDATE volunteer_columns SET column_name = ?, data_type = ?, field_subtype = ?, field_options = ?, allow_multiple = ?, max_length = ?, min_value = ?, max_value = ?, boolean_display_format = ?, is_required = ? WHERE id = ?'
                        );
                        $stmt->execute([
                            $columnName, $dataType, $fieldSubtype, $fieldOptions, $allowMultiple,
