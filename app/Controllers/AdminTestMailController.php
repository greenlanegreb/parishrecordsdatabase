...
class AdminTestMailController
{
    ...
    public function send(): void
    {
        ...
        try {
            $stmt = $this->pdo->query(
                "SELECT setting_key, setting_value FROM site_settings WHERE setting_key IN ('mail_domain', 'mail_from')"
            );
            if ($stmt !== false) {
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $sKey = isset($row['setting_key']) ? (string) $row['setting_key'] : '';
                    $sVal = isset($row['setting_value']) && is_string($row['setting_value'])
        ...
        $fromEmail = ($mailFrom !== '')
            ? $mailFrom
            : (($mailDomain !== '') ? ('no-reply@' . $mailDomain) : 'webmaster@localhost');
        ...
        try {
            ...
            if ($mailDriver === 'smtp') {
                ...
                $mail->SMTPSecure = (($smtp['smtp_encryption'] ?? 'tls') === 'ssl')
        ...
        if ($success) {
            $_SESSION['message'] = 'Test email successfully dispatched to '
                . htmlspecialchars($testEmail, ENT_QUOTES, 'UTF-8') . '!';
            ...
            $audit->execute([
                $currentUser['id'],
                'TEST_MAIL',
                "Dispatched test email successfully to: {$testEmail}",
                $remoteAddr,
            ]);
        } else {
