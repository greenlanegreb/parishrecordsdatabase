...
class FeedbackSchemaController
{
    ...
    public function index(): void
    {
        ...
        $stmtColumns = $this->pdo->query(
            'SELECT fc.* FROM feedback_columns fc ORDER BY fc.sort_order ASC, fc.column_name ASC'
        );
    ...
    public function store(): void
    {
        ...
        if ($action === 'create' || $action === 'update') {
            ...
            $formTitle = isset($post['form_title']) && is_string($post['form_title'])
                ? trim($post['form_title']) : 'Submit Support Ticket or Feedback';
            $formIntro = isset($post['form_intro']) && is_string($post['form_intro'])
                ? trim($post['form_intro']) : '';
            $stmt = $this->pdo->prepare(
                "INSERT INTO feedback_form_settings (setting_key, setting_value) VALUES ('form_title', ?), ('form_intro', ?)
