...
if (function_exists('get_site_datetime_defaults') && isset($pdo) && $pdo instanceof PDO) {
    $siteDt = get_site_datetime_defaults($pdo);
    if (isset($siteDt[1]) && is_string($siteDt[1]) && $siteDt[1] !== '') {
        $userDateFormat = $siteDt[1];
    }
}
...
} elseif (isset($pdo) && $pdo instanceof PDO && isset($_SESSION['user_id']) && function_exists('get_current_user_data')) {
    $currUserData = get_current_user_data($pdo);
    if (is_array($currUserData) && isset($currUserData['date_format']) && is_string($currUserData['date_format']) && $currUserData['date_format'] !== '') {
        $userDateFormat = $currUserData['date_format'];
        $currentUser = $currentUser ?? $currUserData;
    }
}
...
const datePlaceholder = <?= json_encode(function_exists('get_date_placeholder') ? get_date_placeholder($userDateFormat ?? null) : 'DD/MM/YYYY', JSON_UNESCAPED_UNICODE) ?>;
...
if (draftProposed !== '' && typeof draftProposed === 'string') {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            // Date format is checked by partials/date_input_script.php
        });
    }
});
</script>
