...
<div class="container py-4" role="region" aria-label="Admin User Management" style="max-width: 1300px;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
            <h3 class="fw-bold mb-1"><?= htmlspecialchars(__('admin_users.heading') ?? 'User Management', ENT_QUOTES, 'UTF-8') ?></h3>
            <p class="text-muted mb-0"><?= htmlspecialchars(__('admin_users.subheading') ?? 'Manage registered users, roles, email verification, and security settings.', ENT_QUOTES, 'UTF-8') ?></p>
        </div>
        <div>
            <a href="<?= $basePath ?>/admin/users/emails" class="btn btn-outline-secondary">✉️ <?= htmlspecialchars(__('admin_users.manage_templates_btn') ?? 'Manage Email Templates', ENT_QUOTES, 'UTF-8') ?></a>
        </div>
    </div>
        ...
        <div class="card-body">
            <details id="invite-user-section" <?= ($volunteerId > 0) ? 'open' : '' ?>>
                <summary class="fw-bold text-dark fs-6" style="cursor: pointer; outline: none;">
                    ➕ <?= htmlspecialchars(__('create_user.heading') ?? 'Invite New User', ENT_QUOTES, 'UTF-8') ?>
                </summary>
                <div class="mt-3 pt-3 border-top">
                    <p class="text-muted small mb-3"><?= htmlspecialchars(__('create_user.subheading') ?? 'Send an account invitation email to a new user.', ENT_QUOTES, 'UTF-8') ?></p>
                    <form method="POST" action="<?= $basePath ?>/admin/users/create" style="max-width: 650px;">
                        ...
                        <div class="row g-2 mb-3">
                            ...
                            <div class="col-md-6">
                                <label for="surname" class="form-label small fw-bold"><?= htmlspecialchars(__('create_user.surname') ?? 'Surname', ENT_QUOTES, 'UTF-8') ?></label>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0" role="table">
                <thead class="table-light">
                    <tr>
                        <th scope="col" class="py-3 ps-3"><?= htmlspecialchars(__('feedback_schema.th_id') ?? 'ID', ENT_QUOTES, 'UTF-8') ?></th>
                        <th scope="col" class="py-3"><?= htmlspecialchars(__('admin_users.th_username') ?? 'Username', ENT_QUOTES, 'UTF-8') ?></th>
                        <th scope="col" class="py-3"><?= htmlspecialchars(__('admin_users.th_email_override') ?? 'Email / Verification', ENT_QUOTES, 'UTF-8') ?></th>
                        <th scope="col" class="py-3"><?= htmlspecialchars(__('admin_users.th_role_assignment') ?? 'Role', ENT_QUOTES, 'UTF-8') ?></th>
                        <th scope="col" class="py-3"><?= htmlspecialchars(__('admin_users.th_score') ?? 'Points', ENT_QUOTES, 'UTF-8') ?></th>
                        <th scope="col" class="py-3"><?= htmlspecialchars(__('admin_users.th_status') ?? 'Status', ENT_QUOTES, 'UTF-8') ?></th>
                        <th scope="col" class="py-3"><?= htmlspecialchars(__('admin_users.th_2fa') ?? '2FA', ENT_QUOTES, 'UTF-8') ?></th>
                        <th scope="col" class="py-3 pe-3"><?= htmlspecialchars(__('admin_users.th_actions') ?? 'Actions', ENT_QUOTES, 'UTF-8') ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($users)): ?>
                        <tr>
                            <td colspan="8" class="text-center py-4 text-muted"><?= htmlspecialchars(__('admin_users.no_users') ?? 'No users found.', ENT_QUOTES, 'UTF-8') ?></td>
                        </tr>
                    <?php else: ?>
                            ...
                            <tr data-search="<?= htmlspecialchars(
                                ...
                                <td>
                                    <form method="POST" action="<?= $basePath ?>/admin/users" class="d-flex gap-1 align-items-center mb-1">
                                        ...
                                        <button type="submit" class="btn btn-sm btn-outline-secondary py-0 px-2" style="font-size: 0.75rem;" title="<?= htmlspecialchars(__('admin_users.save_email_title') ?? 'Save Email', ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars(__('btn.save') ?? 'Save', ENT_QUOTES, 'UTF-8') ?></button>
                                    </form>
                                ...
                                <td>
                                    <?php if ($isFirstAdmin): ?>
                                        <span class="small text-muted fst-italic">
                                            ...
                                            <small>(<?= htmlspecialchars(__('admin_users.protected_admin') ?? 'Protected Admin', ENT_QUOTES, 'UTF-8') ?>)</small>
                                        </span>
                                    <?php else: ?>
                                        <form method="POST" action="<?= $basePath ?>/admin/users" class="d-flex gap-1 align-items-center">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-outline-secondary py-0 px-2" style="font-size: 0.75rem;"><?= htmlspecialchars(__('admin_users.update_btn') ?? 'Update', ENT_QUOTES, 'UTF-8') ?></button>
                                        </form>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td>
                                    <?php if ($uActive): ?>
                                        <span class="badge bg-success"><?= htmlspecialchars(__('admin_users.status_active') ?? 'Active', ENT_QUOTES, 'UTF-8') ?></span>
                                    <?php else: ?>
                                        <span class="badge bg-warning text-dark"><?= htmlspecialchars(__('admin_users.status_suspended') ?? 'Suspended', ENT_QUOTES, 'UTF-8') ?></span>
                                    <?php endif; ?>
                                </td>
                                        ...
                                        <div class="d-flex flex-wrap gap-1">
                                            ...
                                            <?php if ($uId !== (int)$currentUser['id'] && !$isFirstAdmin): ?>
                                                <?php if ($uActive): ?>
                                                    <form method="POST" action="<?= $basePath ?>/admin/users" onsubmit="return confirm('<?= htmlspecialchars(__('admin_users.suspend_confirm') ?? 'Are you sure you want to suspend this user?', ENT_QUOTES, 'UTF-8') ?>');" class="d-inline">
                                                        ...
                                                        <input type="hidden" name="target_user_id" value="<?= $uId ?>">
                                                        <button type="submit" class="btn btn-sm btn-outline-danger py-0 px-2" style="font-size: 0.75rem;" aria-label="Suspend <?= htmlspecialchars($uUsername, ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars(__('admin_users.suspend_btn') ?? 'Suspend', ENT_QUOTES, 'UTF-8') ?></button>
                                                    </form>
                                                <?php else: ?>
                                            ...
                                            <?php if ($u2fa): ?>
                                                <form method="POST" action="<?= $basePath ?>/admin/users" onsubmit="return confirm('<?= htmlspecialchars(__('admin_users.reset_2fa_confirm') ?? 'Reset 2FA for this user?', ENT_QUOTES, 'UTF-8') ?>');" class="d-inline">
                                                    ...
                                                    <input type="hidden" name="target_user_id" value="<?= $uId ?>">
                                                    <button type="submit" class="btn btn-sm btn-outline-warning text-dark py-0 px-2" style="font-size: 0.75rem;" aria-label="Reset 2FA for <?= htmlspecialchars($uUsername, ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars(__('admin_users.reset_2fa_btn') ?? 'Reset 2FA', ENT_QUOTES, 'UTF-8') ?></button>
                                                </form>
                                            <?php endif; ?>
                                        </div>
                                    </div>
