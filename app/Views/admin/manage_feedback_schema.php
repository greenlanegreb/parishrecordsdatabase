 ...
 * Migrated Date: 2026-08-05 03:14:40
 */declare(strict_types=1);

/**
 * MIGRATED FILE MAPPING
 * ---------------------
 * Original Old File: admin/manage_feedback_schema.php
...
<div class="container py-4" style="max-width: 1100px;">
    <h3 class="fw-bold mb-1"><?= htmlspecialchars(__('feedback_schema.heading'), ENT_QUOTES, 'UTF-8') ?></h3>
    <p class="text-muted mb-3"><?= htmlspecialchars(__('feedback_schema.subheading'), ENT_QUOTES, 'UTF-8') ?></p>
        ...
        <div class="card-body">
            <details>
                <summary class="fw-bold fs-5 text-dark" style="cursor: pointer;">
                    ✏️ <?= htmlspecialchars(__('feedback_schema.settings_summary'), ENT_QUOTES, 'UTF-8') ?>
                </summary>
                <div class="mt-3 pt-3 border-top">
                    <form method="POST" action="/admin/feedback/schema/store">
                        <?= csrf_field() ?>
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-body">
            <details id="create-column-details" <?= $editCol ? 'open' : '' ?>>
                <summary class="fw-bold fs-5 text-dark" style="cursor: pointer;">
                    <?= $editCol ? htmlspecialchars(__('feedback_schema.edit_field_title'), ENT_QUOTES, 'UTF-8') . ' ' . htmlspecialchars((string)($editCol['column_name'] ?? ''), ENT_QUOTES, 'UTF-8') : htmlspecialchars(__('feedback_schema.add_field_title'), ENT_QUOTES, 'UTF-8') ?>
                </summary>
                ...
                <div class="mt-3 pt-3 border-top">
                    <form method="POST" action="/admin/feedback/schema/store">
                        ...
                        <button type="submit" class="btn btn-primary"><?= $editCol ? htmlspecialchars(__('feedback_schema.save_field_btn'), ENT_QUOTES, 'UTF-8') : htmlspecialchars(__('feedback_schema.create_field_btn'), ENT_QUOTES, 'UTF-8') ?></button>
                        <?php if ($editCol): ?>
                            <a href="/admin/feedback/schema" class="btn btn-outline-secondary ms-2"><?= htmlspecialchars(__('btn.cancel'), ENT_QUOTES, 'UTF-8') ?></a>
                        <?php endif; ?>
                    </form>
                </div>
            </details>
    ...
    <h3 class="fw-bold mb-3"><?= htmlspecialchars(__('feedback_schema.existing_fields_heading'), ENT_QUOTES, 'UTF-8') ?></h3>
            ...
            <table class="table table-hover align-middle mb-0" role="table">
                ...
                <tbody id="sortable-columns-body">
                    <?php if (empty($columns)): ?>
                        <tr>
                            <td colspan="8" class="text-center py-4 text-muted"><?= htmlspecialchars(__('feedback_schema.no_fields'), ENT_QUOTES, 'UTF-8') ?></td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($columns as $col): ?>
                            <?php 
                                ...
                                $createdBy = isset($col['username']) && is_string($col['username']) ? $col['username'] : __('feedback_schema.system_user');
                            ?>
                            <tr data-column-id="<?= $colId ?>" style="cursor: grab;">
                                ...
                                <td><code class="text-secondary"><?= htmlspecialchars($colSub !== '' ? $colSub : __('feedback_schema.subtype_standard_lower'), ENT_QUOTES, 'UTF-8') ?></code></td>
                                ...
                                <td class="text-end pe-3 text-nowrap">
                                    <a href="/admin/feedback/schema?edit_column=<?= $colId ?>#create-column-details" class="btn btn-sm btn-outline-secondary me-1">Edit</a>
                                    
                                    <form method="POST" action="/admin/feedback/schema/store" class="d-inline" onsubmit="return confirm('<?= htmlspecialchars(__('feedback_schema.delete_confirm'), ENT_QUOTES, 'UTF-8') ?>');">
                                        <?= csrf_field() ?>
        ...
        if (tbody) {
            Sortable.create(tbody, {
                ...
                onEnd: function () {
                    ...
                    fetch('/admin/feedback/schema/store', { method: 'POST', body: formData });
                }
            });
        }
    });
