<?php
declare(strict_types=1);

    // ------------------------------------------------------------------
    // Language File for English Street Language - Chavstyle
    // ------------------------------------------------------------------

return [

    // ------------------------------------------------------------------
    // 1. Navigation
    // ------------------------------------------------------------------
    'nav.admin'                                   => 'Big Boss',
    'nav.data_entry'                              => 'Type Up the Patter',
    'nav.feedback'                                => 'Have Your Say / Beef',
    'nav.feedback_dashboard'                      => 'Beef & Complaints Deck',
    'nav.high_contrast'                           => 'Blinding Bright',
    'nav.invite_user'                             => 'Roll Call / Add Mate',
    'nav.language_label'                          => 'Lingo',
    'nav.leaderboard'                             => 'Top Boy Board',
    'nav.leaderboard_score'                       => 'Rep / Street Cred Score',
    'nav.login'                                   => 'Log In (Innit)',
    'nav.logout'                                  => 'Bunk Off / Log Out',
    'nav.low_contrast'                            => 'Dim / Low Key',
    'nav.main_aria'                               => 'Main gaff',
    'nav.manage_tables'                           => 'Sort the Setup',
    'nav.manage_users'                            => 'Manage the Crew',
    'nav.moderation'                              => 'Guv’nor / Security',
    'nav.profile'                                 => 'Me, Innit',
    'nav.search'                                  => 'Have a Butcher’s',
    'nav.select_language'                         => 'Pick your lingo',
    'nav.settings'                                => 'System Settings (Bare Tech)',
    'nav.similar_records'                         => 'Similar sortsa records',
    'nav.toggle_navigation'                       => 'Flip the bits',
    'nav.volunteer'                               => 'Step Up / Graft',
    'nav.volunteer_dashboard'                     => 'Graft Dashboard',
    'nav.welcome'                                 => 'Alright family,',
    'nav.zoom_group'                              => 'Text Size',
    'nav.zoom_in'                                 => 'Larger Text',
    'nav.zoom_out'                                => 'Smaller Text',
    'nav.zoom_reset'                              => 'Reset Text Size',

    // ------------------------------------------------------------------
    // 2. Public search (index)
    // ------------------------------------------------------------------
    'index.date_from_label'                       => 'From',
    'index.filter_aria'                           => 'Proper snoop for',
    'index.no_public_tables'                      => 'Aint no sodding tables on ere for the general public, bruv.',
    'record.back'                                 => 'Jog on back',
    'record.heading'                              => 'Record',
    'record.print'                                => 'Print This Bad Boy Out',
    'record.view'                                 => 'Av a butchers',
    'search.export_csv'                           => 'Send Loot to CSV (Spreadsheet ting)',
    'search.heading'                              => 'Proper Heavy Duty Search Filter',
    'search.heading_for'                          => 'for',
    'search.load_error'                           => 'Kit’s fucked, couldn’t load that. Try again.',
    'search.no_records'                           => 'Absolute tumbleweeds round here bruv, fuck all found.',
    'search.reset'                                => 'Wipe It Clean',

    // ------------------------------------------------------------------
    // 3. Common buttons
    // ------------------------------------------------------------------
    'btn.cancel'                                  => 'Nah Forget It',
    'btn.delete'                                  => 'Chuck It In the Bin',
    'btn.save'                                    => 'Lock It In',
    'btn.submit'                                  => 'Send It Fam',

    // ------------------------------------------------------------------
    // 4. actions/save_feedback.php & feedback.php Strings
    // ------------------------------------------------------------------
    'feedback.error_all_fields'                   => 'Fill out the whole lot you weapon.',
    'feedback.error_invalid_email'                => 'Put a proper email in, that one’s bent.',
    'feedback.error_save_failed'                  => 'Glitch in the matrix fam, didn’t save. Try again.',
    'feedback.success_message'                    => 'Safe bruv! Your message has dropped straight through.',

    // ------------------------------------------------------------------
    // 5. Index / Public Directory Page
    // ------------------------------------------------------------------
    'index.admin_add_columns_guide'               => 'Boss man, get over to Admin → Manage Tables and punch at least one column in.',
    'index.admin_create_table_guide'              => 'As the big boss, get down to Admin → Manage Tables to build a rack and slap a column in before you start noseying about.',
    'index.clipboard_success'                     => 'Coped that straight to your clipboard geezer! Paste it into Excel or Google Sheets.',
    'index.contact_admin_columns'                 => 'Give the gaffer a shout to sort out the columns for this bit.',
    'index.contact_admin_tables'                  => 'Go holler at the main man to wire up the tables.',
    'index.copy_entire_table'                     => 'Copy Whole Lot',
    'index.copy_filtered_table'                   => 'Copy Filtered Lot',
    'index.date_to_label'                         => 'till',
    'index.download_entire_csv'                   => 'Grab Whole CSV',
    'index.download_entire_json'                  => 'Grab Whole JSON',
    'index.download_filtered_csv'                 => 'Grab Filtered CSV',
    'index.download_filtered_json'                => 'Grab Filtered JSON',
    'index.go_to_manage_tables'                   => 'Sort the Tables Out',
    'index.guest_login_tables_guide'              => 'Oi, log in or go badger the admin to set up the tables.',
    'index.modal_desc'                            => 'Give us the straight gen or a better shout for this record. The security crew will size it up.',
    'index.modal_heading'                         => 'Spot a Blunder? Fix It Up',
    'index.modal_input_placeholder'               => 'Drop the updated gen here...',
    'index.modal_proposed_value'                  => 'Proper Value / Correction:',
    'index.modal_submit_btn'                      => 'Send Word',
    'index.modal_target_column'                   => 'Target Slot:',
    'index.no_columns_desc'                       => 'Tables are there, but nobody’s built any data slots for this one yet.',
    'index.no_columns_heading'                    => 'No Columns In Sight',
    'index.no_tables_desc'                        => 'Ain’t got no active database tables wired up yet mate.',
    'index.no_tables_heading'                     => 'Fck All Tables Round Here',
    'index.opt_cross'                             => '✘ (Crossed Out)',
    'index.opt_false'                             => 'Cap',
    'index.opt_female'                            => 'Lass / Bird',
    'index.opt_male'                              => 'Lad',
    'index.opt_no_false'                          => 'Nah / Bullshit',
    'index.opt_tick'                              => '✔ (Sorted)',
    'index.opt_true'                              => 'Legit',
    'index.opt_yes_true'                          => 'Yeah / Legit',
    'index.option_all'                            => '-- The Whole Lot --',
    'index.search_placeholder'                    => 'Search for gaffs, names...',
    'index.select_directory_database'             => 'Pick your turf database:',
    'index.th_actions'                            => 'Moves',
    'index.th_created_by'                         => 'Planted By',
    'index.th_date_added'                         => 'Dropped On',
    'index.th_record_id'                          => 'Tag ID',

    // ------------------------------------------------------------------
    // 6. Print Records Functionality
    // ------------------------------------------------------------------
    'cols.back'                                   => 'Go back',
    'cols.print_btn'                              => 'Print',
    'cols.print_entire'                           => 'Print the Lot',
    'cols.print_filtered'                         => 'Print the Filtered Ones',
    'cols.print_now'                              => 'Print this ting or save it as a PDF',
    'cols.show_heading'                           => 'Pick which bare columns you actually want on show, innit',
    'cols.show_help'                              => 'Unclick a column if you wanna hide it on here, when you download stuff, or when you print it out. On a phone, fewer columns makes it way easier to read, blud.',

    // ------------------------------------------------------------------
    // 7. Admin: Create User / Invite Form
    // ------------------------------------------------------------------
    'create_user.email_label'                     => 'In Box / Email Addy:',
    'create_user.first_name'                      => 'Christian Name / Moniker:',
    'create_user.heading'                         => 'New Recruit Call-Up Form',
    'create_user.role_label'                      => 'Rank / Position:',
    'create_user.subheading'                      => 'This is gonna spin up a lockdown link good for 24 hours straight to their burner inbox.',
    'create_user.submit_btn'                      => 'Bring ‘Em In & Fire Invites',
    'create_user.surname'                         => 'Family Name / Surname:',
    'create_user.username_help'                   => 'Leave this empty and we’ll spit out a proper unique tag based on your name.',
    'create_user.username_label'                  => 'Tag / Alias (Optional):',
    'create_user.username_placeholder'            => 'Leave blank if you want a random gen',

    // ------------------------------------------------------------------
    // 8. Admin: Feedback / Support Tickets Dashboard
    // ------------------------------------------------------------------
    'feedback_dash.anonymous'                     => 'Ghost / Anon',
    'feedback_dash.default_subject'               => 'Random Chinwag',
    'feedback_dash.delete_confirm'                => 'Wipe this ticket and all the back-and-forth for good?',
    'feedback_dash.heading'                       => 'Beef & Support Ticket Deck',
    'feedback_dash.manage_emails'                 => 'Sort Email Slang Templates',
    'feedback_dash.manage_schema'                 => 'Sort Ticket Form Blueprint',
    'feedback_dash.msg_deleted'                   => 'Ticket #:id got binned proper.',
    'feedback_dash.no_tickets'                    => 'No beef or tickets found fam.',
    'feedback_dash.open_ticket_btn'               => 'Open Up & Have a Parley',
    'feedback_dash.subheading'                    => 'Handle the public complaints, flip statuses, and chat back in the trenches.',
    'feedback_dash.th_status'                     => 'Vibe / Status',
    'feedback_dash.th_subject_info'               => 'The Issue / Core Details',
    'feedback_dash.th_submitter'                  => 'Whistleblower',
    'feedback_dash.th_ticket_date'                => 'When',

    // ------------------------------------------------------------------
    // 9. Admin: Manage Feedback Email Templates
    // ------------------------------------------------------------------
    'feedback_emails.back_to_dashboard'           => 'Back to Beef Deck',
    'feedback_emails.custom_tags'                 => 'Custom Form Tags:',
    'feedback_emails.custom_tags_desc'            => 'Pulled straight from your ticket form builder:',
    'feedback_emails.email_body'                  => 'Message Blueprint:',
    'feedback_emails.email_subject'               => 'Email Subject / Title:',
    'feedback_emails.fixed_tags'                  => 'Standard Fixed Tags:',
    'feedback_emails.heading'                     => 'Ticket Email Templates (Auto-Chat)',
    'feedback_emails.placeholders_desc'           => 'Use these anywhere in the subject or body to drop custom details:',
    'feedback_emails.placeholders_heading'        => 'Tags You Can Slap In',
    'feedback_emails.save_template_btn'           => 'Lock In Template',
    'feedback_emails.subheading'                  => 'Customise the automated emails sent out when handling beef. Wrap your dynamic tags in curly brackets.',

    // ------------------------------------------------------------------
    // 10. Admin: Manage Feedback Ticket Schema & Fields
    // ------------------------------------------------------------------
    'feedback_schema.add_field_title'             => '+ Slap a New Field Into Ticket Form',
    'feedback_schema.allow_multiple'              => 'Let them pick loads (Multi-select)',
    'feedback_schema.boolean_format'              => 'Yes/No Display Style:',
    'feedback_schema.create_field_btn'            => 'Build Ticket Field',
    'feedback_schema.data_type_label'             => 'Data Flavor:',
    'feedback_schema.delete_confirm'              => 'Wipe this field and all data tied to it?',
    'feedback_schema.edit_btn'                    => 'Twist',
    'feedback_schema.edit_field_title'            => 'Twist Ticket Field:',
    'feedback_schema.existing_fields_heading'     => 'Already Built Ticket Fields',
    'feedback_schema.field_name_label'            => 'Field Tag / Title:',
    'feedback_schema.form_intro_label'            => 'Intro / Hook Text:',
    'feedback_schema.form_title_label'            => 'Form Title:',
    'feedback_schema.heading'                     => 'Sort Ticket Form Blueprint',
    'feedback_schema.is_required_label'           => 'Mandatory field (Can’t skip this)',
    'feedback_schema.max_length_label'            => 'Max Length / Character Limit (Optional):',
    'feedback_schema.no_fields'                   => 'No custom ticket fields set up yet fam.',
    'feedback_schema.options_help'                => 'Separate options with commas or drop them on separate lines.',
    'feedback_schema.options_label'               => 'Choices (Comma-separated or stack ‘em per line):',
    'feedback_schema.save_field_btn'              => 'Save Field Tweaks',
    'feedback_schema.save_settings_btn'           => 'Save Form Setup',
    'feedback_schema.settings_summary'            => 'Sort Form Title & Disclaimer Small Print',
    'feedback_schema.sub_checkbox'                => 'Tick Box',
    'feedback_schema.sub_email'                   => 'Email',
    'feedback_schema.sub_number'                  => 'Number Pad',
    'feedback_schema.sub_radio'                   => 'Radio Button Stack',
    'feedback_schema.sub_select'                  => 'Drop-down Menu',
    'feedback_schema.sub_textarea'                => 'Big Rant Box (Textarea)',
    'feedback_schema.sub_url'                     => 'URL Link',
    'feedback_schema.subheading'                  => 'Set up custom fields, data types, character ceilings, subtypes, options, and how they look.',
    'feedback_schema.subtype_label'               => 'Field Subtype / Input Style:',
    'feedback_schema.subtype_standard'            => '-- Basic --',
    'feedback_schema.subtype_standard_lower'      => 'basic',
    'feedback_schema.system_user'                 => 'The System',
    'feedback_schema.th_created_by'               => 'Made By',
    'feedback_schema.th_data_type'                => 'Data Type',
    'feedback_schema.th_email'                    => 'Email',
    'feedback_schema.th_field_name'               => 'Field Name',
    'feedback_schema.th_id'                       => 'ID',
    'feedback_schema.th_max_length'               => 'Max Size',
    'feedback_schema.th_move'                     => 'Shift',
    'feedback_schema.th_required'                 => 'Must Have?',
    'feedback_schema.th_subtype'                  => 'Subtype',
    'feedback_schema.type_boolean'                => 'BOOLEAN (Yeah/Nah Flag)',
    'feedback_schema.type_date'                   => 'DATE (Calendar Day)',
    'feedback_schema.type_int'                    => 'INT (Whole Number)',
    'feedback_schema.type_text'                   => 'TEXT (Long Essay / Rant)',
    'feedback_schema.type_varchar'                => 'VARCHAR (Short Text)',

    // ------------------------------------------------------------------
    // 11. Admin: Manage Tables & Column Schemas
    // ------------------------------------------------------------------
    'manage_tables.add_col_summary_prefix'        => '+ Slap new column into',
    'manage_tables.allow_multiple_label'          => 'Let \'em pick more than one geezer (multi-select)',
    'manage_tables.bool_male_female'              => 'Fella / Bird',
    'manage_tables.bool_tick_cross'               => 'Tick / Cross',
    'manage_tables.bool_true_false'               => 'Dead right / Bollocks',
    'manage_tables.bool_yes_no'                   => 'Yeah / Nah',
    'manage_tables.choice_options_help'           => 'Example: Christening, Weddin, Planting — bang \'em on their own line.',
    'manage_tables.choice_options_label'          => 'Options (one per line, innit)',
    'manage_tables.col_name_label'                => 'Column Title:',
    'manage_tables.create_col_btn'                => 'Build Column',
    'manage_tables.create_table_btn'              => 'Build Table Blueprint',
    'manage_tables.create_table_summary'          => '+ Build Fresh Dynamic Table',
    'manage_tables.date_behavior_label'           => 'Date Search Vibe:',
    'manage_tables.date_bhv_admin'                => 'Boss dates only',
    'manage_tables.date_bhv_all'                  => 'All dates including boss inputs',
    'manage_tables.date_bhv_manual'               => 'Database date (manual punching only)',
    'manage_tables.delete_col_confirm'            => 'WARNING: Trashing this column deletes every single cell of data tied to it across all records. Sure?',
    'manage_tables.delete_table_btn'              => 'Bin the Table',
    'manage_tables.delete_table_confirm'          => 'WARNING: Binned tables drop every single column and saved record forever. Are you absolutely certain geezer?',
    'manage_tables.edit_col_summary'              => 'Twist Column:',
    'manage_tables.edit_metadata_btn'             => 'Edit Table Details',
    'manage_tables.edit_table_summary'            => 'Twist Table Specs:',
    'manage_tables.err_col_name_empty'            => 'Oi, type a name for the column.',
    'manage_tables.err_col_name_taken'            => 'This table already’s got a column named %s. Pick a proper different name.',
    'manage_tables.err_table_name_empty'          => 'Oi, type a name for the table.',
    'manage_tables.err_table_name_taken'          => 'Fam, a table called %s is already here. Pick a different name, g.',
    'manage_tables.exclude_search_label'          => 'Hide This Column From Public Prying Eyes',
    'manage_tables.existing_cols_heading_prefix'  => 'Current columns for',
    'manage_tables.heading'                       => 'Dynamic Table & Column Management',
    'manage_tables.intro'                         => 'Ere, change the names, field types, and whatever shows up on the search lists or the proper record page right proper, innit.',
    'manage_tables.max_value_label'               => 'Absolute maximum',
    'manage_tables.min_value_label'               => 'Bare minimum',
    'manage_tables.no'                            => 'Nah mate',
    'manage_tables.no_columns_found'              => 'No dynamic columns set up for this table yet.',
    'manage_tables.req_toggle_label'              => 'Make This Column Mandatory (Stop Them Skiving Data Entry)',
    'manage_tables.save_and_add_col'              => 'Save this shit and chuck another one in',
    'manage_tables.save_table_btn'                => 'Save Table Changes',
    'manage_tables.show_in_list'                  => 'Show On The Search / Data-Entry List',
    'manage_tables.show_in_record'                => 'Show on the Full Record Page',
    'manage_tables.status_hidden'                 => 'Undercover / Hidden',
    'manage_tables.subheading'                    => 'Change names, field types, and what shows up on the search list or unique record page right proper.',
    'manage_tables.switcher_label'                => 'Pick active table blueprint:',
    'manage_tables.table_desc_label'              => 'What’s it for / Description:',
    'manage_tables.table_name_label'              => 'Street Name for Table:',
    'manage_tables.th_date_created'               => 'Created When',
    'manage_tables.th_display_format'             => 'Display Style',
    'manage_tables.th_public_search'              => 'Public Search?',
    'manage_tables.type_boolean'                  => 'Bool (Lads / Lasses, Tick / Cross, Safe / Bullshit, Yeah / Nah)',
    'manage_tables.type_choice'                   => 'Pick List',
    'manage_tables.type_date'                     => 'Date Bruv',
    'manage_tables.type_email'                    => 'Proper Email Addy',
    'manage_tables.type_int'                      => 'Proper Number (Whole Num)',
    'manage_tables.type_location'                 => 'Where Abouts (Map Pin)',
    'manage_tables.type_location_help'            => 'Gotta pick places proper off the drop-down list so they pop up on this table’s map. Use a standard text column if you just want a written-down place name, innit.',
    'manage_tables.type_text_long'                => 'Long Ass Text',
    'manage_tables.type_time'                     => 'Time',
    'manage_tables.type_url'                      => 'Website Link',
    'manage_tables.type_varchar'                  => 'Short Text Blud',
    'manage_tables.url_new_tab'                   => 'Open In A Fresh Tab Fam',
    'manage_tables.yes'                           => 'Yeah bruv',

    // ------------------------------------------------------------------
    // 12. Map Functionality Translations
    // ------------------------------------------------------------------
    'map.apply_filters'                           => 'Sort filters out',
    'map.back_to_table'                           => 'Back to the table',
    'map.canvas_aria'                             => 'Map of records',
    'map.filters'                                 => 'Filters',
    'map.heading'                                 => 'Map',
    'map.help'                                    => 'Zoom right in to load up the bits in view. Use the filters to match up with the table search.',
    'map.list_empty'                              => 'Nowt in this view, fam.',
    'map.list_heading'                            => 'Places in this view',
    'map.list_show_more'                          => 'Show more in the list (:n left)',
    'map.open_btn'                                => 'Map',
    'map.skip_to_map'                             => 'Skip straight to the map',

    // ------------------------------------------------------------------
    // 13. Admin: Manage User Notification Email Templates
    // ------------------------------------------------------------------
    'user_emails.back_to_creation'                => 'Back to Recruitment',
    'user_emails.currently_editing'               => 'Working on:',
    'user_emails.desc_invitation'                 => 'Fired out automatically when an admin invites a brand new recruit.',
    'user_emails.desc_outcome'                    => 'This gets sent out when a gaffer accepts or bins a suggested edit and the geezer asked for an email ping.',
    'user_emails.desc_reset'                      => 'Fired out when someone triggers a password reset or re-requests a link.',
    'user_emails.email_body_label'                => 'Email Content:',
    'user_emails.heading'                         => 'Sort User Notice Email Templates',
    'user_emails.opt_invitation'                  => 'New Mate Account Invite Template',
    'user_emails.opt_reset'                       => 'Password Reset / Secret Entry Link Template',
    'user_emails.opt_suggestion_outcome'          => 'What happened with the suggested edit, fam',
    'user_emails.placeholders_hint'               => 'These tokens get filled in when this template goes out. Only the cards below apply to the template you proper selected, mate.',
    'user_emails.select_template_label'           => 'Pick template to twist:',
    'user_emails.subheading'                      => 'Customise email layouts sent out when bringing new mates on board or firing out password resets.',

    // ------------------------------------------------------------------
    // 14. Admin: Manage Volunteer Email Templates
    // ------------------------------------------------------------------
    'volunteer_emails.back_to_dashboard'          => 'Back to Graft Submissions',
    'volunteer_emails.custom_tags_desc'           => 'Pulled automatically from form builder fields:',
    'volunteer_emails.heading'                    => 'Graft / Volunteer Email Templates',
    'volunteer_emails.subheading'                 => 'Set up automatic replies for graft workers at different stages. Use curly brackets for dynamic details.',

    // ------------------------------------------------------------------
    // 15. Admin: Manage Volunteer Form Schema & Fields
    // ------------------------------------------------------------------
    'volunteer_schema.add_field_title'            => '+ Slap New Field Into Volunteer Form',
    'volunteer_schema.back_to_dashboard'          => 'Back to Graft Submissions',
    'volunteer_schema.create_field_btn'           => 'Build Field',
    'volunteer_schema.delete_confirm'             => 'Wipe this field and all response values tied to it?',
    'volunteer_schema.edit_field_title'           => 'Twist Field:',
    'volunteer_schema.existing_fields_heading'    => 'Current Volunteer Form Fields',
    'volunteer_schema.heading'                    => 'Sort Graft / Volunteer Form Blueprint',
    'volunteer_schema.no_fields'                  => 'No custom volunteer fields set up yet.',
    'volunteer_schema.settings_summary'           => 'Sort Form Title & Small Print',
    'volunteer_schema.subheading'                 => 'Configure custom fields, data types, subtypes, choices, and general display styles for the form.',

    // ------------------------------------------------------------------
    // 16. Admin: Moderation Queue & Suggestions Review
    // ------------------------------------------------------------------
    'dup_merge.col_a'                             => 'Record #%s',
    'dup_merge.col_b'                             => 'Record #%s',
    'dup_merge.col_field'                         => 'Field',
    'dup_merge.confirm'                           => 'Merge these into a single record, fam? The other one is getting deleted.',
    'dup_merge.fields_caption'                    => 'Pick a value for every single field',
    'dup_merge.heading'                           => 'Merge two similar records together',
    'dup_merge.intro'                             => 'These two are about %s%% similar, bruv. Pick which record to keep, then choose a value for every field.',
    'dup_merge.keep_a'                            => 'Keep record #%s',
    'dup_merge.keep_b'                            => 'Keep record #%s',
    'dup_merge.keep_legend'                       => 'Which record we keeping then?',
    'dup_merge.save_btn'                          => 'Merge into one single record',
    'dup_merge_btn'                               => 'Merge into one',
    'dup_merged'                                  => 'Those two records are one now, blud. The change is in the history of the one left over.',
    'dup_queue.choose_table'                      => 'Pick a table',
    'dup_queue.col_actions'                       => 'What to do',
    'dup_queue.col_records'                       => 'Records',
    'dup_queue.col_score'                         => 'How similar',
    'dup_queue.col_table'                         => 'Table',
    'dup_queue.dismiss_btn'                       => 'Aint a duplicate',
    'dup_queue.dismissed'                         => 'That pair got marked as not a duplicate, g.',
    'dup_queue.empty'                             => 'Nuffing waiting around. Scan a table to look for old similar items.',
    'dup_queue.gone'                              => 'That review ain’t waiting around no more.',
    'dup_queue.heading'                           => 'Similar records to check over',
    'dup_queue.intro'                             => 'Scan the table for rows that look dead similar already. You can state that they’re completely different, or merge them into one big record.',
    'dup_queue.scan_btn'                          => 'Scan this table, blud',
    'dup_queue.scan_done'                         => 'Scan’s finished, fam. %s similar pairs found, %s new items slapped onto the list.',
    'dup_queue.scan_label'                        => 'Look for similar records in',
    'moderate.approve_btn'                        => 'Approve (Stamp)',
    'moderate.approve_confirm'                    => 'Stamp it and push this value live?',
    'moderate.by_label'                           => 'Who:',
    'moderate.column_label'                       => 'Column:',
    'moderate.decline_btn'                        => 'Decline (Scrap)',
    'moderate.decline_confirm'                    => 'Bin this suggestion and scrap it?',
    'moderate.dup_approved_note'                  => 'Sorted. This is now %s%% like record #%s, innit.',
    'moderate.dup_blocked'                        => 'Nah mate, can’t approve this: this looks like %s%% bare like record #%s.',
    'moderate.empty_placeholder'                  => '[Fck All / Blank]',
    'moderate.evidence_label'                     => 'The Proof / Backstory:',
    'moderate.guest_user'                         => 'Random / Passerby',
    'moderate.heading'                            => 'Security Queue & Edits Review',
    'moderate.historical_dates_title'             => 'Old-school historical dates supported',
    'moderate.live_value_label'                   => 'Current Live Gen:',
    'moderate.no_evidence'                        => 'No proof or backstory given.',
    'moderate.no_suggestions'                     => 'No pending suggestions sitting in your authorized security queues.',
    'moderate.override_label'                     => 'Override Value:',
    'moderate.proposed_value_label'               => 'Proposed Twist:',
    'moderate.rationale_label'                    => 'Note for the gyaldem or bloke who suggested this',
    'moderate.rationale_optional'                 => 'Optional note (stays on the record, bruv).',
    'moderate.rationale_placeholder'              => 'Why you actually accepted or binned this change',
    'moderate.rationale_will_be_sent'             => 'An email has been requested. Your note below is getting sent out exact same way you wrote it, so keep it peak or calm, g.',
    'moderate.record_id_label'                    => 'Record ID:',
    'moderate.required_badge'                     => 'Must Have',
    'moderate.select_placeholder'                 => '-- Pick One --',
    'moderate.shortcut_desc'                      => 'Press Ctrl + Enter to sign off instantly or Esc to clear the override box!',
    'moderate.shortcut_label'                     => 'Keyboard Cheat Sheet:',
    'moderate.subheading'                         => 'Compare user-suggested edits against live records in your approved tables. Stamp ‘em, override ‘em, or chuck ‘em out.',
    'moderate.tab_similar'                        => 'Bare similar records',
    'moderate.tab_suggestions'                    => 'Suggestions',
    'moderate.th_actions'                         => 'Guv’nor Moves',
    'moderate.th_comparison'                      => 'Side-by-Side (Live vs Proposed) & Proof',
    'moderate.th_id_date'                         => 'ID / When',
    'moderate.th_table_record'                    => 'Table, Record & Column',

    // ------------------------------------------------------------------
    // 17. Admin: Notices & Announcements Manager
    // ------------------------------------------------------------------
    'notices.active'                              => 'Proper live',
    'notices.add_new'                             => 'Slap up a fresh shout',
    'notices.audience'                            => 'The mandem / Who sees it',
    'notices.content_label'                       => 'The Gist',
    'notices.create_heading'                      => 'Build New Alert',
    'notices.create_notice_btn'                   => 'Post the shout',
    'notices.delete_btn'                          => 'Bin it',
    'notices.delete_confirm'                      => 'You actually wanna trash this shout, fam?',
    'notices.dismissible'                         => 'Can shut it',
    'notices.dismissible_label'                   => "Can be dismissed (Slaps an 'X' close button on it)",
    'notices.display_order'                       => 'Order of play',
    'notices.display_order_label'                 => 'Display Order:',
    'notices.error_blank'                         => 'Title and content can’t be left blank mate.',
    'notices.everyone'                            => 'Absolute everyone',
    'notices.existing_heading'                    => 'Active & Existing Banners',
    'notices.heading'                             => 'Banners & Site Alerts Manager',
    'notices.msg_created'                         => 'Alert successfully published!',
    'notices.msg_deleted'                         => 'Alert binned.',
    'notices.no_notices'                          => 'No alerts made yet.',
    'notices.no_sticky'                           => 'Nah (Sticky / Clinging on)',
    'notices.publish_btn'                         => 'Publish Alert',
    'notices.role_admins'                         => 'Main Bosses (Administrators)',
    'notices.role_everyone'                       => 'The Whole Lot',
    'notices.role_moderators'                     => 'Security Crew (Moderators)',
    'notices.role_public'                         => 'Public Walk-ins (Guests)',
    'notices.role_users'                          => 'Registered Lads (Users)',
    'notices.save_btn'                            => 'Lock it in',
    'notices.subheading'                          => 'Slap up dynamic alerts, welcome banners, or targetted announcements for specific user ranks.',
    'notices.target_roles_label'                  => 'Target Crew (Pick roles or everyone):',
    'notices.th_dismissible'                      => 'Dismissible',
    'notices.th_order'                            => 'Order',
    'notices.th_target_roles'                     => 'Target Ranks',
    'notices.th_title'                            => 'Title',
    'notices.title_label'                         => 'What’s it called',
    'notices.yes'                                 => 'Yeah',

    // ------------------------------------------------------------------
    // 18. Admin: Global Site Settings, Modules & Permissions
    // ------------------------------------------------------------------
    'appearance.add_link'                         => 'Slap A Link In',
    'appearance.brand_heading'                    => 'Logo And Strapline',
    'appearance.colors_heading'                   => 'Colours',
    'appearance.colors_help'                      => 'High Contrast still mugs these off when someone smacks that toggle.',
    'appearance.custom_heading'                   => 'Custom Links Fam',
    'appearance.custom_help'                      => 'Add as many as you proper want bruv. Sort out the parent so the link sits under another heading or a built-in menu like Admin. Use # as the link addy if it is just a deadting title.',
    'appearance.custom_label'                     => 'Label',
    'appearance.custom_parent'                    => 'Bossman Parent',
    'appearance.custom_url'                       => 'Addy',
    'appearance.custom_who'                       => 'Who Can See This Shizz',
    'appearance.drag'                             => 'Drag It Blud To Sort The Order',
    'appearance.nav_heading'                      => 'Navigation',
    'appearance.nav_help'                         => 'Grab the handle and drag it to sort out the built-in and custom links together bruv. To stick a link under Admin (or another heading), set the parent right on that link. The # address makes a heading with no actual page. Login and profile stay dead where they are.',
    'appearance.new_tab'                          => 'Fresh Tab',
    'appearance.parent_builtin'                   => 'Built-In Shizz',
    'appearance.parent_custom'                    => 'Your Own Headings Init',
    'appearance.parent_none'                      => 'Top Dog — Proper Bossman Menu Item',
    'appearance.remove_link'                      => 'Bin It',
    'appearance.reset'                            => 'Reset to Default',
    'appearance.reset_ok'                         => 'Look Reset to Default, Proper.',
    'appearance.save'                             => 'Save The Look',
    'appearance.saved'                            => 'Look Saved, Safe.',
    'appearance.type_heading'                     => 'Type',
    'role.label_admin'                            => 'Big Boss / Admin',
    'role.label_guest'                            => 'Random passer-by',
    'role.label_moderator'                        => 'Mod / Bouncer',
    'role.label_user'                             => 'Data-bashing geezer',
    'settings.add_notice_btn'                     => '+ Add New Alert',
    'settings.audit_heading'                      => 'CCTV Audit Log Browser',
    'settings.audit_limit_note'                   => 'Showing the last 250 CCTV audit log entries.',
    'settings.audit_subheading'                   => 'Check logged security actions, data inputs, and moderation moves. Use maintenance options below to hose down logs if needed.',
    'settings.captcha_hcaptcha'                   => 'hCaptcha',
    'settings.captcha_heading'                    => 'Security & CAPTCHA Setup',
    'settings.captcha_none'                       => 'Off (No Bot Blocker)',
    'settings.captcha_provider_label'             => 'CAPTCHA Bot Blocker:',
    'settings.captcha_recaptcha'                  => 'Google reCAPTCHA v2 / v3',
    'settings.captcha_turnstile'                  => 'Cloudflare Turnstile',
    'settings.clear_all_audit_btn'                => 'Hose Down All Audit Logs',
    'settings.clear_records_audit_btn'            => 'Hose Record Audits Only',
    'settings.core_sys_heading'                   => 'Core System Setup',
    'settings.create_role_btn'                    => 'Make the Role',
    'settings.create_role_heading'                => 'Drop a New Role',
    'settings.cron_command_label'                 => 'Here is the command to paste into your cron jobs (Once a day is sweet as g!)',
    'settings.cron_exec_ok'                       => 'This Page Was Allowed To Test PHP Right Here (If You Want)',
    'settings.cron_maintenance_desc'              => 'pRD checked out your web server automatically and reckons you need a nightly command. Still gotta paste it into your cron jobs bruv. You can run a clean right now too.',
    'settings.cron_maintenance_heading'           => 'Proper Automated Token Maintins & Cron Setup, Safe G',
    'settings.cron_php_ok'                        => 'Found The PHP Command Safe',
    'settings.cron_script_ok'                     => 'Big Ups G - Your Cleanup Script Is Sorted!',
    'settings.db_updates_heading'                 => 'Database Upgrades',
    'settings.default_date_format'                => 'Standard day layout',
    'settings.default_lang_desc'                  => 'Used for random guests and users without a preset language. Drop files into lang/ (e.g., en_GB_chav.php) for more.',
    'settings.default_lang_label'                 => 'Default Site Slang / Language:',
    'settings.default_lang_note'                  => 'Default lingo is already sorted up top. Your profile bits override these when you’re logged in proper.',
    'settings.default_time_format'                => 'Standard hour layout',
    'settings.default_timezone'                   => 'Standard clock time',
    'settings.delete_btn'                         => 'Bin it',
    'settings.delete_role_confirm'                => 'You dead-ass sure you wanna scrap this role? Anyone stuck on it will get safely banged back over to the standard default user tier.',
    'settings.download_backup_btn'                => 'Download Database Backup (SQL)',
    'settings.download_backup_desc'               => 'Saves a full .sql snapshot to your rig. Stash it safely before running updates.',
    'settings.driver_native'                      => 'Native Mail (Local Postfix Relay)',
    'settings.driver_smtp'                        => 'Authenticated SMTP (PHPMailer)',
    'settings.dup_heading'                        => 'Duplicate Record Management, bruv',
    'settings.dup_help'                           => 'pRD compares what you typed to other rows in the same table. You can warn people, block the save, or turn this off entirely.',
    'settings.dup_legend'                         => 'If a new record looks like one you already got',
    'settings.dup_mode_block'                     => 'Don’t let them save it, blud',
    'settings.dup_mode_flag'                      => 'Warn them, and flag it up for an admin if they save it anyway',
    'settings.dup_mode_off'                       => 'Don’t check for similar records at all',
    'settings.dup_mode_warn'                      => 'Show a warning — they can still save it anyway',
    'settings.dup_picky_exact'                    => 'Only when the words match up dead tight',
    'settings.dup_picky_label'                    => 'How strict should the check be, fam?',
    'settings.dup_picky_similar'                  => 'Catch close misses too (spelling is a bit peak)',
    'settings.enc_ssl'                            => 'SSL (Port 465)',
    'settings.enc_tls'                            => 'TLS (Port 587)',
    'settings.error_log_tab'                      => 'Messed up log',
    'settings.existing_roles_heading'             => 'Current Roles & Management',
    'settings.footer_compiled_notice'             => 'Bottom footer source gaff',
    'settings.footer_notice_helper'               => 'Shows right at the very bottom of the gaff. Leave it blank if you want the default blurb from the lingo file. Software credits and copyright stay put.',
    'settings.hcaptcha_heading'                   => 'hCaptcha Setup',
    'settings.heading'                            => 'Global Site Settings, Modules & Permissions',
    'settings.mail_domain_label'                  => 'System Mail Domain (Fallback):',
    'settings.mail_driver_label'                  => 'Mail Engine:',
    'settings.mail_from_desc'                     => 'A dedicated address flashed as the sender for outgoing mail.',
    'settings.mail_from_label'                    => "Custom 'From' Email Addy:",
    'settings.mail_heading'                       => 'Mail Delivery Setup',
    'settings.maintenance_eta_label'              => 'Estimated Back Up Time (ETA):',
    'settings.maintenance_heading'                => 'System Lockdown Mode',
    'settings.maintenance_reason_label'           => 'Reason / Message for Visitors:',
    'settings.maintenance_toggle'                 => 'Enable Lockdown Mode (Take site offline for everyone)',
    'settings.map_geocode_api_key'                => 'Place search API key',
    'settings.map_geocode_api_key_help'           => 'Only needed for LocationIQ or OpenCage. Leave blank to keep your current key. Without a key, free Nominatim does the job.',
    'settings.map_geocode_locationiq'             => 'LocationIQ (API key required)',
    'settings.map_geocode_nominatim'              => 'Nominatim / OpenStreetMap (Free, rate-limited)',
    'settings.map_geocode_opencage'               => 'OpenCage (API key required)',
    'settings.map_geocode_provider'               => 'Place Search (Geocoding)',
    'settings.map_geocode_provider_help'          => 'Used when entering data and looking up a place name. Results get cached right here on the site.',
    'settings.map_key_placeholder'                => 'Paste the key if needed (leave blank to keep current)',
    'settings.map_provider_heading'               => 'Map Provider Setup',
    'settings.map_provider_intro'                 => 'Optional, bruv. Leave these blank to use the free built-in map imagery (CARTO/OpenStreetMap) and free place search (Nominatim). Paid keys stay strictly on your server and ain’t shared with anyone else.',
    'settings.map_tile_api_key'                   => 'Map tile API key',
    'settings.map_tile_api_key_help'              => 'Needed for CARTO, Mapbox or Stadia. Leave blank to keep your current key. Default OpenStreetMap don’t need jack.',
    'settings.map_tile_carto'                     => 'CARTO Voyager (Free Key Available From carto.com/basemaps/apikey)',
    'settings.map_tile_custom'                    => 'Custom tile URL',
    'settings.map_tile_default'                   => 'Default free (OpenStreetMap)',
    'settings.map_tile_mapbox'                    => 'Mapbox (API key required)',
    'settings.map_tile_osm'                       => 'OpenStreetMap (Free)',
    'settings.map_tile_provider'                  => 'Map imagery (tiles)',
    'settings.map_tile_provider_help'             => 'If a paid option ain’t got a key, pRD just drops back to the free default, safe.',
    'settings.map_tile_stadia'                    => 'Stadia Maps (API key required)',
    'settings.migration_confirm'                  => 'Did you grab a database backup first? This is gonna apply pending schema upgrades.',
    'settings.mod_feedback'                       => 'Feedback Submission',
    'settings.mod_feedback_desc'                  => 'Enables public feedback form and corresponding admin dashboard.',
    'settings.mod_leaderboard'                    => 'Leaderboard & Gamification',
    'settings.mod_leaderboard_desc'               => 'Gives props for grafting transcriptions and tracks scores.',
    'settings.mod_leaderboard_note'               => '(Needs User Management & Multi-User Access turned on)',
    'settings.mod_maps'                           => 'Maps',
    'settings.mod_maps_desc'                      => 'Shows a proper map for every table with a dynamic location column.',
    'settings.mod_moderation'                     => 'Moderation Workflow',
    'settings.mod_moderation_desc'                => 'Enables edit suggestion reviews and the security queue.',
    'settings.mod_users'                          => 'User Management & Multi-User Access',
    'settings.mod_users_desc'                     => 'Enables signups, user tracking, and multi-user authentication.',
    'settings.mod_volunteers'                     => 'Graft Portal & Applications',
    'settings.mod_volunteers_desc'                => 'Enables public volunteer intent form and admin dashboard.',
    'settings.modules_heading'                    => 'App Module Switches & Performance Control',
    'settings.modules_subheading'                 => 'Flip features on or off to speed up performance and trim down what you don’t need.',
    'settings.no_audit_logs'                      => 'No audit logs found on CCTV.',
    'settings.no_notices'                         => 'No alerts configured.',
    'settings.notice_content_label'               => 'Content:',
    'settings.notices_heading'                    => 'Site Notices & Banners',
    'settings.perm_info_mark'                     => 'i',
    'settings.perm_locked'                        => 'This permission is dead mandatory for Admins.',
    'settings.perm_more_info'                     => 'More about',
    'settings.perm_needs'                         => 'Requires',
    'settings.perm_needs_parent'                  => 'Turn that on first, fam.',
    'settings.perm_no_links'                      => 'This permission doesn’t rely on another one.',
    'settings.perm_save_failed'                   => 'Gutted - pRD couldn’t save this setting, bruv.',
    'settings.perm_saved'                         => 'Saved',
    'settings.perm_swipe'                         => 'On a tiny screen, swipe this list sideways if you need a gander.',
    'settings.perm_used_by'                       => 'Needed by',
    'settings.permissions_heading'                => 'Dynamic Ranks & Permissions Matrix',
    'settings.permissions_subheading'             => 'Permissions are grouped by system capabilities. Expand boxes to tweak what each rank can do and save below.',
    'settings.port_label'                         => 'Port:',
    'settings.purge_all_confirm'                  => '⚠️ WARNING: This will WIPE EVERY SINGLE SYSTEM AUDIT LOG FOREVER. Are you sure?',
    'settings.purge_records_confirm'              => 'Sure you want to wipe all audit logs tied to records?',
    'settings.recaptcha_heading'                  => 'Google reCAPTCHA Setup',
    'settings.role_name_label'                    => 'Role Name Bruv',
    'settings.run_token_cleanup_btn'              => 'Run Token Scrub Right Now',
    'settings.save_core_mail_btn'                 => 'Save Core & Mail Setup',
    'settings.save_maintenance_btn'               => 'Save Lockdown Setup',
    'settings.save_modules_btn'                   => 'Save Module Setup',
    'settings.save_notice_btn'                    => 'Save Alert',
    'settings.save_permissions_btn'               => 'Save Permissions Matrix',
    'settings.saved'                              => 'Saved.',
    'settings.schema_current'                     => 'Current schema version:',
    'settings.schema_latest'                      => 'Latest available build:',
    'settings.schema_update_notice'               => 'Database updates are ready to roll. Grab a backup above before you touch anything.',
    'settings.schema_uptodate'                    => 'Database is fully up to date.',
    'settings.secret_key_label'                   => 'Secret Key (Private):',
    'settings.send_test_btn'                      => 'Fire Test Email',
    'settings.site_key_label'                     => 'Site Key (Public):',
    'settings.smtp_encryption_label'              => 'Encryption:',
    'settings.smtp_heading'                       => 'SMTP Server Setup',
    'settings.smtp_host_label'                    => 'SMTP Host geezer:',
    'settings.smtp_pass_label'                    => 'SMTP Password (leave blank to keep current):',
    'settings.smtp_port_label'                    => 'Port:',
    'settings.smtp_user_label'                    => 'SMTP Username:',
    'settings.status_active'                      => 'Active',
    'settings.status_inactive'                    => 'Dormant',
    'settings.subheading'                         => 'Manage the look, core setup, mail drivers, security/CAPTCHA gear, modules, maintenance mode, site shouts, and role permissions.',
    'settings.sys_name_label'                     => 'System / App Name:',
    'settings.system_guest'                       => 'System / Random Guest',
    'settings.tab_appearance'                     => 'Look',
    'settings.tab_audit'                          => 'Audit Log (CCTV)',
    'settings.tab_core'                           => 'Core & Mail',
    'settings.tab_maintenance'                    => 'Lockdown Mode',
    'settings.tab_modules'                        => 'Modules',
    'settings.tab_notices'                        => 'Site Alerts',
    'settings.tab_permissions'                    => 'Ranks & Permissions',
    'settings.test_email_label'                   => 'Recipient Email Addy:',
    'settings.test_mail_heading'                  => 'Test Email Delivery',
    'settings.th_action'                          => 'Action',
    'settings.th_actions'                         => 'Moves',
    'settings.th_actor'                           => 'Who Did It',
    'settings.th_capabilities'                    => 'Powers Assigned to This Crew',
    'settings.th_details'                         => 'Details',
    'settings.th_id'                              => 'ID',
    'settings.th_ip'                              => 'IP Address',
    'settings.th_record_id'                       => 'Record ID',
    'settings.th_role'                            => 'Rank',
    'settings.th_timestamp'                       => 'Timestamp',
    'settings.turnstile_heading'                  => 'Cloudflare Turnstile Setup',
    'settings.update_db_btn'                      => 'Upgrade Database Now',
    'update_database.emergency_active'            => 'Database update access is wide open so you can finish this step without logging in. It’ll shut itself once the database is proper fresh.',
    'updates.already_current'                     => 'Files already match the selected update channel. Fuck all changed.',
    'updates.apply_btn'                           => 'Apply File Package',
    'updates.archive_failed'                      => 'Gutted - pRD couldn’t write the private database copy. File update didn’t even start.',
    'updates.archive_missing'                     => 'Gutted - couldn’t find that private backup, mate.',
    'updates.channel_help'                        => 'Don’t muck about with the update channel unless you’re a dev. Devs can stick a tag or a commit reference in this box.',
    'updates.channel_label'                       => 'Update Channel (This should usually stay as "Main", innit)',
    'updates.channel_saved'                       => 'Update Channel Saved.',
    'updates.confirm'                             => 'Wanna apply the file package now? Don’t sweat it, your records are safe. Download a copy of your database first just in case, yeah?',
    'updates.db_behind'                           => 'Database: An update is waiting for ya',
    'updates.db_pending'                          => 'If pRD asks you to update the database next, open that page. You don’t need to make any extra files.',
    'updates.db_ran'                              => 'The database got updated too, safe.',
    'updates.download_heading'                    => 'Downloading Your Database, Hold Tight..',
    'updates.download_please'                     => 'Grab your database first, mate. Keep your own copy as well as the server backup.',
    'updates.files_behind'                        => 'Files: A fresher package is available',
    'updates.files_current'                       => 'Files: Bang up to date with the selected update channel',
    'updates.files_error'                         => 'Files: Safe, pRD couldn’t reach GitHub just now. Have another go later, bruv.',
    'updates.files_ok'                            => 'Absolute belter! File Package Applied.',
    'updates.files_unknown'                       => 'Files: Not checked yet (apply a package once to start tracking)',
    'updates.force_apply'                         => 'Force Replace Files (Only if something looks proper mashed)',
    'updates.heading'                             => 'File Updates',
    'updates.help'                                => 'Download your database first, fam. pRD keeps a private backup on the server too. Updating files doesn’t touch your records. Once files are updated, hit "Update Database" if a newer database version is hanging about.',
    'updates.no_sha'                              => 'No Package Recorded Just Yet, Safe',
    'updates.now_db'                              => 'If the database version is behind, hit "Update database" straight after.',
    'updates.page_heading'                        => 'Upgrading pRD',
    'updates.private_copies'                      => 'Private Server Copies (Gaffers Only)',
    'updates.running'                             => 'This Gaff Is Live',

    // ------------------------------------------------------------------
    // 19. Admin: User Account Management & Leaderboard Moderation
    // ------------------------------------------------------------------
    'admin_users.delete_btn'                      => 'Bin it',
    'admin_users.disabled'                        => 'Off',
    'admin_users.email_for'                       => 'Email for',
    'admin_users.enabled'                         => 'On',
    'admin_users.find_user'                       => 'Track down a geezer',
    'admin_users.heading'                         => 'User Account Management & Rep Moderation',
    'admin_users.invite_user_btn'                 => 'Invite New Mate',
    'admin_users.manage_templates_btn'            => 'Sort Email Slang Templates',
    'admin_users.no'                              => 'Nah',
    'admin_users.no_search_match'                 => 'Found absolute jack shit matching that search.',
    'admin_users.no_users'                        => 'No users found.',
    'admin_users.not_verified_badge'              => 'Unverified, Init',
    'admin_users.points_for'                      => 'Points for',
    'admin_users.protected_admin'                 => 'Protected Top Boss',
    'admin_users.reactivate_btn'                  => 'Unfreeze',
    'admin_users.resend_invite_btn'               => 'Re-send Invite',
    'admin_users.resend_invite_confirm'           => 'Re-send account invite email to this user?',
    'admin_users.reset_2fa_btn'                   => 'Turn Off 2FA',
    'admin_users.reset_2fa_confirm'               => 'Nuke and turn off 2FA for this user?',
    'admin_users.reset_password_btn'              => 'Reset Password',
    'admin_users.reset_pwd_confirm'               => 'Fire a password reset link to this user?',
    'admin_users.save_email_title'                => 'Save New Email Addy',
    'admin_users.search_help'                     => 'Filters the list as you bash the keyboard. Clear the box to see everyone again.',
    'admin_users.search_placeholder'              => 'Username, email, or what they do…',
    'admin_users.set_score_btn'                   => 'Set Rep Score',
    'admin_users.status_active'                   => 'Active',
    'admin_users.status_suspended'                => 'Frozen / Banned',
    'admin_users.subheading'                      => 'Check user status, switch roles, override emails, trigger password resets or re-invites, clear 2FA, or freeze accounts.',
    'admin_users.suspend_btn'                     => 'Freeze Account',
    'admin_users.suspend_confirm'                 => 'Freeze user and lock them out due to messing about?',
    'admin_users.th_2fa'                          => '2FA Lock',
    'admin_users.th_actions'                      => 'Actions & Security',
    'admin_users.th_email_override'               => 'Email & Override',
    'admin_users.th_role_assignment'              => 'Assign Rank',
    'admin_users.th_score'                        => 'Rep Score',
    'admin_users.th_status'                       => 'Status',
    'admin_users.th_username'                     => 'Username',
    'admin_users.update_btn'                      => 'Update',
    'admin_users.verified_badge'                  => 'Proper Verified',
    'admin_users.verified_label'                  => 'Verified:',
    'admin_users.yes'                             => 'Yeah',

    // ------------------------------------------------------------------
    // 20. Admin: View Ticket & Threaded Dialogue
    // ------------------------------------------------------------------
    'view_ticket.admin_label'                     => 'Top Boss',
    'view_ticket.back_to_dashboard'               => 'Return to Beef Deck',
    'view_ticket.dialogue_heading'                => 'Back-and-Forth Thread',
    'view_ticket.no_replies'                      => 'No replies logged yet.',
    'view_ticket.on_date'                         => 'on',
    'view_ticket.post_reply_heading'              => 'Drop Reply & Email Whistleblower',
    'view_ticket.reply_placeholder'               => 'Type your reply here bruv...',
    'view_ticket.send_reply_btn'                  => 'Send Reply & Email Submitter',
    'view_ticket.staff'                           => 'Crew Member',
    'view_ticket.status_completed'                => 'Sorted / Done',
    'view_ticket.status_pending'                  => 'Pending',
    'view_ticket.status_progress'                 => 'In Progress (Working it)',
    'view_ticket.status_rejected'                 => 'Scrapped',
    'view_ticket.submitted_by'                    => 'Dropped by:',
    'view_ticket.submitted_fields'                => 'Submitted Form Bits:',
    'view_ticket.support_request'                 => 'Support Issue',
    'view_ticket.ticket_heading_prefix'           => 'Ticket',
    'view_ticket.ticket_status_label'             => 'Ticket Status:',

    // ------------------------------------------------------------------
    // 21. Admin: Volunteer Submissions & Workflow Dashboard
    // ------------------------------------------------------------------
    'volunteer_dashboard.accept_invite_btn'       => 'Approve & Fire Invite',
    'volunteer_dashboard.accept_title'            => 'Approve via Recruitment Invite System',
    'volunteer_dashboard.chat_label'              => 'Chat:',
    'volunteer_dashboard.chat_notes_btn'          => 'Chat & Notes',
    'volunteer_dashboard.delete_confirm'          => 'Wipe this graft record?',
    'volunteer_dashboard.heading'                 => 'Graft Submissions & Workflow',
    'volunteer_dashboard.manage_emails_btn'       => 'Sort Email Slang Templates',
    'volunteer_dashboard.manage_schema_btn'       => 'Sort Form Blueprint',
    'volunteer_dashboard.modal_date_label'        => 'Booked Chat Date & Time:',
    'volunteer_dashboard.modal_heading'           => 'Manage Candidate Interview & Notes',
    'volunteer_dashboard.modal_notes_label'       => 'Interview / Meet Notes:',
    'volunteer_dashboard.modal_notes_placeholder' => 'Jot down how the chat went here...',
    'volunteer_dashboard.modal_status_label'      => 'App Status:',
    'volunteer_dashboard.no_notes'                => 'No notes yet',
    'volunteer_dashboard.no_submissions'          => 'No graft apps found.',
    'volunteer_dashboard.notes_label'             => 'Notes:',
    'volunteer_dashboard.save_changes_btn'        => 'Save Tweaks',
    'volunteer_dashboard.status_accepted'         => 'Approved',
    'volunteer_dashboard.status_chat'             => 'Chat Booked In',
    'volunteer_dashboard.status_pending'          => 'Pending Review',
    'volunteer_dashboard.status_rejected'         => 'Scrapped',
    'volunteer_dashboard.subheading'              => 'Review graft apps, set up chats, record interview notes, and sign off candidates into the system.',
    'volunteer_dashboard.th_interview_notes'      => 'Interview / Notes',
    'volunteer_dashboard.th_name'                 => 'Name',
    'volunteer_dashboard.th_status'               => 'Status',
    'volunteer_dashboard.volunteer_prefix'        => 'Graft Worker',

    // ------------------------------------------------------------------
    // 22. API: AJAX Search & Filtering
    // ------------------------------------------------------------------
    'api_search.error_public_forbidden'           => '403 Blocked: Public noseying is turned off.',
    'api_search.error_unauthorized_table'         => 'Unauthorized table access. Jog on.',
    'api_search.history_btn'                      => 'CCTV History',
    'api_search.no_records'                       => 'No records found in this directory.',
    'api_search.suggest_edit_btn'                 => 'Suggest Twist',

    // ------------------------------------------------------------------
    // 23. Errors & HTTP Templates
    // ------------------------------------------------------------------
    'admin_errors.btn_find'                       => 'Find it',
    'admin_errors.file'                           => 'File',
    'admin_errors.found_heading'                  => 'Jackpot:',
    'admin_errors.heading'                        => 'Messed up log',
    'admin_errors.label_error_id'                 => 'Error reference ID',
    'admin_errors.line_prefix'                    => 'Line',
    'admin_errors.local_time'                     => 'Your local time',
    'admin_errors.message'                        => 'The beef / Message',
    'admin_errors.no_recent'                      => 'Clean sheet, no entries in the structured error log yet.',
    'admin_errors.not_found'                      => 'Found zero log entries for',
    'admin_errors.placeholder_id'                 => 'E-YYYYMMDD-XXXXXX',
    'admin_errors.recent_heading'                 => 'Fresh disasters',
    'admin_errors.request'                        => 'Request',
    'admin_errors.stack_trace'                    => 'The trail of doom',
    'admin_errors.subheading'                     => 'Have a butcher’s at the reference ID on the error screen (e.g. E-20260810-A1B2C3). Full details are banged into the server log; what randoms see on screen depends on APP_DEBUG.',
    'admin_errors.th_id'                          => 'ID',
    'admin_errors.th_message'                     => 'The beef',
    'admin_errors.th_time'                        => 'Time',
    'admin_errors.time_utc'                       => 'Time (UTC)',
    'admin_errors.type'                           => 'Sort / Type',
    'error_template.debug_details'                => 'Debugging bits',
    'error_template.file_label'                   => 'File:',
    'error_template.line_label'                   => 'Line',
    'error_template.return_home_btn'              => 'Get Back to Public Yard',
    'error_template.stack_trace'                  => 'The trail of doom',

    // ------------------------------------------------------------------
    // 24. Public: Ticket Intake & Feedback Portal
    // ------------------------------------------------------------------
    'feedback.email_label'                        => 'Email Addy:',
    'feedback.first_name_label'                   => 'First Name:',
    'feedback.hp_label'                           => 'Leave this empty you mug',
    'feedback.multi_select_hint'                  => 'Hold Ctrl or Cmd down to grab multiple.',
    'feedback.required_title'                     => 'Must Have Field',
    'feedback.select_placeholder'                 => '-- Pick One --',
    'feedback.subject_label'                      => 'Subject / Issue Title:',
    'feedback.submit_btn'                         => 'Fire Off Ticket',
    'feedback.surname_label'                      => 'Last Name:',

    // ------------------------------------------------------------------
    // 25. Centralised Feedback System (gh.*) ---
    // ------------------------------------------------------------------
    'gh.action_placeholder'                       => 'Describe your moves so we can track down the rogue files',
    'gh.add_comment_below'                        => 'Type your message below and send it flying.',
    'gh.additional_context'                       => 'Extra juice',
    'gh.additional_placeholder'                   => 'Slap any extra gossip right here.',
    'gh.attach_logs'                              => 'Attach a cheeky system log snippet',
    'gh.bug_desc_placeholder'                     => 'Keep it brief and don\'t chat shit, what\'s actually broke?',
    'gh.click_to_comment'                         => 'Click to jump on their thread instead →',
    'gh.comment_help'                             => 'This gets slapped onto the existing GitHub issue. It don\'t change the tags or how high priority it is.',
    'gh.comment_label'                            => 'Your two pence',
    'gh.commenting_on'                            => 'Chucking your two pence on an existing issue',
    'gh.consent_agree'                            => 'Yeah alright, let\'s roll with the Feedback Shack under these rules.',
    'gh.consent_text'                             => 'This ting connects to the pRD server just to prove you ain’t chatting absolute wet. No personal chat gets leaked to the server during this bit, but your report is gonna be banging around the public internet for everyone to see. Press start if you’re down.',
    'gh.consent_title'                            => 'Oi, System Link & Permission Check',
    'gh.created_success'                          => 'created proper safe.',
    'gh.creating_new'                             => 'Makin\' a fresh issue, bruv',
    'gh.detailed_description'                     => 'Give us the full gen',
    'gh.doc_paragraphs'                           => 'Drop the paragraph numbers or quote the dodgy bit below:',
    'gh.doc_placeholder'                          => 'Wiki page name or link',
    'gh.doc_proposed'                             => 'What should this bit actually say instead?',
    'gh.doc_reasoning'                            => 'Why you changing it, then?',
    'gh.doc_related'                              => 'What page in the Wiki are we chatting about?',
    'gh.duplicate_confirm_text'                   => 'Sound like someone already banged on about this. I swear down this is a completely separate issue.',
    'gh.duplicate_warning'                        => 'Madness, that\'s a double',
    'gh.error_occurred'                           => 'Absolute nightmare, an error popped up.',
    'gh.expected_behavior'                        => 'What was supposed to pop off',
    'gh.expected_placeholder'                     => 'What you actually wanted to happen instead of this shambles.',
    'gh.failed_check_issues'                      => 'Couldn\'t check the existing issues, safe to say it\'s fucked.',
    'gh.feature_problem'                          => 'Is this upgrade request tied to a proper headache? Spill the beans.',
    'gh.feature_solution'                         => 'How do you want it fixed, then?',
    'gh.heading'                                  => 'pRD Proper Heavy Feedback Shack',
    'gh.intro_p1'                                 => 'Big up for trusting pRD with your data yard, fam. This don’t is strictly for you, the big boss, innit.',
    'gh.intro_step1'                              => 'This ting links straight to GitHub to check if anyone else is muggin’ off about the same glitch.',
    'gh.intro_step2'                              => 'Chuck your two pence in on an existing ticket if your drama ain’t fully sorted yet.',
    'gh.intro_step3'                              => 'Or if you\'ve found a fresh madness, this ting lets you drop a brand new issue on GitHub so the pRD mandem can sort it.',
    'gh.issue_created_label'                      => 'Issue',
    'gh.locked_onto_issue'                        => 'Locked onto target issue',
    'gh.make_note_link'                           => 'Screenshot this link so you can check back later.',
    'gh.network_error'                            => 'Wi-Fi dropped out while processing that, typical.',
    'gh.no_recent_logs'                           => 'Ain\'t found fuck all in the logs for the last 48 hours.',
    'gh.no_similar_found'                         => 'Clean as a whistle, no matching issues found.',
    'gh.open_on_github'                           => 'Open issue in a fresh tab',
    'gh.optional_diagnostic'                      => 'optional nerd diagnostics for the tech heads',
    'gh.paste_placeholder'                        => 'Drop the text here',
    'gh.post_comment_btn'                         => 'Post Your Chat to the Issue',
    'gh.posting_comment'                          => 'Posting your chat...',
    'gh.preview_back'                             => 'Back to the form',
    'gh.preview_body_label'                       => 'Body (edit or strip out your personal info here)',
    'gh.preview_heading'                          => 'Have a proper butcher\'s before you blast it off',
    'gh.preview_reminder'                         => 'Have a quick read-through before you smash send. This text is gonna be completely public on GitHub, so strip out your government name and postcode if you don\'t want man knowing where you live. Team checkboxes get wiped when it sends.',
    'gh.preview_send'                             => 'Blast it to GitHub',
    'gh.preview_title_label'                      => 'Title',
    'gh.proposed_placeholder'                     => 'Slap the new text right here',
    'gh.public_confirm'                           => 'I\'ve checked the preview and I know everyone\'s gonna see this',
    'gh.quote_placeholder'                        => 'Paragraph reference or quote',
    'gh.reasoning_placeholder'                    => 'Give us your logic, fam',
    'gh.save_tracking_link'                       => 'Save this tracking link or lose it:',
    'gh.select_audit_logs'                        => 'Grab the relevant CCTV logs (Last 48 Hours)',
    'gh.select_audit_logs_desc'                   => 'Tick any recent log events below that match this drama so the mandem got some context.',
    'gh.select_type'                              => 'Pick your poison',
    'gh.sending'                                  => 'Sending it over…',
    'gh.sensitive_confirm'                        => 'Yeah I know it\'s dodgy and I still wanna post it',
    'gh.sensitive_warn'                           => 'Oi! This text looks like it\'s leaking passwords or secret keys. Delete that shite or tick below if you\'re bare reckless.',
    'gh.sev_high'                                 => 'Absolute emergency (priority: high)',
    'gh.sev_high_desc'                            => 'Security meltdown and/or completely bricked feature, blud.',
    'gh.sev_low'                                  => 'Bare minor (priority: low)',
    'gh.sev_low_desc'                             => 'ONLY IF — Ain\'t no security breach and/or the whole ting ain\'t completely in the bin.',
    'gh.sev_med'                                  => 'Proper annoying (priority: medium)',
    'gh.sev_med_desc'                             => 'ONLY IF — Still not a hack, but a massive chunk of the yard is properly dead.',
    'gh.severity_level'                           => 'How fucked is it?',
    'gh.similar_issues_desc'                      => 'As you type out your title, we\'re checking if anyone else is crying about it. Click an issue below if you wanna jump on their bandwagon instead of making a clone.',
    'gh.similar_issues_title'                     => 'Other geezers reported similar shite',
    'gh.solution_placeholder'                     => 'Give us the dream scenario, keep it sweet.',
    'gh.steps_to_reproduce'                       => 'How to trigger this bullshit',
    'gh.submit_btn'                               => 'Send It',
    'gh.submitting'                               => 'Sending it over...',
    'gh.switch_new'                               => 'Switch over to a new issue',
    'gh.title_placeholder'                        => 'Brief vibe check of the problem',
    'gh.title_summary'                            => 'Title / Quick Summary',
    'gh.tracking_link_aria'                       => 'GitHub tracking link for your freshly minted issue',
    'gh.trans_action'                             => 'What were you actually up to in pRD when it went left?',
    'gh.trans_offending'                          => 'Copy and paste that dead text right here:',
    'gh.type_at_least_3'                          => 'Type at least 3 letters, bruv...',
    'gh.type_bug'                                 => '1. Proper Glitch / Bug Report',
    'gh.type_documentation'                       => '3. Fix Up the Manual / Docs',
    'gh.type_enhancement'                         => '2. Fresh Upgrade or Feature Shout',
    'gh.type_to_search'                           => 'Start typing a title to snoop...',
    'gh.type_translation'                         => '4. Slap on a New Lingo Translation',

    // ------------------------------------------------------------------
    // 26. Security Engine & Firewall
    // ------------------------------------------------------------------
    'security_engine.err_access_denied'           => 'Security Alarm: Access denied, jog on.',
    'security_engine.err_captcha_failed'          => 'Bot check failed. Try again.',
    'security_engine.err_complete_captcha'        => 'Complete the bot blocker check first geezer.',
    'security_engine.err_excessive_links'         => 'Submission blocked because it’s jammed full of spammy links.',
    'security_engine.err_rate_limit'              => 'Too many clicks from your IP address bruv. Wind your neck in and try later.',
    'security_engine.err_suspicious_agent'        => 'Security Alarm: Sketchy client signature detected.',

    // ------------------------------------------------------------------
    // 27. Installer Wizard
    // ------------------------------------------------------------------
    'install.admin_confirm_password_label'        => 'Confirm Boss Password',
    'install.admin_email_label'                   => 'Boss Email',
    'install.admin_heading'                       => 'Main Boss Site Account',
    'install.admin_password_label'                => 'Boss Password (min. 8 characters)',
    'install.admin_subheading'                    => 'These are your sign-in details for this site (not a database account).',
    'install.admin_username_label'                => 'Boss Username',
    'install.back_btn'                            => 'Whip it back',
    'install.close_alert'                         => 'Shut it',
    'install.complete_desc'                       => 'Your pRD install is all sorted and the installer is locked down so nobody can run it again by accident.',
    'install.complete_heading'                    => 'Absolute Belter!',
    'install.complete_title'                      => 'Setup Done & Dusted',
    'install.continue_btn'                        => 'Keep Going',
    'install.db_heading'                          => 'Database Hookup',
    'install.db_hint'                             => 'Grab MySQL details from your hosting control panel. This is not your site login.',
    'install.db_host_label'                       => 'Database Host',
    'install.db_name_label'                       => 'Database Name',
    'install.db_pass_label'                       => 'Database Password',
    'install.db_submit_btn'                       => 'Build Tables & Keep Going',
    'install.db_user_label'                       => 'Database Username',
    'install.delete_folder_hint'                  => 'Loads of hosts won’t let the installer delete its own folder. If the button messes about and does nothing, open your hosting file manager or FTP, head to your pRD project folder, and delete or rename the install directory yourself, mate.',
    'install.demo_choose'                         => 'Which packs you tryin\' to load up?',
    'install.demo_continue'                       => 'Load selected packs and wrap it up',
    'install.demo_heading'                        => 'Optional Demo Packs (For a laugh)',
    'install.demo_help'                           => 'Chuck in some starter tables for parish logs or a book library if you\'re feeling lazy. Pick tables and columns only, or grab some fake sample rows to fill the space. You can skip this and chuck or scrap packs later in Settings. If you bin a pack later, it deletes any extra rows you jammed in those demo tables too.',
    'install.demo_library'                        => 'Book Library Demo',
    'install.demo_library_desc'                   => 'A proper basic catalogue of titles, writers, and shelf spots you can mess around with.',
    'install.demo_parish'                         => 'Parish Records Demo',
    'install.demo_parish_desc'                    => 'Baptisms, Weddins, and Plantings — The exact hustle pRD was built for!',
    'install.demo_schema_data'                    => 'Tables, Columns, and Fake Sample Data',
    'install.demo_schema_only'                    => 'Tables and Columns only (keep it completely empty)',
    'install.demo_skip'                           => 'Skip for now',
    'install.demo_what'                           => 'What\'s the play? What you want to add?',
    'install.detail_prefix'                       => 'Detail:',
    'install.done_heading'                        => 'Absolute scenes! pRD is fully installed.',
    'install.done_message'                        => 'The installer is locked down tight now. Log in with that admin account you just sorted out.',
    'install.err_admin_required'                  => 'All administrator fields are required.',
    'install.err_admin_save_failed'               => 'Failed to save admin user. Check your users table setup.',
    'install.err_complete_db_first'               => 'Finish the database setup step first.',
    'install.err_config_load'                     => 'Couldn’t load current config:',
    'install.err_db_not_empty'                    => 'This database isn’t empty mate. Hook up a fresh empty database (or wipe all tables) and try again.',
    'install.err_db_required'                     => 'Database name and username are mandatory.',
    'install.err_install_not_removed'             => 'pRD couldn\'t automatically nuke the install folder (server\'s being muggy). Go into your file manager and delete or rename it manually. Your site data is safe though.',
    'install.err_invalid_email'                   => 'Bent email address.',
    'install.err_password_length'                 => 'Password needs to be at least 8 characters long.',
    'install.err_passwords_match'                 => 'Passwords don’t match up.',
    'install.err_write_permission'                => 'PHP can’t write files in this project folder. Fix your permissions.',
    'install.finish_btn'                          => 'Finish Setup',
    'install.heading'                             => 'Welcome to the pRD Setup Shack, fam.',
    'install.hide_password'                       => 'Lock it down',
    'install.home_link'                           => 'Go to Yard',
    'install.lang_label'                          => 'Lingo',
    'install.login_link'                          => 'Log In',
    'install.mod_feedback'                        => 'Feedback Submissions',
    'install.mod_feedback_desc'                   => 'Enables a public feedback form and an admin tracking dashboard.',
    'install.mod_leaderboard'                     => 'Leaderboard & Gamification',
    'install.mod_leaderboard_desc'                => 'Optional points table. Needs user accounts turned on.',
    'install.mod_maps'                            => 'Maps',
    'install.mod_maps_desc'                       => 'Location fields and a map per table. You can change this later under Admin → Settings.',
    'install.mod_moderation'                      => 'Moderation Workflow',
    'install.mod_moderation_desc'                 => 'A moderation workflow system letting users suggest edits while a gaffer approves or bins the lot.',
    'install.mod_users'                           => 'User Management & Multi-User Access',
    'install.mod_users_desc'                      => 'Let people sign up and log in, innit.',
    'install.mod_volunteers'                      => 'Volunteer Portal & Submissions',
    'install.mod_volunteers_desc'                 => 'Enables a public volunteer interest form and an admin management dashboard.',
    'install.modules_continue'                    => 'Save and keep it moving',
    'install.modules_heading'                     => 'Which parts of pRD you actually trying to use?',
    'install.modules_help'                        => 'Tick off the features you want from day one, bruv. You can change this later down admin → site settings.',
    'install.modules_later'                       => 'Turn any of these modules on or off later in Admin → Site Settings.',
    'install.msg_admin_created'                   => 'Admin account is locked and loaded. Chuck in some demo packs or bounce straight past.',
    'install.msg_db_ready'                        => 'Database is locked and loaded. Set up your main boss account to finish up.',
    'install.msg_install_removed'                 => 'Install folder is gone for good. You can sign in now.',
    'install.msg_installation_complete'           => 'Setup complete.',
    'install.msg_modules_saved'                   => 'Your module choices are locked in. Chuck in optional demo packs or skip this step if you can’t be arsed.',
    'install.msg_schema_imported'                 => 'Database wired up and schema pulled in. Now make your boss account.',
    'install.page_title'                          => 'Setup — Parish Records Directory',
    'install.remove_already_gone'                 => 'The install folder was already binned off.',
    'install.remove_failed'                       => 'Gutted - pRD couldn’t delete the install folder automatically (web hosts are useless like that). Jump into your hosting file manager or use FTP, open the pRD folder and delete or rename the install directory, bruv.',
    'install.remove_folder_btn'                   => 'Oi, delete that install folder!',
    'install.remove_ok'                           => 'The install folder has been binned off.',
    'install.req_all_ok'                          => 'Absolute winner, all checks came back clean. Keep it movin\'.',
    'install.req_fail_msg'                        => 'Safe to say your server ain\'t cutting the mustard. Sort these bits out and run the installer back. Ain\'t touched your data yet.',
    'install.req_heading'                         => 'pRD\'s done a background check on your server setup.',
    'install.req_logs'                            => 'Writable logs folder (or project folder)',
    'install.req_logs_fail'                       => 'Logs folder is locked down read-only — Nightmare.',
    'install.req_logs_ok'                         => 'Logs folder is fully writable — Happy days!',
    'install.req_pdo'                             => 'PDO MySQL Extension',
    'install.req_pdo_fail'                        => 'PDO MySQL Extension — Missing in action, sort it out.',
    'install.req_pdo_ok'                          => 'PDO MySQL Extension — Looking lush!',
    'install.req_php'                             => 'PHP 8.0+ (detected %s)',
    'install.req_php_fail'                        => 'pRD needs PHP 8.0+ — Unlucky bruv, your PHP is ancient.',
    'install.req_php_ok'                          => 'pRD needs PHP 8.0+ — Proper job!',
    'install.req_probe'                           => 'Permission to write files in this project folder',
    'install.req_probe_fail'                      => 'Can\'t make new files in this project folder — Server is being a melt.',
    'install.req_probe_ok'                        => 'You can drop new files in this project folder — Safe!',
    'install.show_password'                       => 'Let it peak',
    'install.subheading'                          => 'Make sure your database yard is completely empty before you keep pushing.',

    // ------------------------------------------------------------------
    // 28. Demo Packs
    // ------------------------------------------------------------------
    'demo.already_installed'                      => 'This demo is already bolted on',
    'demo.already_installed_hint'                 => 'This pack is already installed, stop trying to double dip.',
    'demo.choose_packs'                           => 'Which packs you want, chief?',
    'demo.heading'                                => 'Demo Packs',
    'demo.install_btn'                            => 'Load Selected Packs',
    'demo.install_heading'                        => 'Load a Demo Pack',
    'demo.install_help'                           => 'Tables and columns only: bare bones structure for you to fill. Tables, columns, and sample data: a bunch of fake rows so you can start clicking around immediately. You can strip out the sample rows later without losing your tables.',
    'demo.intro'                                  => 'Optional dummy tables so you can test drive pRD before you build your own empire. Add one or both, makes no odds. None of this is mandatory for a live site.',
    'demo.none_installed'                         => '???',
    'demo.remove_data_btn'                        => 'Bin the Sample Data Only',
    'demo.remove_data_help'                       => 'Wipes out the original dummy rows only. Your tables, columns, and anything you added yourself stay untouched.',
    'demo.remove_heading'                         => 'Nuke a Demo Pack',
    'demo.remove_pack_btn'                        => 'Nuke Tables, Columns, and Everything in this Pack',
    'demo.remove_pack_confirm'                    => 'This completely vaporises the demo tables, columns, sample rows, and any extra chat you put in those demo tables. Rest of your site is untouched. You absolutely sure?',
    'demo.remove_pack_help'                       => 'Deletes the demo tables, columns, sample rows, and whatever else you typed into those specific demo tables since. Your other tables are safe.',
    'demo.schema_and_data'                        => 'Tables, Columns, and Sample Data',
    'demo.schema_only'                            => 'Tables and Columns Only (Zero Sample Rows)',
    'demo.what_to_add'                            => 'What to sling in',

    // ------------------------------------------------------------------
    // 29. Leaderboard
    // ------------------------------------------------------------------
    'leaderboard.aria_region'                     => 'Top Boy Leaderboard View',
    'leaderboard.default_role'                    => 'Mate',
    'leaderboard.heading'                         => 'Community Graft & Rep Leaderboard',
    'leaderboard.medal_bronze'                    => 'Bronze Bling',
    'leaderboard.medal_crown'                     => 'Level 10 Crown (The Don)',
    'leaderboard.medal_glowing'                   => 'Level 9 Glowing Star',
    'leaderboard.medal_gold'                      => 'Gold Bling',
    'leaderboard.medal_military'                  => 'Level 8 Military Medal',
    'leaderboard.medal_ribbon'                    => 'Level 4 Ribbon',
    'leaderboard.medal_rosette'                   => 'Level 5 Rosette',
    'leaderboard.medal_silver'                    => 'Silver Bling',
    'leaderboard.medal_star'                      => 'Level 7 Star',
    'leaderboard.medal_trophy'                    => 'Level 6 Trophy',
    'leaderboard.no_users'                        => 'No active legends on the leaderboard yet.',
    'leaderboard.subheading'                      => 'Bigging up the crew members who help log, transcribe, or sort out database records.',
    'leaderboard.th_contributor'                  => 'Top Boy / Contributor',
    'leaderboard.th_rank'                         => 'Rank',
    'leaderboard.th_role'                         => 'Rank',
    'leaderboard.th_score'                        => 'Rep Score',
    'leaderboard.you_badge'                       => '(That’s You)',

    // ------------------------------------------------------------------
    // 30. Site Footer
    // ------------------------------------------------------------------
    'footer.compiled_notice'                      => 'Your footer goes here bruv!',
    'footer.rights_reserved'                      => 'All rights reserved bruv.',
    'footer.software_notice'                      => 'pRD (V1.18) is open-source under the MIT License - With thanks to GitHub\'s @katherinehuk for her second pair of eyes!',

    // ------------------------------------------------------------------
    // 31. Site Header & Head
    // ------------------------------------------------------------------
    'header.default_title'                        => 'Parish Records Database',
    'header.skip_to_content'                      => 'Skip straight to the good stuff',

    // ------------------------------------------------------------------
    // 32. Notices Banner Module
    // ------------------------------------------------------------------
    'notices_banner.close_title'                  => 'Shut notice',

    // ------------------------------------------------------------------
    // 33. Record History & Audit Trail
    // ------------------------------------------------------------------
    'record_history.actor_label'                  => 'Who Did It:',
    'record_history.directory_table_label'        => 'Directory Table:',
    'record_history.empty_value'                  => '[Blank]',
    'record_history.exit_no_record'               => 'No record specified.',
    'record_history.exit_not_found'               => 'Record not found anywhere.',
    'record_history.heading_prefix'               => 'CCTV History & Trail: Record',
    'record_history.no_history'                   => 'No CCTV history events logged for this record yet.',
    'record_history.proposed_value'               => 'Proposed Value:',
    'record_history.purge_btn'                    => 'Wipe Log',
    'record_history.purge_confirm'                => 'Wipe this specific CCTV log entry?',
    'record_history.reasoning_evidence'           => 'The Backstory / Proof:',
    'record_history.return_btn'                   => 'Go Back',
    'record_history.snapshot_heading'             => 'Snapshot of Current Live Gen',
    'record_history.subheading_lifecycle'         => 'Shows the background story of tweaks, suggestions, and proof tied to this record.',
    'record_history.system_guest'                 => 'System / Random Guest',
    'record_history.target_column'                => 'Target Column:',
    'record_history.timeline_heading'             => 'Timeline of Moves & Activity',

    // -----------------------------------------------------------------------------
    // 34. Standalone Update Database Gateway and Settings Database Update Dialogue
    // -----------------------------------------------------------------------------
    'index.remove_emergency_file'                 => 'Nuke the emergency backdoor file',
    'update_database.backup_notice'               => 'Back up your database before you run any updates, otherwise you could be totally screwed.',
    'update_database.confirm_prompt'              => 'Did you back up your database? Hit OK to apply pending schema upgrades.',
    'update_database.current_version'             => 'Current schema version:',
    'update_database.err_failed'                  => 'Migration completely tanked:',
    'update_database.heading'                     => '⚠️ System Upgrade Required',
    'update_database.latest_version'              => 'Latest available build:',
    'update_database.msg_success'                 => 'Database upgraded proper! Ran %d migrations.',
    'update_database.msg_uptodate'                => 'Database is already fully up to date.',
    'update_database.page_title'                  => 'System Upgrade Required — Parish Records',
    'update_database.proceed_login'               => 'Head to Log In Page',
    'update_database.remove_emergency_file'       => 'Nuke the emergency backdoor file',
    'update_database.subheading'                  => 'The database schema is outdated and needs an upgrade before the site can function normally.',
    'update_database.update_btn'                  => 'Upgrade Database Now',

    // ------------------------------------------------------------------
    // 35. User Authentication Action
    // ------------------------------------------------------------------
    'authenticate.err_invalid_credentials'        => 'Bad credentials or your account is locked down.',

    // ------------------------------------------------------------------
    // 36. Save Data Entry Action
    // ------------------------------------------------------------------
    'save_data_entry.audit_created_prefix'        => 'Record planted in table with ID %d.',
    'save_data_entry.err_email'                   => 'Gimme a proper valid email for %s blud.',
    'save_data_entry.err_invalid_choice'          => 'Pick a proper option from the list for %s, mate.',
    'save_data_entry.err_location'                => 'Pick a spot from the list and chuck in a title and short blurb for %s.',
    'save_data_entry.err_max'                     => '%s is blowing past the maximum.',
    'save_data_entry.err_min'                     => '%s is way too low, bump it up.',
    'save_data_entry.err_not_number'              => '%s\'s gotta be a proper whole number.',
    'save_data_entry.err_required_field'          => 'Mandatory field \'%s\' cannot be left blank.',
    'save_data_entry.err_url'                     => 'Gimme a website link starting with http:// or https:// for %s fam.',
    'save_data_entry.msg_success'                 => 'Record successfully logged!',

    // ------------------------------------------------------------------
    // 37. Save Public Suggestion Action
    // ------------------------------------------------------------------
    'save_public_suggestion.err_failed_submit'    => 'Failed to log edit suggestion. Try again.',
    'save_public_suggestion.err_field_required'   => 'This field is mandatory and can’t be blank.',
    'save_public_suggestion.err_invalid_column'   => 'Bent column specified.',
    'save_public_suggestion.err_invalid_params'   => 'Bent record submission parameters.',
    'save_public_suggestion.err_spam_detected'    => 'Spam detected. Submission tossed out.',
    'save_public_suggestion.msg_success'          => 'Your edit suggestion has been fired straight to the security queue. Safe geezer!',

    // ------------------------------------------------------------------
    // 38. Data Entry Workstation
    // ------------------------------------------------------------------
    'data_entry.active_table_label'               => 'Active data entry table:',
    'data_entry.add_entry_summary'                => '➕ Log New Data Entry (Click to expand/collapse)',
    'data_entry.admin_cols_prompt'                => 'As the boss, head over to Admin → Manage Tables to add at least one column.',
    'data_entry.admin_tables_prompt'              => 'As the boss, head over to Admin → Manage Tables to build a table and add a column before logging records.',
    'data_entry.apply_filters_btn'                => 'Apply Filters',
    'data_entry.bool_cross'                       => '✘ (Crossed Out)',
    'data_entry.bool_false'                       => 'Cap',
    'data_entry.bool_female'                      => 'Lass / Bird',
    'data_entry.bool_male'                        => 'Lad',
    'data_entry.bool_no_false'                    => 'Nah / False',
    'data_entry.bool_tick'                        => '✔ (Sorted)',
    'data_entry.bool_true'                        => 'Legit',
    'data_entry.bool_yes_true'                    => 'Yeah / True',
    'data_entry.clipboard_alert'                  => 'Table data copied to clipboard! Paste it straight into Excel or Google Sheets.',
    'data_entry.contact_admin_cols'               => 'Contact an administrator to configure columns for this table.',
    'data_entry.contact_admin_tables'             => 'Contact an administrator to sort out tables and columns.',
    'data_entry.copy_entire_btn'                  => 'Copy Entire Table',
    'data_entry.copy_filtered_btn'                => 'Copy Filtered Table',
    'data_entry.csv_entire_btn'                   => 'Download Entire CSV',
    'data_entry.csv_filtered_btn'                 => 'Download Filtered CSV',
    'data_entry.date_format_hint'                 => 'Use the date format shown in the box, mate.',
    'data_entry.date_from_label'                  => 'From',
    'data_entry.date_placeholder_dmy'             => 'DD/MM/YYYY (or rough year)',
    'data_entry.date_placeholder_mdy'             => 'MM/DD/YYYY (or rough year)',
    'data_entry.date_placeholder_ymd'             => 'YYYY-MM-DD (or rough year)',
    'data_entry.date_title_hint'                  => 'Takes full or rough dates (e.g. 1842 or 1842-05)',
    'data_entry.date_to_label'                    => 'To',
    'data_entry.delete_record_btn'                => 'Bin it',
    'data_entry.delete_record_confirm'            => 'Permanently bin this record? Values, map pins and related suggestions will be wiped out completely. Can’t undo this, bruv.',
    'data_entry.dup_blocked'                      => 'This record is way too similar to one already saved, so it can’t be added, bruv.',
    'data_entry.dup_close_match'                  => 'Spelling is dead similar',
    'data_entry.dup_confirm_btn'                  => 'Yeah, Confirm & Save Duplicate',
    'data_entry.dup_desc'                         => 'Check the cards down below, fam. You can still save if this is a completely different record.',
    'data_entry.dup_empty'                        => '(empty)',
    'data_entry.dup_flag_btn'                     => 'Save and tell an admin',
    'data_entry.dup_heading'                      => '⚠️ Potential Duplicate Alert',
    'data_entry.dup_item_format'                  => 'Record ID: %d — Value: %s',
    'data_entry.dup_overview'                     => 'Already saved, blud',
    'data_entry.dup_prompt'                       => 'Want to push ahead and save this duplicate record anyway?',
    'data_entry.dup_review_btn'                   => 'Review what I typed out',
    'data_entry.dup_score_exact'                  => 'Calculated similarity %s percent, g.',
    'data_entry.dup_similar'                      => 'This is %s%% bare similar to record #%s',
    'data_entry.edit_record_btn'                  => 'Tweak',
    'data_entry.edit_record_saved'                => 'Record has been updated - sorted.',
    'data_entry.enter_value_placeholder'          => 'Punch value in...',
    'data_entry.error_loading'                    => 'Absolute shocker loading up the records',
    'data_entry.existing_records_heading'         => 'Existing Records Table',
    'data_entry.filter_all_option'                => '-- All --',
    'data_entry.filter_placeholder'               => 'Filter...',
    'data_entry.go_manage_tables'                 => 'Go to Admin → Manage Tables',
    'data_entry.json_entire_btn'                  => 'Download Entire JSON',
    'data_entry.json_filtered_btn'                => 'Download Filtered JSON',
    'data_entry.location_body'                    => 'Popup blurb',
    'data_entry.location_busy'                    => 'Place search is having a proper moment. Try again in a minute.',
    'data_entry.location_color'                   => 'Pin colour',
    'data_entry.location_help'                    => 'Look up the place as it’s known today, pick a match, then you can style the label as whatever old-school name you want. Title and short blurb are mandatory for the map popup.',
    'data_entry.location_label'                   => 'Name to show (historical names are chill)',
    'data_entry.location_none'                    => 'Nowt matched that. Try a ends nearby.',
    'data_entry.location_results'                 => 'D’you mean',
    'data_entry.location_search'                  => 'Find a place',
    'data_entry.location_search_btn'              => 'Have a look',
    'data_entry.location_show_on_map'             => 'Show this spot on the map',
    'data_entry.location_show_on_map_help'        => 'Untick to keep the record in the table but hide the pin (e.g., if it ain’t open yet).',
    'data_entry.location_title'                   => 'Popup title',
    'data_entry.multiselect_hint'                 => 'Hold down Ctrl or Cmd to grab a bunch at once.',
    'data_entry.na_value'                         => 'N/A',
    'data_entry.no_cols_desc'                     => 'Tables exist, but no data columns have been defined for the active table.',
    'data_entry.no_cols_heading'                  => '⚠️ No Columns Configured',
    'data_entry.no_records'                       => 'No records found.',
    'data_entry.no_tables_desc'                   => 'There are currently no active tables set up for data entry.',
    'data_entry.no_tables_heading'                => '⚠️ No Database Tables Found',
    'data_entry.page_label'                       => 'Page:',
    'data_entry.record_deleted'                   => 'Record has been binned.',
    'data_entry.reset_filter_btn'                 => 'Wipe Filter',
    'data_entry.search_summary'                   => '🔍 Search & Filter Existing Records (Click to expand/collapse)',
    'data_entry.shortcuts_tip'                    => '💡 Tip: Hit Ctrl + Enter to submit or Esc to wipe the current field.',
    'data_entry.submit_data_btn'                  => 'Submit Data',
    'data_entry.th_added_by'                      => 'Added By',
    'data_entry.th_date_created'                  => 'Created When',
    'data_entry.time_format_hint'                 => 'Use the time format shown in the box, mate.',
    'edit_record.heading'                         => 'Tweak record',

    // ------------------------------------------------------------------
    // 39. Forgot Password
    // ------------------------------------------------------------------
    'forgot_password.aria_region'                 => 'Password Recovery',
    'forgot_password.back_login_link'             => 'Back to Log In',
    'forgot_password.email_label'                 => 'Email Addy:',
    'forgot_password.heading'                     => 'Reset Your Password',
    'forgot_password.subheading'                  => 'Drop your account email address below and we’ll fire a secure password reset link straight to your inbox.',
    'forgot_password.submit_btn'                  => 'Send Reset Link',

    // ------------------------------------------------------------------
    // 40. User Login
    // ------------------------------------------------------------------
    'login.aria_region'                           => 'User Log In',
    'login.err_password_required'                 => 'Don’t forget your password, mate.',
    'login.err_username_required'                 => 'Oi, drop your username in.',
    'login.forgot_password_link'                  => 'Forgot Password?',
    'login.heading'                               => 'Log In to Account',
    'login.password_label'                        => 'Password:',
    'login.submit_btn'                            => 'Log In',
    'login.username_label'                        => 'Username or Email:',

    // ------------------------------------------------------------------
    // 41. User Onboarding Setup Wizard
    // ------------------------------------------------------------------
    'onboarding.attr_anon_text'                   => 'Shows initials and a random number to everyone.',
    'onboarding.attr_anon_title'                  => 'Ghost / Anon:',
    'onboarding.attr_opt_anon'                    => 'Anon (Initials & random number)',
    'onboarding.attr_opt_public'                  => 'Public (Show full name)',
    'onboarding.attr_opt_vol'                     => 'Crew only',
    'onboarding.attr_public_text'                 => 'Shows your full name to everyone.',
    'onboarding.attr_public_title'                => 'Public:',
    'onboarding.attr_vol_text'                    => 'Shows initials publicly, but your full name to logged-in crew, security, and bosses.',
    'onboarding.attr_vol_title'                   => 'Crew Only:',
    'onboarding.attribution_desc1'                => 'Controls how your name flashes on the public leaderboard and records.',
    'onboarding.attribution_label'                => 'Leaderboard Display Choice:',
    'onboarding.btn_continue_2fa'                 => 'Roll to 2FA',
    'onboarding.btn_skip_for_now'                 => 'Fuck it, skip',
    'onboarding.date_fmt_dmy'                     => '23/07/2026 (Proper UK - Day/Month/Year)',
    'onboarding.date_fmt_dmy_short'               => '23/07/26 (Shorty - Day/Month/Yr)',
    'onboarding.date_fmt_dots'                    => '23.07.2026 (Dotty - Day.Month.Year)',
    'onboarding.date_fmt_full'                    => 'Thursday 23rd July 2026 (Full Blown)',
    'onboarding.date_fmt_mdy'                     => '07/23/2026 (Yank style - Month/Day/Year)',
    'onboarding.date_format_label'                => 'Date Display Style:',
    'onboarding.err_names_required'               => 'Gotta give us your gov\'nor name and your family, bruv.',
    'onboarding.err_save_failed'                  => 'Peak, that didn\'t save. Try again, blud.',
    'onboarding.heading'                          => 'Welcome to the Crew!',
    'onboarding.lang_site_default'                => 'Standard Gaff Default',
    'onboarding.language_label'                   => 'Lingo',
    'onboarding.msg_welcome'                      => 'Safe fam! Sorted.',
    'onboarding.page_title'                       => 'Welcome — Account Setup Wizard',
    'onboarding.security_hint'                    => 'Swag your settings anytime by tapping your tag in the gaff.',
    'onboarding.subheading'                       => 'Before you start graft, take a second to set up your regional display and privacy choices. You can twist these in your profile anytime.',
    'onboarding.submit_btn'                       => 'Save Choices & Keep Going',
    'onboarding.time_12'                          => '12-hour AM/PM (e.g. 04:07 PM)',
    'onboarding.time_24'                          => '24-hour (e.g. 16:07)',
    'onboarding.time_format_label'                => 'Clock Format:',
    'onboarding.time_none'                        => 'Date only (hide time completely)',
    'onboarding.timezone_label'                   => 'Timezone / Turf:',

    // ------------------------------------------------------------------
    // 42. User Profile & Security Settings
    // ------------------------------------------------------------------
    'profile.aria_new_email'                      => 'New Email Addy',
    'profile.aria_region'                         => 'User Profile Management',
    'profile.backup_codes_heading'                => 'Your New Security Backup Codes',
    'profile.change_email_label'                  => 'Change Email Addy:',
    'profile.confirm_password_label'              => 'Confirm New Password:',
    'profile.current_email_label'                 => 'Current Email:',
    'profile.current_password_label'              => 'Current Password:',
    'profile.download_codes_btn'                  => 'Download New Codes as .txt',
    'profile.email_heading'                       => 'Email Address',
    'profile.email_unverified'                    => '(Unverified - Check your inbox)',
    'profile.email_verified'                      => '(Verified)',
    'profile.err_personal_update'                 => 'Nah fam, couldn\'t update your info.',
    'profile.generate_codes_btn'                  => 'Generate New Backup Codes',
    'profile.generate_codes_confirm'              => 'Are you sure? This nukes any existing backup codes.',
    'profile.heading'                             => 'User Profile & Security',
    'profile.lang_site_default'                   => 'Site Default',
    'profile.language_label'                      => 'Preferred Slang / Language:',
    'profile.msg_personal_updated'                => 'Done g, your stuff is fully locked in and fresh!',
    'profile.new_password_label'                  => 'New Password (min. 8 characters):',
    'profile.password_heading'                    => 'Change Password',
    'profile.personal_details_heading'            => 'Personal Details',
    'profile.setup_tfa_btn'                       => 'Setup 2FA',
    'profile.show_passwords_label'                => 'Show passwords in plain text',
    'profile.tfa_active_desc'                     => '2FA lock is actively protecting your account log-in.',
    'profile.tfa_disabled'                        => 'Unlocked Off',
    'profile.tfa_enabled'                         => 'Locked On',
    'profile.tfa_heading'                         => 'Two-Factor Authentication (2FA Lock)',
    'profile.tfa_status_label'                    => 'Status:',
    'profile.update_details_btn'                  => 'Update Personal Details',
    'profile.update_email_btn'                    => 'Update Email & Verify',
    'profile.update_password_btn'                 => 'Update Password',

    // ------------------------------------------------------------------
    // 43. User Registration
    // ------------------------------------------------------------------
    'register.aria_region'                        => 'User Registration',
    'register.email_label'                        => 'Email Addy',
    'register.heading'                            => 'Register New Account',
    'register.submit_btn'                         => 'Sign Up',
    'register.username_label'                     => 'Username:',

    // ------------------------------------------------------------------
    // 44. Set Password via Secure Token
    // ------------------------------------------------------------------
    'set_password.aria_region'                    => 'Set Password',
    'set_password.confirm_password_label'         => 'Confirm Password:',
    'set_password.exit_expired_token'             => 'This password link has gone out of date or expired.',
    'set_password.exit_invalid_token'             => 'Setup token is bent or missing.',
    'set_password.heading_format'                 => 'Set Password for %s',
    'set_password.msg_ready'                      => 'Password sorted! You’re good to log in now.',
    'set_password.msg_ready_onboarding'           => 'Password locked down. Log in now — we’ll talk you through a quick setup.',
    'set_password.new_password_label'             => 'New Password (min. 8 characters):',
    'set_password.proceed_login_btn'              => 'Head to Log In Page',
    'set_password.save_password_btn'              => 'Save Password',
    'set_password.show_password_label'            => 'Show Password',
    'set_password.subheading_format'              => 'Welcome to your new account, %s! Pick your password below.',

    // ------------------------------------------------------------------
    // 45. Setup 2FA Wizard
    // ------------------------------------------------------------------
    'setup_2fa.aria_code_input'                   => '6-digit verification code',
    'setup_2fa.aria_region'                       => '2FA Setup Wizard',
    'setup_2fa.backup_desc'                       => 'Stash these backup codes somewhere safe. Each code can be used only once if you lose your phone:',
    'setup_2fa.backup_heading'                    => 'Emergency Recovery Codes',
    'setup_2fa.cancel_link'                       => 'Cancel & Return to Profile',
    'setup_2fa.code_label'                        => 'Enter the 6-digit code from the app to verify and lock it down:',
    'setup_2fa.download_btn'                      => 'Download Codes as .txt',
    'setup_2fa.heading'                           => 'Setup 2FA',
    'setup_2fa.manual_prompt'                     => 'Or punch this secret key in manually:',
    'setup_2fa.qr_alt'                            => 'QR code for 2FA setup',
    'setup_2fa.subheading'                        => 'Scan the QR code below with your authenticator app.',
    'setup_2fa.submit_btn'                        => 'Verify & Lock 2FA',

    // ------------------------------------------------------------------
    // 46. Suggest Edit View
    // ------------------------------------------------------------------
    'suggest_edit.aria_region'                    => 'Suggest Edit',
    'suggest_edit.confirm_prompt'                 => 'Sure you want to fire this edit suggestion in for admin review?',
    'suggest_edit.current_values_heading'         => 'Current Values:',
    'suggest_edit.decision_accepted'              => 'Accepted',
    'suggest_edit.decision_not_accepted'          => 'Not accepted, fam',
    'suggest_edit.dup_of_help'                    => 'Optional. Admins will see this note, g.',
    'suggest_edit.dup_of_label'                   => 'If you know the other record number, slap it in right here',
    'suggest_edit.dup_please_check'               => 'This change would be dead similar to another record. Check below, bruv.',
    'suggest_edit.empty_label'                    => '(blank)',
    'suggest_edit.err_notify_email'               => 'Type an email address if you actually wanna hear the outcome, g.',
    'suggest_edit.heading_prefix'                 => 'Suggest Edit for Record',
    'suggest_edit.leave_blank'                    => 'Leave this box completely bare',
    'suggest_edit.no_rationale'                   => 'No extra note was added.',
    'suggest_edit.notify_email_help'              => 'Only used to tell you what happened. Not shown on the public record.',
    'suggest_edit.notify_email_label'             => 'Your email address',
    'suggest_edit.notify_label'                   => 'Email me when an admin makes a choice',
    'suggest_edit.proposed_value_label'           => 'Proposed New Value:',
    'suggest_edit.reasoning_label'                => 'Proof / Backstory / Source Notes:',
    'suggest_edit.reasoning_optional'             => 'Optional — receipts, sources, or a quick note for the mods.',
    'suggest_edit.reasoning_placeholder'          => 'Give context, source citation, or reason for this change...',
    'suggest_edit.report_dup'                     => 'I reckon this record is a duplicate of another one',
    'suggest_edit.return_btn'                     => 'Back to Record',
    'suggest_edit.select_column_label'            => 'Select column to twist:',
    'suggest_edit.submit_btn'                     => 'Submit Suggestion for Review',
    'suggest_edit.submit_heading'                 => 'Submit New Proposed Value & Proof',
    'suggest_edit.success_msg_suffix'             => 'You can fire another edit below or use the return link above when you’re done.',

    // ------------------------------------------------------------------
    // 47. Verify 2FA Login Challenge
    // ------------------------------------------------------------------
    'verify_2fa.aria_code_input'                  => 'Enter verification code or security code',
    'verify_2fa.aria_region'                      => '2FA Verification',
    'verify_2fa.code_label'                       => 'Verification Code / Backup Code:',
    'verify_2fa.heading'                          => 'Two-Factor Authentication Check',
    'verify_2fa.subheading'                       => 'Enter the 6-digit code from your authenticator app or a security backup code.',
    'verify_2fa.submit_btn'                       => 'Verify & Log In',

    // ------------------------------------------------------------------
    // 48. Verify Email
    // ------------------------------------------------------------------
    'verify_email.aria_region'                    => 'Email Verification Status',
    'verify_email.err_expired_token'              => 'This verification link has expired (24-hour limit exceeded). Sign up again or ask for a new link.',
    'verify_email.err_invalid_token'              => 'Bent verification token.',
    'verify_email.err_no_token'                   => 'No verification token provided.',
    'verify_email.err_update_failed'              => 'An error happened while verifying your email. Try again.',
    'verify_email.heading'                        => 'Email Verification Status',
    'verify_email.login_btn'                      => 'Click Here to Log In',
    'verify_email.msg_already_verified'           => 'Your email is already verified bruv. Go log in.',
    'verify_email.msg_success'                    => 'Email verified proper! Your account is active. Head over to log in.',

    // ------------------------------------------------------------------
    // 49. Volunteer Form View
    // ------------------------------------------------------------------
    'index.allocate_unique_username'              => 'Just give us a random unique username',
    'index.check_availability'                    => 'Check if the username is free',
    'volunteer.allocate_unique_username'          => 'Just give us a random unique username',
    'volunteer.aria_region'                       => 'Graft Application Form',
    'volunteer.check_availability'                => 'Check if they’re free',
    'volunteer.honeypot_label'                    => 'Leave this field blank you mug:',
    'volunteer.multi_select_hint'                 => 'Hold Ctrl or Cmd to select multiple.',
    'volunteer.required_field_title'              => 'Must Have Field',
    'volunteer.submit_btn'                        => 'Submit Graft App',
];
