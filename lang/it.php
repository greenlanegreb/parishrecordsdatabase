<?php
declare(strict_types=1);

    // ------------------------------------------------------------------
    // Language File for Italian Language
    // ------------------------------------------------------------------

return [

    // ------------------------------------------------------------------
    // 1. Navigation
    // ------------------------------------------------------------------
    'nav.admin'                                   => 'Amministratore',
    'nav.data_entry'                              => 'Inserimento Dati',
    'nav.feedback'                                => 'Feedback',
    'nav.feedback_dashboard'                      => 'Pannello Feedback',
    'nav.high_contrast'                           => 'Alto Contrasto',
    'nav.invite_user'                             => 'Invita Utente',
    'nav.language_label'                          => 'Lingua',
    'nav.leaderboard'                             => 'Classifica',
    'nav.leaderboard_score'                       => 'Punteggio Classifica',
    'nav.login'                                   => 'Accedi',
    'nav.logout'                                  => 'Esci',
    'nav.low_contrast'                            => 'Basso Contrasto',
    'nav.main_aria'                               => 'Principale',
    'nav.manage_tables'                           => 'Gestisci Tabelle',
    'nav.manage_users'                            => 'Gestisci Utenti',
    'nav.moderation'                              => 'Moderazione',
    'nav.profile'                                 => 'Profilo',
    'nav.search'                                  => 'Cerca',
    'nav.select_language'                         => 'Seleziona lingua',
    'nav.settings'                                => 'Impostazioni di Sistema',
    'nav.similar_records'                         => 'Record simili',
    'nav.toggle_navigation'                       => 'Attiva/disattiva navigazione',
    'nav.volunteer'                               => 'Diventa Volontario',
    'nav.volunteer_dashboard'                     => 'Pannello Volontari',
    'nav.welcome'                                 => 'Benvenuto,',
    'nav.zoom_group'                              => 'Dimensione del testo',
    'nav.zoom_in'                                 => 'Testo più grande',
    'nav.zoom_out'                                => 'Testo più piccolo',
    'nav.zoom_reset'                              => 'Ripristina dimensione del testo',

    // ------------------------------------------------------------------
    // 2. Public search (index)
    // ------------------------------------------------------------------
    'index.date_from_label'                       => 'Da',
    'index.filter_aria'                           => 'Filtro di ricerca per',
    'index.no_public_tables'                      => 'Nessuna tabella disponibile al pubblico su questo sito.',
    'record.back'                                 => 'Torna all\'elenco',
    'record.heading'                              => 'Record',
    'record.print'                                => 'Stampa questo record',
    'record.view'                                 => 'Visualizza record',
    'search.export_csv'                           => 'Esporta Risultati Filtrati come CSV',
    'search.heading'                              => 'Filtri di Ricerca Multicolonna',
    'search.heading_for'                          => 'per',
    'search.load_error'                           => 'Errore durante il caricamento dei risultati. Riprova.',
    'search.no_records'                           => 'Nessun record trovato in questa directory.',
    'search.reset'                                => 'Reimposta Ricerca',

    // ------------------------------------------------------------------
    // 3. Common buttons
    // ------------------------------------------------------------------
    'btn.cancel'                                  => 'Annulla',
    'btn.delete'                                  => 'Elimina',
    'btn.save'                                    => 'Salva',
    'btn.submit'                                  => 'Invia',

    // ------------------------------------------------------------------
    // 4. actions/save_feedback.php & feedback.php Strings
    // ------------------------------------------------------------------
    'feedback.error_all_fields'                   => 'Compila tutti i campi.',
    'feedback.error_invalid_email'                => 'Inserisci un indirizzo email valido.',
    'feedback.error_save_failed'                  => 'Si è verificato un errore durante il salvataggio del feedback. Riprova.',
    'feedback.success_message'                    => 'Grazie! Il tuo feedback è stato inviato con successo.',

    // ------------------------------------------------------------------
    // 5. Index / Public Directory Page
    // ------------------------------------------------------------------
    'index.admin_add_columns_guide'               => 'In qualità di Amministratore, vai su Admin → Gestisci Tabelle per aggiungere almeno una colonna alla tua tabella.',
    'index.admin_create_table_guide'              => 'In qualità di Amministratore, vai su Admin → Gestisci Tabelle per creare una tabella, quindi aggiungi almeno una colonna prima di poter visualizzare o inserire record.',
    'index.clipboard_success'                     => 'I dati della tabella sono stati copiati negli appunti! Puoi incollarli in Excel o Google Fogli.',
    'index.contact_admin_columns'                 => 'Contatta l\'amministratore per configurare le colonne di questa tabella.',
    'index.contact_admin_tables'                  => 'Contatta un amministratore per configurare le tabelle e le colonne del database.',
    'index.copy_entire_table'                     => 'Copia Tabella Completa',
    'index.copy_filtered_table'                   => 'Copia Tabella Filtrata',
    'index.date_to_label'                         => 'a',
    'index.download_entire_csv'                   => 'Scarica CSV Completo',
    'index.download_entire_json'                  => 'Scarica JSON Completo',
    'index.download_filtered_csv'                 => 'Scarica CSV Filtrato',
    'index.download_filtered_json'                => 'Scarica JSON Filtrato',
    'index.go_to_manage_tables'                   => 'Vai a Gestisci Tabelle',
    'index.guest_login_tables_guide'              => 'Accedi come Amministratore e vai su Admin → Gestisci Tabelle per creare una tabella, quindi aggiungi almeno una colonna.',
    'index.modal_desc'                            => 'Fornisci una correzione o informazioni alternative per questo record. Il nostro team di moderazione lo esaminerà.',
    'index.modal_heading'                         => 'Suggerisci Correzione del Record',
    'index.modal_input_placeholder'               => 'Inserisci le informazioni aggiornate...',
    'index.modal_proposed_value'                  => 'Valore Proposto / Correzione:',
    'index.modal_submit_btn'                      => 'Invia Suggerimento',
    'index.modal_target_column'                   => 'Colonna di Destinazione:',
    'index.no_columns_desc'                       => 'Ci sono tabelle nel sistema, ma non è stata definita alcuna colonna di dati per la tabella attiva.',
    'index.no_columns_heading'                    => 'Nessuna Colonna Configurata',
    'index.no_tables_desc'                        => 'Al momento non ci sono tabelle del database attive configurate nel sistema.',
    'index.no_tables_heading'                     => 'Nessuna Tabella del Database Trovata',
    'index.opt_cross'                             => '✘ (Croce)',
    'index.opt_false'                             => 'Falso',
    'index.opt_female'                            => 'Femmina',
    'index.opt_male'                              => 'Maschio',
    'index.opt_no_false'                          => 'No / Falso',
    'index.opt_tick'                              => '✔ (Spuntato)',
    'index.opt_true'                              => 'Vero',
    'index.opt_yes_true'                          => 'Sì / Vero',
    'index.option_all'                            => '-- Tutti --',
    'index.search_placeholder'                    => 'Cerca...',
    'index.select_directory_database'             => 'Seleziona il database della directory:',
    'index.th_actions'                            => 'Azioni',
    'index.th_created_by'                         => 'Creato da',
    'index.th_date_added'                         => 'Data di Aggiunta',
    'index.th_record_id'                          => 'ID Record',

    // ------------------------------------------------------------------
    // 6. Print Records Functionality
    // ------------------------------------------------------------------
    'cols.back'                                   => 'Indietro',
    'cols.print_btn'                              => 'Stampa',
    'cols.print_entire'                           => 'Stampa tutto',
    'cols.print_filtered'                         => 'Stampa elementi filtrati',
    'cols.print_now'                              => 'Stampa o salva come PDF',
    'cols.show_heading'                           => 'Scegli quali colonne mostrare',
    'cols.show_help'                              => 'Disvuota la spunta da una colonna per nasconderla qui, nei download e durante la stampa. Su uno smartphone, un minor numero di colonne è più facile da leggere.',

    // ------------------------------------------------------------------
    // 7. Admin: Create User / Invite Form
    // ------------------------------------------------------------------
    'create_user.email_label'                     => 'Indirizzo Email:',
    'create_user.first_name'                      => 'Nome:',
    'create_user.heading'                         => 'Modulo di Invito Nuovo Utente',
    'create_user.role_label'                      => 'Ruolo Utente:',
    'create_user.subheading'                      => 'Verrà generato un link di configurazione sicuro valido per 24 ore e inviato direttamente via email all\'utente.',
    'create_user.submit_btn'                      => 'Crea Utente e Invia Invito',
    'create_user.surname'                         => 'Cognome:',
    'create_user.username_help'                   => 'Se lasciato vuoto, verrà generato automaticamente un nome utente univoco basato sul nome.',
    'create_user.username_label'                  => 'Nome Utente (Opzionale):',
    'create_user.username_placeholder'            => 'Lascia vuoto per la generazione automatica',

    // ------------------------------------------------------------------
    // 8. Admin: Feedback / Support Tickets Dashboard
    // ------------------------------------------------------------------
    'feedback_dash.anonymous'                     => 'Anonimo',
    'feedback_dash.default_subject'               => 'Richiesta Generale',
    'feedback_dash.delete_confirm'                => 'Eliminare questo ticket di supporto e tutte le risposte associate?',
    'feedback_dash.heading'                       => 'Pannello Ticket di Supporto e Feedback',
    'feedback_dash.manage_emails'                 => 'Gestisci Modelli Email',
    'feedback_dash.manage_schema'                 => 'Gestisci Schema Modulo Ticket',
    'feedback_dash.msg_deleted'                   => 'Ticket #:id eliminato con successo.',
    'feedback_dash.no_tickets'                    => 'Nessun ticket di feedback trovato.',
    'feedback_dash.open_ticket_btn'               => 'Apri Ticket e Conversazione',
    'feedback_dash.subheading'                    => 'Gestisci le richieste di supporto pubblico, aggiorna gli stati e partecipa alla conversazione.',
    'feedback_dash.th_status'                     => 'Stato',
    'feedback_dash.th_subject_info'               => 'Oggetto / Informazioni di Base',
    'feedback_dash.th_submitter'                  => 'Mittente',
    'feedback_dash.th_ticket_date'                => 'Data',

    // ------------------------------------------------------------------
    // 9. Admin: Manage Feedback Email Templates
    // ------------------------------------------------------------------
    'feedback_emails.back_to_dashboard'           => 'Torna al Pannello Ticket',
    'feedback_emails.custom_tags'                 => 'Tag di Schema Personalizzati:',
    'feedback_emails.custom_tags_desc'            => 'Generati automaticamente dai campi del generatore di moduli dei ticket:',
    'feedback_emails.email_body'                  => 'Modello Corpo Email:',
    'feedback_emails.email_subject'               => 'Oggetto Email:',
    'feedback_emails.fixed_tags'                  => 'Tag Fissi Principali:',
    'feedback_emails.heading'                     => 'Modelli Email per Ticket di Supporto',
    'feedback_emails.placeholders_desc'           => 'Puoi utilizzare questi tag ovunque nell\'oggetto o nel corpo:',
    'feedback_emails.placeholders_heading'        => 'Placeholder Disponibili',
    'feedback_emails.save_template_btn'           => 'Salva Modello',
    'feedback_emails.subheading'                  => 'Personalizza le notifiche email automatiche inviate durante il flusso dei ticket. Usa le parentesi graffe per i valori dinamici.',

    // ------------------------------------------------------------------
    // 10. Admin: Manage Feedback Ticket Schema & Fields
    // ------------------------------------------------------------------
    'feedback_schema.add_field_title'             => '+ Aggiungi Nuovo Campo al Modulo Ticket',
    'feedback_schema.allow_multiple'              => 'Consenti selezione multipla (Selezione Multipla)',
    'feedback_schema.boolean_format'              => 'Formato di Visualizzazione Booleano:',
    'feedback_schema.create_field_btn'            => 'Crea Campo Ticket',
    'feedback_schema.data_type_label'             => 'Tipo di Dati:',
    'feedback_schema.delete_confirm'              => 'Eliminare questo campo e tutti i valori di risposta associati?',
    'feedback_schema.edit_btn'                    => 'Modifica',
    'feedback_schema.edit_field_title'            => 'Modifica Campo Ticket:',
    'feedback_schema.existing_fields_heading'     => 'Campi Ticket Esistenti',
    'feedback_schema.field_name_label'            => 'Etichetta / Nome Campo:',
    'feedback_schema.form_intro_label'            => 'Testo di Introduzione / Descrizione:',
    'feedback_schema.form_title_label'            => 'Titolo del Modulo:',
    'feedback_schema.heading'                     => 'Gestisci Schema Modulo Feedback',
    'feedback_schema.is_required_label'           => 'Rendi questo campo obbligatorio per i mittenti',
    'feedback_schema.max_length_label'            => 'Lunghezza Massima / Limite Caratteri Opzionale:',
    'feedback_schema.no_fields'                   => 'Nessun campo ticket personalizzato definito.',
    'feedback_schema.options_help'                => 'Fornisci opzioni separate da virgole o interruzioni di riga.',
    'feedback_schema.options_label'               => 'Opzioni (separate da virgola o una per riga):',
    'feedback_schema.save_field_btn'              => 'Salva Modifiche Campo',
    'feedback_schema.save_settings_btn'           => 'Salva Impostazioni Modulo',
    'feedback_schema.settings_summary'            => 'Configura Titolo del Modulo e Testo di Disclaim',
    'feedback_schema.sub_checkbox'                => 'Casella di Controllo (Checkbox)',
    'feedback_schema.sub_email'                   => 'Email',
    'feedback_schema.sub_number'                  => 'Input Numerico',
    'feedback_schema.sub_radio'                   => 'Gruppo Pulsanti Radio',
    'feedback_schema.sub_select'                  => 'Menu a Tendina (Select)',
    'feedback_schema.sub_textarea'                => 'Casella di Testo Multilinea',
    'feedback_schema.sub_url'                     => 'URL',
    'feedback_schema.subheading'                  => 'Configura campi personalizzati, tipi di dati, limiti di caratteri, sottotipi, opzioni e stile di visualizzazione.',
    'feedback_schema.subtype_label'               => 'Sottotipo Campo / Stile di Rendering Input:',
    'feedback_schema.subtype_standard'            => '-- Standard --',
    'feedback_schema.subtype_standard_lower'      => 'standard',
    'feedback_schema.system_user'                 => 'Sistema',
    'feedback_schema.th_created_by'               => 'Creato da',
    'feedback_schema.th_data_type'                => 'Tipo di Dati',
    'feedback_schema.th_email'                    => 'Email',
    'feedback_schema.th_field_name'               => 'Nome Campo',
    'feedback_schema.th_id'                       => 'ID',
    'feedback_schema.th_max_length'               => 'Lunghezza Massima',
    'feedback_schema.th_move'                     => 'Sposta',
    'feedback_schema.th_required'                 => 'Obbligatorio?',
    'feedback_schema.th_subtype'                  => 'Sottotipo',
    'feedback_schema.type_boolean'                => 'BOOLEAN (Indicatore Sì/No)',
    'feedback_schema.type_date'                   => 'DATE (Data Calendario)',
    'feedback_schema.type_int'                    => 'INT (Numero Intero)',
    'feedback_schema.type_text'                   => 'TEXT (Paragrafo Lungo / Messaggio)',
    'feedback_schema.type_varchar'                => 'VARCHAR (Testo Breve)',

    // ------------------------------------------------------------------
    // 11. Admin: Manage Tables & Column Schemas
    // ------------------------------------------------------------------
    'manage_tables.add_col_summary_prefix'        => '+ Aggiungi nuova colonna tabella per',
    'manage_tables.allow_multiple_label'          => 'Consenti più di una scelta (selezione multipla)',
    'manage_tables.bool_male_female'              => 'Maschio / Femmina',
    'manage_tables.bool_tick_cross'               => 'Spunta / Croce',
    'manage_tables.bool_true_false'               => 'Vero / Falso',
    'manage_tables.bool_yes_no'                   => 'Sì / No',
    'manage_tables.choice_options_help'           => 'Esempio: Battesimo, Matrimonio, Sepoltura — ciascuno sulla propria riga.',
    'manage_tables.choice_options_label'          => 'Scelte (una per riga)',
    'manage_tables.col_name_label'                => 'Nome Colonna:',
    'manage_tables.create_col_btn'                => 'Crea Colonna',
    'manage_tables.create_table_btn'              => 'Crea Schema Tabella',
    'manage_tables.create_table_summary'          => '+ Crea Nuova Tabella Dinamica',
    'manage_tables.date_behavior_label'           => 'Comportamento Ricerca Data:',
    'manage_tables.date_bhv_admin'                => 'Solo date amministratore',
    'manage_tables.date_bhv_all'                  => 'Tutte le date, comprese quelle dell\'amministratore',
    'manage_tables.date_bhv_manual'               => 'Data del database (solo inserimento manuale)',
    'manage_tables.delete_col_confirm'            => 'ATTENZIONE: L\'eliminazione di questa colonna rimuoverà anche tutti i dati di cella associati in ogni record. Sei sicuro?',
    'manage_tables.delete_table_btn'              => 'Elimina tabella',
    'manage_tables.delete_table_confirm'          => 'ATTENZIONE: L\'eliminazione di questa tabella rimuoverà permanentemente tutte le colonne e il contenuto salvato. Sei assolutamente sicuro?',
    'manage_tables.edit_col_summary'              => 'Modifica Colonna Dinamica:',
    'manage_tables.edit_metadata_btn'             => 'Modifica dettagli tabella',
    'manage_tables.edit_table_summary'            => 'Modifica Definizione Tabella:',
    'manage_tables.err_col_name_empty'            => 'Inserisci il nome di una colonna.',
    'manage_tables.err_col_name_taken'            => 'Questa tabella ha già una colonna chiamata %s. Scegli un nome diverso.',
    'manage_tables.err_table_name_empty'          => 'Inserisci il nome di una tabella.',
    'manage_tables.err_table_name_taken'          => 'Esiste già una tabella chiamata %s. Scegli un nome diverso.',
    'manage_tables.exclude_search_label'          => 'Escludi questa colonna dalla ricerca pubblica',
    'manage_tables.existing_cols_heading_prefix'  => 'Colonne esistenti per',
    'manage_tables.heading'                       => 'Gestione dinamica tabelle e colonne',
    'manage_tables.intro'                         => 'Qui è possibile modificare i nomi, i tipi di campo e ciò che appare negli elenchi di ricerca o nella pagina del singolo record.',
    'manage_tables.max_value_label'               => 'Massimo',
    'manage_tables.min_value_label'               => 'Minimo',
    'manage_tables.no'                            => 'No',
    'manage_tables.no_columns_found'              => 'Nessuna colonna dinamica definita per questa tabella.',
    'manage_tables.req_toggle_label'              => 'Rendi questa colonna obbligatoria (inserimento dati obbligatorio)',
    'manage_tables.save_and_add_col'              => 'Salva e aggiungi un altro',
    'manage_tables.save_table_btn'                => 'Salva Modifiche Tabella',
    'manage_tables.show_in_list'                  => 'Mostra nell\'elenco di ricerca / inserimento dati',
    'manage_tables.show_in_record'                => 'Mostra nella pagina del record completo',
    'manage_tables.status_hidden'                 => 'Nascosto',
    'manage_tables.subheading'                    => 'È possibile modificare nomi, tipi di campo e ciò che appare nell\'elenco di ricerca o nella pagina del singolo record.',
    'manage_tables.switcher_label'                => 'Seleziona schema di tabella attivo:',
    'manage_tables.table_desc_label'              => 'Descrizione / Scopo:',
    'manage_tables.table_name_label'              => 'Nome Amichevole della Tabella:',
    'manage_tables.th_date_created'               => 'Data di Creazione',
    'manage_tables.th_display_format'             => 'Formato di Visualizzazione',
    'manage_tables.th_public_search'              => 'Ricerca Pubblica?',
    'manage_tables.type_boolean'                  => 'Booleano (Maschio / Femmina, Spunta / Croce, Vero / Falso, Sì / No)',
    'manage_tables.type_choice'                   => 'Elenco di scelte',
    'manage_tables.type_date'                     => 'Data',
    'manage_tables.type_email'                    => 'Indirizzo email',
    'manage_tables.type_int'                      => 'Intero (Numero intero)',
    'manage_tables.type_location'                 => 'Posizione (Pin sulla mappa)',
    'manage_tables.type_location_help'            => 'I luoghi devono essere scelti dalla ricerca affinché possano comparire sulla mappa di questa tabella. Si prega di utilizzare una normale colonna di testo se serve solo un nome di luogo scritto.',
    'manage_tables.type_text_long'                => 'Testo lungo',
    'manage_tables.type_time'                     => 'Ora',
    'manage_tables.type_url'                      => 'Indirizzo web',
    'manage_tables.type_varchar'                  => 'Testo breve',
    'manage_tables.url_new_tab'                   => 'Apri in una nuova scheda',
    'manage_tables.yes'                           => 'Sì',

    // ------------------------------------------------------------------
    // 12. Map Functionality Translations
    // ------------------------------------------------------------------
    'map.apply_filters'                           => 'Applica filtri',
    'map.back_to_table'                           => 'Torna alla tabella',
    'map.canvas_aria'                             => 'Mappa dei record',
    'map.filters'                                 => 'Filtri',
    'map.heading'                                 => 'Mappa',
    'map.help'                                    => 'Ingrandisci per caricare i luoghi visibili. Usa i filtri per abbinarli alla ricerca della tabella.',
    'map.list_empty'                              => 'Nessun luogo in questa vista.',
    'map.list_heading'                            => 'Luoghi in questa vista',
    'map.list_show_more'                          => 'Mostra di più nell’elenco (:n rimanenti)',
    'map.open_btn'                                => 'Mappa',
    'map.skip_to_map'                             => 'Salta alla mappa',

    // ------------------------------------------------------------------
    // 13. Admin: Manage User Notification Email Templates
    // ------------------------------------------------------------------
    'user_emails.back_to_creation'                => 'Torna alla Creazione Utente',
    'user_emails.currently_editing'               => 'Modifica in corso:',
    'user_emails.desc_invitation'                 => 'Inviato automaticamente quando un amministratore crea o invita un nuovo utente.',
    'user_emails.desc_outcome'                    => 'Questo viene inviato quando un moderatore accetta o rifiuta una modifica suggerita e il mittente ha richiesto di ricevere un\'e-mail.',
    'user_emails.desc_reset'                      => 'Inviato quando viene richiesto il reset della password o l\'invio di un link di accesso.',
    'user_emails.email_body_label'                => 'Corpo Email:',
    'user_emails.heading'                         => 'Gestisci Modelli Email di Notifica Utente',
    'user_emails.opt_invitation'                  => 'Modello Invito Account Utente',
    'user_emails.opt_reset'                       => 'Modello Reset Password / Link di Accesso',
    'user_emails.opt_suggestion_outcome'          => 'Esito della modifica suggerita',
    'user_emails.placeholders_hint'               => 'Questi token vengono compilati all\'invio di questo modello. Solo le schede sottostanti si applicano al modello selezionato.',
    'user_emails.select_template_label'           => 'Seleziona modello da modificare:',
    'user_emails.subheading'                      => 'Personalizza i layout email invitati quando si invitano utenti o si inviano link di reset password.',

    // ------------------------------------------------------------------
    // 14. Admin: Manage Volunteer Email Templates
    // ------------------------------------------------------------------
    'volunteer_emails.back_to_dashboard'          => 'Torna alle Richieste Volontari',
    'volunteer_emails.custom_tags_desc'           => 'Generati automaticamente dai campi del generatore di moduli:',
    'volunteer_emails.heading'                    => 'Modelli Email e Attivatori per Volontari',
    'volunteer_emails.subheading'                 => 'Personalizza le risposte email automatiche per i volontari nelle diverse fasi del flusso di lavoro. Usa le parentesi graffe per i valori dinamici.',

    // ------------------------------------------------------------------
    // 15. Admin: Manage Volunteer Form Schema & Fields
    // ------------------------------------------------------------------
    'volunteer_schema.add_field_title'            => '+ Aggiungi Nuovo Campo al Modulo Volontari',
    'volunteer_schema.back_to_dashboard'          => 'Torna alle Richieste Volontari',
    'volunteer_schema.create_field_btn'           => 'Crea Campo',
    'volunteer_schema.delete_confirm'             => 'Eliminare questo campo e tutti i valori di risposta associati?',
    'volunteer_schema.edit_field_title'           => 'Modifica Campo:',
    'volunteer_schema.existing_fields_heading'    => 'Campi Modulo Volontari Esistenti',
    'volunteer_schema.heading'                    => 'Gestisci Schema Modulo Volontari',
    'volunteer_schema.no_fields'                  => 'Nessun campo volontario personalizzato definito.',
    'volunteer_schema.settings_summary'           => 'Configura Titolo del Modulo e Testo di Disclaim',
    'volunteer_schema.subheading'                 => 'Configura campi personalizzati, tipi di dati, sottotipi, opzioni e preferenze generali di visualizzazione del modulo.',

    // ------------------------------------------------------------------
    // 16. Admin: Moderation Queue & Suggestions Review
    // ------------------------------------------------------------------
    'dup_merge.col_a'                             => 'Record #%s',
    'dup_merge.col_b'                             => 'Record #%s',
    'dup_merge.col_field'                         => 'Campo',
    'dup_merge.confirm'                           => 'Vuoi unire questi elementi in un unico record? L\'altro record verrà rimosso.',
    'dup_merge.fields_caption'                    => 'Scegli un valore per ciascun campo',
    'dup_merge.heading'                           => 'Unisci due record simili',
    'dup_merge.intro'                             => 'Questi due elementi sono simili per circa il %s%%. Scegli quale record mantenere, quindi seleziona un valore per ciascun campo.',
    'dup_merge.keep_a'                            => 'Mantieni il record #%s',
    'dup_merge.keep_b'                            => 'Mantieni il record #%s',
    'dup_merge.keep_legend'                       => 'Quale record deve essere conservato?',
    'dup_merge.save_btn'                          => 'Unisci in un unico record',
    'dup_merged'                                  => 'Quei due record ora sono uno solo. La modifica si trova nella cronologia del record rimanente.',
    'dup_queue.choose_table'                      => 'Scegli una tabella',
    'dup_queue.col_actions'                       => 'Cosa fare',
    'dup_queue.col_records'                       => 'Record',
    'dup_queue.col_score'                         => 'Grado di somiglianza',
    'dup_queue.col_table'                         => 'Tabella',
    'dup_queue.dismiss_btn'                       => 'Non è un duplicato',
    'dup_queue.dismissed'                         => 'Quella coppia è stata contrassegnata come non duplicata.',
    'dup_queue.empty'                             => 'Nessun elemento in attesa. Scansiona una tabella per cercare eventuali duplicati precedenti.',
    'dup_queue.gone'                              => 'Quella revisione non è più in attesa.',
    'dup_queue.heading'                           => 'Record simili da revisionare',
    'dup_queue.intro'                             => 'Scansiona una tabella alla ricerca di righe che sembrano già simili. Puoi indicare che sono diverse o unirle in un unico record.',
    'dup_queue.merge_btn'                         => 'Unisci in uno',
    'dup_queue.scan_btn'                          => 'Scansiona questa tabella',
    'dup_queue.scan_done'                         => 'Scansione completata. Trovate %s coppie simili, %s nuovi elementi aggiunti alla lista.',
    'dup_queue.scan_label'                        => 'Cerca record simili in',
    'moderate.approve_btn'                        => 'Approva',
    'moderate.approve_confirm'                    => 'Approvare e applicare questo valore?',
    'moderate.by_label'                           => 'Da:',
    'moderate.column_label'                       => 'Colonna:',
    'moderate.decline_btn'                        => 'Rifiuta',
    'moderate.decline_confirm'                    => 'Rifiutare e scartare questo suggerimento?',
    'moderate.dup_approved_note'                  => 'Approvato. Ora appare simile al %s%% rispetto al record #%s.',
    'moderate.dup_blocked'                        => 'Impossibile approvare: questo risulterebbe simile al %s%% rispetto al record #%s.',
    'moderate.empty_placeholder'                  => '[Vuoto]',
    'moderate.evidence_label'                     => 'Prova / Motivazione:',
    'moderate.guest_user'                         => 'Ospite / Visitatore',
    'moderate.heading'                            => 'Revisione Coda di Moderazione e Suggerimenti',
    'moderate.historical_dates_title'             => 'Date storiche supportate',
    'moderate.live_value_label'                   => 'Valore attivo attuale:',
    'moderate.no_evidence'                        => 'Nessuna prova o motivazione fornita.',
    'moderate.no_suggestions'                     => 'Nessun suggerimento in sospeso trovato per le tue tabelle di moderazione autorizzate.',
    'moderate.override_label'                     => 'Sovrascrivi Valore:',
    'moderate.proposed_value_label'               => 'Modifica suggerita:',
    'moderate.rationale_label'                    => 'Nota per la persona che ha suggerito questo',
    'moderate.rationale_optional'                 => 'Nota opzionale (conservata sul record).',
    'moderate.rationale_placeholder'              => 'Perché hai accettato o rifiutato questa modifica',
    'moderate.rationale_will_be_sent'             => 'È stato richiesto l\'invio di un\'e-mail. La nota qui sotto verrà inviata esattamente come la scrivi, quindi mantieni un tono gentile e chiaro.',
    'moderate.record_id_label'                    => 'ID Record:',
    'moderate.required_badge'                     => 'Obbligatorio',
    'moderate.select_placeholder'                 => '-- Seleziona --',
    'moderate.shortcut_desc'                      => 'Premi Ctrl + Invio per approvare rapidamente o Esc per pulire la casella di sovrascrittura!',
    'moderate.shortcut_label'                     => 'Suggerimento Scorciatoie da Tastiera:',
    'moderate.subheading'                         => 'Confronta le modifiche suggerite dagli utenti con i record attivi nelle tue tabelle autorizzate. Approva, sovrascrivi o rifiuta i suggerimenti.',
    'moderate.tab_similar'                        => 'Record simili',
    'moderate.tab_suggestions'                    => 'Suggerimenti',
    'moderate.th_actions'                         => 'Azioni Moderatore',
    'moderate.th_comparison'                      => 'Confronto (Attivo vs Suggerito) e Prove',
    'moderate.th_id_date'                         => 'ID / Data',
    'moderate.th_table_record'                    => 'Tabella, Record e Colonna',

    // ------------------------------------------------------------------
    // 17. Admin: Notices & Announcements Manager
    // ------------------------------------------------------------------
    'notices.active'                              => 'Attivo',
    'notices.add_new'                             => 'Aggiungi nuovo avviso',
    'notices.audience'                            => 'Pubblico',
    'notices.content_label'                       => 'Contenuto',
    'notices.create_heading'                      => 'Crea Nuovo Avviso',
    'notices.create_notice_btn'                   => 'Crea avviso',
    'notices.delete_btn'                          => 'Elimina',
    'notices.delete_confirm'                      => 'Eliminare questo avviso?',
    'notices.dismissible'                         => 'Ignorabile',
    'notices.dismissible_label'                   => "Ignorabile (include il pulsante di chiusura 'X')",
    'notices.display_order'                       => 'Ordine di visualizzazione',
    'notices.display_order_label'                 => 'Ordine di Visualizzazione:',
    'notices.error_blank'                         => 'Il titolo e il contenuto non possono essere vuoti.',
    'notices.everyone'                            => 'Tutti',
    'notices.existing_heading'                    => 'Avvisi Attivi ed Esistenti',
    'notices.heading'                             => 'Gestore Avvisi e Annunci del Sito',
    'notices.msg_created'                         => 'Avviso creato con successo!',
    'notices.msg_deleted'                         => 'Avviso eliminato.',
    'notices.no_notices'                          => 'Nessun avviso creato.',
    'notices.no_sticky'                           => 'No (Fisso / Sticky)',
    'notices.publish_btn'                         => 'Pubblica Avviso',
    'notices.role_admins'                         => 'Amministratori',
    'notices.role_everyone'                       => 'Tutti',
    'notices.role_moderators'                     => 'Moderatori',
    'notices.role_public'                         => 'Pubblico (Ospite)',
    'notices.role_users'                          => 'Utenti',
    'notices.save_btn'                            => 'Salva',
    'notices.subheading'                          => 'Crea avvisi dinamici, banner di benvenuto o annunci mirati per ruoli utente specifici.',
    'notices.target_roles_label'                  => 'Pubblico di destinazione (seleziona ruoli o tutti):',
    'notices.th_dismissible'                      => 'Ignorabile',
    'notices.th_order'                            => 'Ordine',
    'notices.th_target_roles'                     => 'Ruoli di Destinazione',
    'notices.th_title'                            => 'Titolo',
    'notices.title_label'                         => 'Titolo',
    'notices.yes'                                 => 'Sì',

    // ------------------------------------------------------------------
    // 18. Admin: Global Site Settings, Modules & Permissions
    // ------------------------------------------------------------------
    'appearance.add_link'                         => 'Aggiungi un link',
    'appearance.brand_heading'                    => 'Logo e slogan',
    'appearance.colors_heading'                   => 'Colori',
    'appearance.colors_help'                      => 'Il contrasto elevato sovrascrive sempre queste impostazioni quando viene utilizzato tale interruttore.',
    'appearance.custom_heading'                   => 'Link personalizzati',
    'appearance.custom_help'                      => 'Sentiti libero di aggiungerne quanti ne ️necessiti. Imposta l\'elemento genitore per annidare un link sotto un\'altra intestazione o sotto un menu integrato come Amministrazione. Usa # come indirizzo per un\'intestazione che sia solo un titolo.',
    'appearance.custom_label'                     => 'Etichetta',
    'appearance.custom_parent'                    => 'Genitore',
    'appearance.custom_url'                       => 'Indirizzo',
    'appearance.custom_who'                       => 'Chi può vederlo',
    'appearance.drag'                             => 'Trascina per riordinare, per favore',
    'appearance.nav_heading'                      => 'Navigazione',
    'appearance.nav_help'                         => 'Puoi trascinare la maniglia per riordinare insieme i link integrati e personalizzati. Per posizionare un link sotto Amministrazione (o un\'altra intestazione), imposta il genitore su quel link. L\'indirizzo # crea un\'intestazione senza una pagina propria. L\'accesso e il profilo rimangono nella loro posizione integrata.',
    'appearance.new_tab'                          => 'Nuova scheda',
    'appearance.parent_builtin'                   => 'Menu integrati',
    'appearance.parent_custom'                    => 'Le tue intestazioni',
    'appearance.parent_none'                      => 'Livello superiore — Voce di menu personale',
    'appearance.remove_link'                      => 'Rimuovi',
    'appearance.reset'                            => 'Ripristina predefinito',
    'appearance.reset_ok'                         => 'Aspetto ripristinato ai valori predefiniti.',
    'appearance.save'                             => 'Salva aspetto',
    'appearance.saved'                            => 'Aspetto salvato.',
    'appearance.type_heading'                     => 'Tipo',
    'role.label_admin'                            => 'Amministratore',
    'role.label_guest'                            => 'Visitatore pubblico',
    'role.label_moderator'                        => 'Moderatore',
    'role.label_user'                             => 'Utente immissione dati',
    'settings.add_notice_btn'                     => '+ Aggiungi Nuovo Avviso',
    'settings.audit_heading'                      => 'Browser del Registro di Audit del Sistema',
    'settings.audit_limit_note'                   => 'Visualizzazione degli ultimi 250 registri di audit.',
    'settings.audit_subheading'                   => 'Ispeziona azioni di sicurezza registrate, immissione dati e moderazione. Usa le opzioni di manutenzione sottostanti per pulire i log, se necessario.',
    'settings.captcha_hcaptcha'                   => 'hCaptcha',
    'settings.captcha_heading'                    => 'Configurazione Sicurezza e CAPTCHA',
    'settings.captcha_none'                       => 'Disattivato (Senza CAPTCHA)',
    'settings.captcha_provider_label'             => 'Meccanismo Provider CAPTCHA:',
    'settings.captcha_recaptcha'                  => 'Google reCAPTCHA v2 / v3',
    'settings.captcha_turnstile'                  => 'Cloudflare Turnstile',
    'settings.clear_all_audit_btn'                => 'Cancella Tutti i Registri di Audit',
    'settings.clear_records_audit_btn'            => 'Cancella Solo Audit dei Record',
    'settings.core_sys_heading'                   => 'Impostazioni Core del Sistema',
    'settings.create_role_btn'                    => 'Crea ruolo',
    'settings.create_role_heading'                => 'Crea nuovo ruolo',
    'settings.cron_command_label'                 => 'Ecco il comando da incollare nelle tue operazioni Cron (Una volta al giorno andrà benissimo!)',
    'settings.cron_exec_ok'                       => 'A questa pagina è stato permesso di testare PHP da qui (facoltativo)',
    'settings.cron_maintenance_desc'              => 'pRD ha analizzato automaticamente il tuo server web e suggerisce un comando notturno. Devi comunque incollarlo nelle tue operazioni Cron. Puoi anche eseguire una pulizia adesso.',
    'settings.cron_maintenance_heading'           => 'Manutenzione automatizzata dei token e configurazione Cron',
    'settings.cron_php_ok'                        => 'Comando PHP trovato',
    'settings.cron_script_ok'                     => 'Congratulazioni - Il tuo script di pulizia è pronto!',
    'settings.db_updates_heading'                 => 'Aggiornamenti del Database',
    'settings.default_date_format'                => 'Formato data predefinito',
    'settings.default_lang_desc'                  => 'Usata per i visitatori e gli utenti senza lingua preferita. Aggiungi file in lang/ (es. it.php) per altre opzioni.',
    'settings.default_lang_label'                 => 'Lingua Predefinita del Sito:',
    'settings.default_lang_note'                  => 'La lingua predefinita è già impostata sopra. Le impostazioni del profilo sovrascrivono queste per gli utenti connessi.',
    'settings.default_time_format'                => 'Formato ora predefinito',
    'settings.default_timezone'                   => 'Fuso orario predefinito',
    'settings.delete_btn'                         => 'Elimina',
    'settings.delete_role_confirm'                => 'Sei sicuro di voler eliminare questo ruolo? Gli utenti ad esso assegnati verranno riassegnati in modo sicuro al ruolo utente predefinito.',
    'settings.download_backup_btn'                => 'Scarica Backup del Database',
    'settings.download_backup_desc'               => 'Salva un file .sql completo sul tuo computer. Conservalo in un luogo sicuro prima di eseguire gli aggiornamenti.',
    'settings.driver_native'                      => 'Email Nativa (Postfix Local Relay)',
    'settings.driver_smtp'                        => 'SMTP Autenticato (PHPMailer)',
    'settings.dup_heading'                        => 'Gestione record duplicati',
    'settings.dup_help'                           => 'pRD confronta il testo inserito con altre righe della stessa tabella. Puoi avvisare gli utenti, bloccare il salvataggio o disattivare questa funzione.',
    'settings.dup_legend'                         => 'Se un nuovo record assomiglia a uno già esistente',
    'settings.dup_mode_block'                     => 'Non consentire il salvataggio',
    'settings.dup_mode_flag'                      => 'Avvisa gli utenti e segnalalo a un moderatore se salvano comunque',
    'settings.dup_mode_off'                       => 'Non controllare record simili',
    'settings.dup_mode_warn'                      => 'Mostra un avviso — è comunque possibile salvare',
    'settings.dup_picky_exact'                    => 'Solo quando le parole coincidono strettamente',
    'settings.dup_picky_label'                    => 'Quanto deve essere rigoroso il controllo?',
    'settings.dup_picky_similar'                  => 'Rileva anche quasi-corrispondenze (ortografia simile)',
    'settings.enc_ssl'                            => 'SSL (Porta 465)',
    'settings.enc_tls'                            => 'TLS (Porta 587)',
    'settings.error_log_tab'                      => 'Registro errori',
    'settings.existing_roles_heading'             => 'Ruoli esistenti e gestione',
    'settings.footer_compiled_notice'             => 'Riga piè di pagina',
    'settings.footer_notice_helper'               => 'Mostrato come prima riga del piè di pagina del sito. Lascia vuoto per usare il testo predefinito dal file di lingua. I crediti del software e il copyright rimangono fissi.',
    'settings.hcaptcha_heading'                   => 'Impostazioni hCaptcha',
    'settings.heading'                            => 'Impostazioni Globali del Sito, Moduli e Permessi',
    'settings.mail_domain_label'                  => 'Dominio Email del Sistema (Fallback):',
    'settings.mail_driver_label'                  => 'Driver / Motore Email:',
    'settings.mail_from_desc'                     => 'Un indirizzo dedicato utilizzato come mittente per le email in uscita.',
    'settings.mail_from_label'                    => "Indirizzo Email 'Da' (From) Personalizzato:",
    'settings.mail_heading'                       => 'Configurazione Consegna Email',
    'settings.maintenance_eta_label'              => 'Previsione di Ritorno (ETA):',
    'settings.maintenance_heading'                => 'Modalità di Manutenzione del Sistema',
    'settings.maintenance_reason_label'           => 'Motivo / Messaggio per gli Utenti:',
    'settings.maintenance_toggle'                 => 'Attiva Modalità di Manutenzione (Metti il sito offline)',
    'settings.map_geocode_api_key'                => 'Chiave API ricerca luoghi',
    'settings.map_geocode_api_key_help'           => 'Necessaria solo per LocationIQ o OpenCage. Lasciare vuoto per mantenere una chiave esistente. Senza chiave, viene utilizzato Nominatim gratuito.',
    'settings.map_geocode_locationiq'             => 'LocationIQ (chiave API richiesta)',
    'settings.map_geocode_nominatim'              => 'Nominatim / OpenStreetMap (gratuito, con limite di richieste)',
    'settings.map_geocode_opencage'               => 'OpenCage (chiave API richiesta)',
    'settings.map_geocode_provider'               => 'Ricerca luoghi (geocodifica)',
    'settings.map_geocode_provider_help'          => 'Utilizzato quando l’inserimento dati cerca un nome di luogo. I risultati vengono memorizzati nella cache di questo sito.',
    'settings.map_key_placeholder'                => 'Incollare la chiave se necessaria (lasciare vuoto per mantenere quella attuale)',
    'settings.map_provider_heading'               => 'Configurazione fornitore mappa',
    'settings.map_provider_intro'                 => 'Facoltativo. Lasciare vuoti questi campi per utilizzare le immagini delle mappe integrate gratuite (CARTO/OpenStreetMap) e la ricerca di luoghi gratuita (Nominatim). Le chiavi a pagamento rimangono sul server e non vengono condivise con altri siti.',
    'settings.map_tile_api_key'                   => 'Chiave API riquadri mappa',
    'settings.map_tile_api_key_help'              => 'Necessario per CARTO, Mapbox o Stadia. Lasciare vuoto per mantenere una chiave esistente. OpenStreetMap predefinito non richiede alcuna chiave.',
    'settings.map_tile_carto'                     => 'CARTO Voyager (Chiave gratuita disponibile su carto.com/basemaps/apikey)',
    'settings.map_tile_custom'                    => 'URL riquadro personalizzato',
    'settings.map_tile_default'                   => 'Predefinito gratuito (OpenStreetMap)',
    'settings.map_tile_mapbox'                    => 'Mapbox (chiave API richiesta)',
    'settings.map_tile_osm'                       => 'OpenStreetMap (gratuito)',
    'settings.map_tile_provider'                  => 'Immagini della mappa (riquadri)',
    'settings.map_tile_provider_help'             => 'Se un’opzione a pagamento non ha una chiave, pRD torna automaticamente all’opzione gratuita predefinita.',
    'settings.map_tile_stadia'                    => 'Stadia Maps (chiave API richiesta)',
    'settings.migration_confirm'                  => 'Hai scaricato un backup del database? Questo applicherà gli aggiornamenti di schema in sospeso.',
    'settings.mod_feedback'                       => 'Invio Feedback',
    'settings.mod_feedback_desc'                  => 'Abilita il modulo di feedback pubblico e il relativo pannello di amministrazione.',
    'settings.mod_leaderboard'                    => 'Classifica e Gamification',
    'settings.mod_leaderboard_desc'               => 'Riconosce l\'impegno di trascrizione e tiene traccia dei punteggi.',
    'settings.mod_leaderboard_note'               => '(Richiede Gestione Utenti e Accesso Multiutente)',
    'settings.mod_maps'                           => 'Mappe',
    'settings.mod_maps_desc'                      => 'Mostra una mappa geografica per ogni tabella che include una colonna di posizione dinamica.',
    'settings.mod_moderation'                     => 'Flusso di Lavoro di Moderazione',
    'settings.mod_moderation_desc'                => 'Abilita la revisione dei suggerimenti di modifica e la coda di moderazione.',
    'settings.mod_users'                          => 'Gestione Utenti e Accesso Multiutente',
    'settings.mod_users_desc'                     => 'Abilita la registrazione, la gestione degli utenti e l\'autenticazione multiutente.',
    'settings.mod_volunteers'                     => 'Portale Volontari e Candidature',
    'settings.mod_volunteers_desc'                => 'Abilita il modulo di intento di volontariato pubblico e il pannello di amministrazione.',
    'settings.modules_heading'                    => 'Interruttori Moduli Applicativi e Controllo Prestazioni',
    'settings.modules_subheading'                 => 'Attiva o disattiva le funzionalità per ottimizzare le prestazioni di esecuzione dell\'applicazione e adattarti ai requisiti di distribuzione specifici.',
    'settings.no_audit_logs'                      => 'Nessun registro di audit trovato.',
    'settings.no_notices'                         => 'Nessun avviso configurato.',
    'settings.notice_content_label'               => 'Contenuto:',
    'settings.notices_heading'                    => 'Avvisi e Annunci del Sito',
    'settings.perm_info_mark'                     => 'i',
    'settings.perm_locked'                        => 'Questo permesso è obbligatorio per gli amministratori.',
    'settings.perm_more_info'                     => 'Ulteriori informazioni su',
    'settings.perm_needs'                         => 'Richiede',
    'settings.perm_needs_parent'                  => 'Si prega di attivare prima questa opzione.',
    'settings.perm_no_links'                      => 'Questo permesso non dipende da un altro.',
    'settings.perm_save_failed'                   => 'Spiacenti: pRD non è riuscito a salvare questa impostazione.',
    'settings.perm_saved'                         => 'Salvato',
    'settings.perm_swipe'                         => 'Su uno schermo piccolo, scorrere questo elenco lateralmente se necessario.',
    'settings.perm_used_by'                       => 'Richiesto da',
    'settings.permissions_heading'                => 'Matrice dei Ruoli e dei Permessi Dinamici',
    'settings.permissions_subheading'             => 'I permessi sono raggruppati per funzionalità di sistema. Espandi le sezioni per configurare le capacità e salva le modifiche sotto.',
    'settings.port_label'                         => 'Porta:',
    'settings.purge_all_confirm'                  => '⚠️ ATTENZIONE: Questo eliminerà PERMANENTEMENTE TUTTI I REGISTRI DI AUDIT DEL SISTEMA. Sei sicuro?',
    'settings.purge_records_confirm'              => 'Sei sicuro di voler cancellare tutti i registri di audit relativi ai record?',
    'settings.recaptcha_heading'                  => 'Impostazioni Google reCAPTCHA',
    'settings.role_name_label'                    => 'Nome del ruolo',
    'settings.run_token_cleanup_btn'              => 'Esegui pulizia dei token adesso',
    'settings.save_core_mail_btn'                 => 'Salva Impostazioni Core ed Email',
    'settings.save_maintenance_btn'               => 'Salva Impostazioni Manutenzione',
    'settings.save_modules_btn'                   => 'Salva Configurazione Moduli',
    'settings.save_notice_btn'                    => 'Salva Avviso',
    'settings.save_permissions_btn'               => 'Salva Matrice dei Permessi',
    'settings.saved'                              => 'Salvato.',
    'settings.schema_current'                     => 'Versione schema attuale:',
    'settings.schema_latest'                      => 'Ultima versione disponibile:',
    'settings.schema_update_notice'               => 'Aggiornamenti del database disponibili. Scarica un backup sopra prima di procedere.',
    'settings.schema_uptodate'                    => 'Il database è aggiornato.',
    'settings.secret_key_label'                   => 'Chiave Segreta (Privata):',
    'settings.send_test_btn'                      => 'Invia Email di Test',
    'settings.site_key_label'                     => 'Chiave del Sito (Pubblica):',
    'settings.smtp_encryption_label'              => 'Crittografia:',
    'settings.smtp_heading'                       => 'Impostazioni Server SMTP',
    'settings.smtp_host_label'                    => 'Host SMTP:',
    'settings.smtp_pass_label'                    => 'Password SMTP (lascia vuoto per mantenere quella attuale):',
    'settings.smtp_port_label'                    => 'Porta:',
    'settings.smtp_user_label'                    => 'Nome Utente SMTP:',
    'settings.status_active'                      => 'Attivo',
    'settings.status_inactive'                    => 'Inattivo',
    'settings.subheading'                         => 'Gestione aspetto, configurazione principale, driver di posta, opzioni di sicurezza/CAPTCHA, moduli, modalità manutenzione, annunci del sito e ruoli.',
    'settings.sys_name_label'                     => 'Nome del Sistema / Applicazione:',
    'settings.system_guest'                       => 'Sistema / Ospite',
    'settings.tab_appearance'                     => 'Aspetto',
    'settings.tab_audit'                          => 'Registro di Audit',
    'settings.tab_core'                           => 'Core ed Email',
    'settings.tab_maintenance'                    => 'Manutenzione',
    'settings.tab_modules'                        => 'Moduli',
    'settings.tab_notices'                        => 'Avvisi del Sito',
    'settings.tab_permissions'                    => 'Ruoli e Permessi',
    'settings.test_email_label'                   => 'Indirizzo Email del Destinatario:',
    'settings.test_mail_heading'                  => 'Testa Configurazione Email',
    'settings.th_action'                          => 'Azione',
    'settings.th_actions'                         => 'Azioni',
    'settings.th_actor'                           => 'Attore',
    'settings.th_capabilities'                    => 'Capacità assegnate a questo gruppo',
    'settings.th_details'                         => 'Dettagli',
    'settings.th_id'                              => 'ID',
    'settings.th_ip'                              => 'Indirizzo IP',
    'settings.th_record_id'                       => 'ID Record',
    'settings.th_role'                            => 'Ruolo',
    'settings.th_timestamp'                       => 'Marca Temporale',
    'settings.turnstile_heading'                  => 'Impostazioni Cloudflare Turnstile',
    'settings.update_db_btn'                      => 'Aggiorna Database',
    'update_database.emergency_active'            => 'L\'accesso all\'aggiornamento del database è aperto per consentire di completare questo passaggio senza effettuare il login. Si chiuderà automaticamente non appena il database sarà aggiornato.',
    'updates.already_current'                     => 'I file corrispondono già al canale di aggiornamento selezionato. Nessuna modifica effettuata.',
    'updates.apply_btn'                           => 'Applica pacchetto file',
    'updates.archive_failed'                      => 'Spiacenti: pRD non è riuscito a scrivere la copia privata del database. L\'aggiornamento dei file non è stato avviato.',
    'updates.archive_missing'                     => 'Spiacenti: backup privato non trovato.',
    'updates.channel_help'                        => 'Non modificare il canale di aggiornamento a meno di non essere uno sviluppatore. Gli sviluppatori possono inserire un tag o un riferimento di commit in questo campo.',
    'updates.channel_label'                       => 'Canale di aggiornamento (di norma dovrebbe rimanere su "Main")',
    'updates.channel_saved'                       => 'Canale di aggiornamento salvato.',
    'updates.confirm'                             => 'Desideri applicare il pacchetto di file ora? Non preoccuparti, i tuoi record rimarranno intatti. Scarica prima una copia del database se non l\'hai già fatto.',
    'updates.db_behind'                           => 'Database: è disponibile un aggiornamento in sospeso',
    'updates.db_pending'                          => 'Se pRD richiede di aggiornare il database in seguito, aprire la pagina corrispondente. Non è necessario creare file aggiuntivi.',
    'updates.db_ran'                              => 'Anche il database è stato aggiornato.',
    'updates.download_heading'                    => 'Download del database in corso..',
    'updates.download_please'                     => 'Scarica prima il database. Conserva la tua copia personale oltre alla copia privata del server.',
    'updates.files_behind'                        => 'File: è disponibile un pacchetto più recente',
    'updates.files_current'                       => 'File: aggiornati con il canale di aggiornamento selezionato',
    'updates.files_error'                         => 'File: spiacenti, pRD non è riuscito a comunicare con GitHub in questo momento. Riprova più tardi.',
    'updates.files_ok'                            => 'Congratulazioni! Pacchetto file applicato.',
    'updates.files_unknown'                       => 'File: non ancora verificati (applicare un pacchetto una volta per avviare il tracciamento)',
    'updates.force_apply'                         => 'Sostituisci comunque i file (solo se qualcosa sembra danneggiato)',
    'updates.heading'                             => 'Aggiornamenti file',
    'updates.help'                                => 'Si prega di scaricare prima il database. pRD mantiene anche una copia privata sul server. L\'aggiornamento dei file non modifica i record. Dopo l\'aggiornamento dei file, utilizzare "Aggiorna database" se è in sospeso una versione del database più recente.',
    'updates.no_sha'                              => 'Nessun pacchetto registrato finora',
    'updates.now_db'                              => 'Se la versione del database è obsoleta, utilizzare "Aggiorna database" subito dopo.',
    'updates.page_heading'                        => 'Aggiornamento di pRD',
    'updates.private_copies'                      => 'Copie private del server (solo admin)',
    'updates.running'                             => 'Questo sito è in esecuzione',

    // ------------------------------------------------------------------
    // 19. Admin: User Account Management & Leaderboard Moderation
    // ------------------------------------------------------------------
    'admin_users.delete_btn'                      => 'Elimina',
    'admin_users.disabled'                        => 'Disattivato',
    'admin_users.email_for'                       => 'Email per',
    'admin_users.enabled'                         => 'Attivato',
    'admin_users.find_user'                       => 'Trova utente',
    'admin_users.heading'                         => 'Gestione Account Utente e Moderazione Classifica',
    'admin_users.invite_user_btn'                 => 'Invita Nuovo Utente',
    'admin_users.manage_templates_btn'            => 'Gestisci Modelli Email',
    'admin_users.no'                              => 'No',
    'admin_users.no_search_match'                 => 'Nessun utente corrisponde a questa ricerca.',
    'admin_users.no_users'                        => 'Nessun utente trovato.',
    'admin_users.not_verified_badge'              => 'Non verificato',
    'admin_users.points_for'                      => 'Punti per',
    'admin_users.protected_admin'                 => 'Amministratore principale protetto',
    'admin_users.reactivate_btn'                  => 'Riattiva',
    'admin_users.resend_invite_btn'               => 'Reinvia Invito',
    'admin_users.resend_invite_confirm'           => 'Reinviare l\'email di invito dell\'account a questo utente?',
    'admin_users.reset_2fa_btn'                   => 'Disattiva 2FA',
    'admin_users.reset_2fa_confirm'               => 'Ripristinare e disattivare il 2FA per questo utente?',
    'admin_users.reset_password_btn'              => 'Reimposta Password',
    'admin_users.reset_pwd_confirm'               => 'Inviare un link di reset password a questo utente?',
    'admin_users.save_email_title'                => 'Salva Nuovo Indirizzo Email',
    'admin_users.search_help'                     => 'Filtra l\'elenco mentre digiti. Cancella il campo per mostrare di nuovo tutti.',
    'admin_users.search_placeholder'              => 'Nome utente, email o ruolo…',
    'admin_users.set_score_btn'                   => 'Imposta Punteggio',
    'admin_users.status_active'                   => 'Attivo',
    'admin_users.status_suspended'                => 'Sospeso',
    'admin_users.subheading'                      => 'Verifica lo stato dell\'utente, assegna ruoli, sovrascrivi email, avvia reset password o inviti, ripristina il 2FA o sospendi gli account.',
    'admin_users.suspend_btn'                     => 'Sospendi Account',
    'admin_users.suspend_confirm'                 => 'Sospendere l\'utente e revocare l\'accesso a causa di un uso improprio?',
    'admin_users.th_2fa'                          => '2FA',
    'admin_users.th_actions'                      => 'Azioni e Moderazione',
    'admin_users.th_email_override'               => 'Email e Sovrascrittura',
    'admin_users.th_role_assignment'              => 'Assegnazione Ruolo',
    'admin_users.th_score'                        => 'Punteggio',
    'admin_users.th_status'                       => 'Stato',
    'admin_users.th_username'                     => 'Nome Utente',
    'admin_users.update_btn'                      => 'Aggiorna',
    'admin_users.verified_badge'                  => 'Verificato',
    'admin_users.verified_label'                  => 'Verificato:',
    'admin_users.yes'                             => 'Sì',

    // ------------------------------------------------------------------
    // 20. Admin: View Ticket & Threaded Dialogue
    // ------------------------------------------------------------------
    'view_ticket.admin_label'                     => 'Amministratore',
    'view_ticket.back_to_dashboard'               => 'Torna al Pannello Ticket',
    'view_ticket.dialogue_heading'                => 'Cronologia della Conversazione',
    'view_ticket.no_replies'                      => 'Nessuna risposta registrata.',
    'view_ticket.on_date'                         => 'il',
    'view_ticket.post_reply_heading'              => 'Pubblica Risposta e Notifica Mittente',
    'view_ticket.reply_placeholder'               => 'Scrivi la tua risposta qui...',
    'view_ticket.send_reply_btn'                  => 'Invia Risposta ed Email al Mittente',
    'view_ticket.staff'                           => 'Personale',
    'view_ticket.status_completed'                => 'Completato',
    'view_ticket.status_pending'                  => 'In Sospeso',
    'view_ticket.status_progress'                 => 'In Corso',
    'view_ticket.status_rejected'                 => 'Rifiutato',
    'view_ticket.submitted_by'                    => 'Inviato da:',
    'view_ticket.submitted_fields'                => 'Campi del modulo inviati:',
    'view_ticket.support_request'                 => 'Richiesta di Supporto',
    'view_ticket.ticket_heading_prefix'           => 'Ticket',
    'view_ticket.ticket_status_label'             => 'Stato del Ticket:',

    // ------------------------------------------------------------------
    // 21. Admin: Volunteer Submissions & Workflow Dashboard
    // ------------------------------------------------------------------
    'volunteer_dashboard.accept_invite_btn'       => 'Approva e Invia Invito',
    'volunteer_dashboard.accept_title'            => 'Approva tramite Sistema di Invito Utente',
    'volunteer_dashboard.chat_label'              => 'Conversazione:',
    'volunteer_dashboard.chat_notes_btn'          => 'Conversazione e Note',
    'volunteer_dashboard.delete_confirm'          => 'Eliminare questo record di volontario?',
    'volunteer_dashboard.heading'                 => 'Candidature Volontari e Flusso di Lavoro',
    'volunteer_dashboard.manage_emails_btn'       => 'Gestisci Modelli Email',
    'volunteer_dashboard.manage_schema_btn'       => 'Gestisci Schema Modulo',
    'volunteer_dashboard.modal_date_label'        => 'Data e Ora della Conversazione Programmata:',
    'volunteer_dashboard.modal_heading'           => 'Gestisci Colloquio e Note del Candidato',
    'volunteer_dashboard.modal_notes_label'       => 'Note Colloquio / Riunione:',
    'volunteer_dashboard.modal_notes_placeholder' => 'Registra il feedback della conversazione qui...',
    'volunteer_dashboard.modal_status_label'      => 'Stato Candidatura:',
    'volunteer_dashboard.no_notes'                => 'Nessuna nota',
    'volunteer_dashboard.no_submissions'          => 'Nessuna candidatura di volontario trovata.',
    'volunteer_dashboard.notes_label'             => 'Note:',
    'volunteer_dashboard.save_changes_btn'        => 'Salva Modifiche',
    'volunteer_dashboard.status_accepted'         => 'Approvato',
    'volunteer_dashboard.status_chat'             => 'Conversazione Programmata',
    'volunteer_dashboard.status_pending'          => 'In Attesa di Revisione',
    'volunteer_dashboard.status_rejected'         => 'Rifiutato',
    'volunteer_dashboard.subheading'              => 'Esamina le candidature, pianifica conversazioni, registra note di colloquio e approva i candidati nel sistema.',
    'volunteer_dashboard.th_interview_notes'      => 'Colloquio / Note',
    'volunteer_dashboard.th_name'                 => 'Nome',
    'volunteer_dashboard.th_status'               => 'Stato',
    'volunteer_dashboard.volunteer_prefix'        => 'Volontario',

    // ------------------------------------------------------------------
    // 22. API: AJAX Search & Filtering
    // ------------------------------------------------------------------
    'api_search.error_public_forbidden'           => '403 Vietato: La visualizzazione pubblica non è abilitata.',
    'api_search.error_unauthorized_table'         => 'Accesso non autorizzato alla tabella.',
    'api_search.history_btn'                      => 'Cronologia',
    'api_search.no_records'                       => 'Nessun record trovato in questa directory.',
    'api_search.suggest_edit_btn'                 => 'Suggerisci Modifica',

    // ------------------------------------------------------------------
    // 23. Errors & HTTP Templates
    // ------------------------------------------------------------------
    'admin_errors.btn_find'                       => 'Trova',
    'admin_errors.file'                           => 'File',
    'admin_errors.found_heading'                  => 'Trovato:',
    'admin_errors.heading'                        => 'Registro errori',
    'admin_errors.label_error_id'                 => 'ID di riferimento errore',
    'admin_errors.line_prefix'                    => 'Riga',
    'admin_errors.local_time'                     => 'Il tuo orario locale',
    'admin_errors.message'                        => 'Messaggio',
    'admin_errors.no_recent'                      => 'Nessuna voce nel registro errori strutturato.',
    'admin_errors.not_found'                      => 'Nessuna voce di log trovata per',
    'admin_errors.placeholder_id'                 => 'E-YYYYMMDD-XXXXXX',
    'admin_errors.recent_heading'                 => 'Errori recenti',
    'admin_errors.request'                        => 'Richiesta',
    'admin_errors.stack_trace'                    => 'Traccia dello stack',
    'admin_errors.subheading'                     => 'Cerca un ID di riferimento mostrato su una pagina di errore (es. E-20260810-A1B2C3). I dettagli completi sono memorizzati nel log del server; i dettagli a schermo per i visitatori dipendono da APP_DEBUG.',
    'admin_errors.th_id'                          => 'ID',
    'admin_errors.th_message'                     => 'Messaggio',
    'admin_errors.th_time'                        => 'Orario',
    'admin_errors.time_utc'                       => 'Orario (UTC)',
    'admin_errors.type'                           => 'Tipo',
    'error_template.debug_details'                => 'Dettagli di debug',
    'error_template.file_label'                   => 'File:',
    'error_template.line_label'                   => 'Riga',
    'error_template.return_home_btn'              => 'Torna alla Pagina Iniziale Pubblica',
    'error_template.stack_trace'                  => 'Traccia dello stack',

    // ------------------------------------------------------------------
    // 24. Public: Ticket Intake & Feedback Portal
    // ------------------------------------------------------------------
    'feedback.email_label'                        => 'Indirizzo Email:',
    'feedback.first_name_label'                   => 'Nome:',
    'feedback.hp_label'                           => 'Lascia vuoto',
    'feedback.multi_select_hint'                  => 'Premi Ctrl o Cmd per selezionare più elementi.',
    'feedback.required_title'                     => 'Campo Obbligatorio',
    'feedback.select_placeholder'                 => '-- Seleziona --',
    'feedback.subject_label'                      => 'Oggetto / Titolo della Richiesta:',
    'feedback.submit_btn'                         => 'Invia Ticket',
    'feedback.surname_label'                      => 'Cognome:',

    // ------------------------------------------------------------------
    // 25. Centralised Feedback System (gh.*) ---
    // ------------------------------------------------------------------
    'gh.action_placeholder'                       => 'Descrivi le azioni per aiutare a individuare i file errati',
    'gh.add_comment_below'                        => 'Aggiungi il tuo commento qui sotto e invia.',
    'gh.additional_context'                       => 'Contesto aggiuntivo',
    'gh.additional_placeholder'                   => 'Aggiungi qualsiasi altro contesto qui.',
    'gh.attach_logs'                              => 'Allega un estratto recente del registro di sistema',
    'gh.bug_desc_placeholder'                     => 'Una descrizione chiara e concisa di cosa sia il bug.',
    'gh.click_to_comment'                         => 'Fai clic per aggiungere un commento a questo problema invece →',
    'gh.comment_help'                             => 'Questo viene aggiunto al problema GitHub esistente. Non modifica le etichette o la priorità di tale problema.',
    'gh.comment_label'                            => 'Il tuo commento',
    'gh.commenting_on'                            => 'Commento sul problema esistente',
    'gh.consent_agree'                            => 'Accetto di procedere con il Sistema di Feedback Centralizzato secondo questi termini.',
    'gh.consent_text'                             => 'Questo sistema si collega al server di pRD per garantire che la tua segnalazione sia autentica. Nessuna informazione viene condivisa con il server di pRD durante questo processo. Tuttavia, la tua segnalazione sarà visibile pubblicamente su Internet. Abbiamo bisogno della tua autorizzazione per procedere.',
    'gh.consent_title'                            => 'Avviso di connessione al sistema e autorizzazione',
    'gh.created_success'                          => 'creato con successo.',
    'gh.creating_new'                             => 'Creazione di un nuovo problema',
    'gh.detailed_description'                     => 'Descrizione dettagliata',
    'gh.doc_paragraphs'                           => 'Si prega di fornire il numero o i numeri dei paragrafi o citare i paragrafi sottostanti:',
    'gh.doc_placeholder'                          => 'Nome del documento Wiki o URL',
    'gh.doc_proposed'                             => 'Cosa pensi che debbano riportare i paragrafi interessati in alternativa?',
    'gh.doc_reasoning'                            => 'Si prega di fornire la motivazione:',
    'gh.doc_related'                              => 'A quale documento all\'interno del Wiki si riferisce il tuo suggerimento?',
    'gh.duplicate_confirm_text'                   => 'Sono stati trovati problemi aperti simili. Confermo che questo è un problema distinto che non è stato ancora segnalato.',
    'gh.duplicate_warning'                        => 'Avviso di duplicato',
    'gh.error_occurred'                           => 'Si è verificato un errore.',
    'gh.expected_behavior'                        => 'Comportamento atteso',
    'gh.expected_placeholder'                     => 'Una descrizione chiara e concisa di ciò che ti aspettavi accadesse.',
    'gh.failed_check_issues'                      => 'Impossibile verificare i problemi esistenti.',
    'gh.feature_problem'                          => 'La tua estensione o richiesta di funzionalità è correlata a un problema? Si prega di descriverlo.',
    'gh.feature_solution'                         => 'Si prega di descrivere la soluzione desiderata',
    'gh.heading'                                  => 'Sistema di Feedback Centralizzato di pRD',
    'gh.intro_p1'                                 => 'Grazie per aver scelto pRD per il tuo progetto di database. Questa risorsa è disponibile esclusivamente per te in qualità di Amministratore.',
    'gh.intro_step1'                              => 'Questo sistema si collega direttamente a GitHub e verifica la presenza di problemi esistenti.',
    'gh.intro_step2'                              => 'Puoi aggiungere un commento a un problema esistente se ritieni che non sia già completamente coperto.',
    'gh.intro_step3'                              => 'Oppure, se hai individuato un nuovo problema, il sistema offre una comoda funzione per aprire una nuova segnalazione su GitHub che verrà presa in carico dal team di pRD.',
    'gh.issue_created_label'                      => 'Problema',
    'gh.locked_onto_issue'                        => 'Associato al problema',
    'gh.make_note_link'                           => 'Prendi nota di questo link per consultarlo in seguito.',
    'gh.network_error'                            => 'Errore di rete durante l\'elaborazione della richiesta.',
    'gh.no_recent_logs'                           => 'Nessun registro di audit trovato nelle ultime 48 ore.',
    'gh.no_similar_found'                         => 'Nessun problema aperto simile trovato.',
    'gh.open_on_github'                           => 'Apri il problema in una nuova scheda',
    'gh.optional_diagnostic'                      => 'traccia di diagnostica facoltativa per gli sviluppatori',
    'gh.paste_placeholder'                        => 'Incolla il testo qui',
    'gh.post_comment_btn'                         => 'Pubblica commento sul problema',
    'gh.posting_comment'                          => 'Pubblicazione del commento...',
    'gh.preview_back'                             => 'Torna al modulo',
    'gh.preview_body_label'                       => 'Corpo (puoi modificare o rimuovere i dettagli personali)',
    'gh.preview_heading'                          => 'Si prega di visualizzare l\'anteprima prima dell\'invio',
    'gh.preview_reminder'                         => 'Si prega di leggere questo un\'ultima volta prima dell\'invio. Il testo sottostante sarà pubblico su GitHub. Puoi rimuovere i dettagli personali; le caselle di assegnazione del team vengono reimpostate al momento dell\'invio.',
    'gh.preview_send'                             => 'Invia a GitHub',
    'gh.preview_title_label'                      => 'Titolo',
    'gh.proposed_placeholder'                     => 'Testo sostitutivo proposto',
    'gh.public_confirm'                           => 'Ho letto l\'anteprima e comprendo che sarà pubblica',
    'gh.quote_placeholder'                        => 'Riferimento al paragrafo o citazione',
    'gh.reasoning_placeholder'                    => 'Perché questa modifica è necessaria',
    'gh.save_tracking_link'                       => 'Salva questo link di tracciamento:',
    'gh.select_audit_logs'                        => 'Seleziona i registri di audit pertinenti (ultime 48 ore)',
    'gh.select_audit_logs_desc'                   => 'Seleziona eventuali eventi di registro recenti sottostanti pertinenti a questo problema per fornire un contesto al team.',
    'gh.select_type'                              => 'Seleziona il tipo di problema',
    'gh.sending'                                  => 'Invio in corso…',
    'gh.sensitive_confirm'                        => 'Comprendo e desidero comunque pubblicare',
    'gh.sensitive_warn'                           => 'Questo testo potrebbe contenere informazioni riservate o credenziali. Rimuovile o conferma qui sotto se desideri comunque pubblicare.',
    'gh.sev_high'                                 => 'Alta (priorità: alta)',
    'gh.sev_high_desc'                            => 'Problema di sicurezza e/o funzionalità completamente non funzionante.',
    'gh.sev_low'                                  => 'Bassa (priorità: bassa)',
    'gh.sev_low_desc'                             => 'SOLO SE - Non è un problema di sicurezza e/o NON riguarda una funzionalità completamente non funzionante.',
    'gh.sev_med'                                  => 'Media (priorità: media)',
    'gh.sev_med_desc'                             => 'SOLO SE - Non è un problema di sicurezza e/o RIGUARDA una funzionalità ampiamente non funzionante.',
    'gh.severity_level'                           => 'Livello di gravità',
    'gh.similar_issues_desc'                      => 'Mentre digiti un titolo, verifichiamo la presenza di problemi aperti. Fai clic su qualsiasi problema sottostante se desideri aggiungere i tuoi commenti direttamente ad esso invece di crearne uno duplicato.',
    'gh.similar_issues_title'                     => 'Problemi esistenti simili',
    'gh.solution_placeholder'                     => 'Una descrizione chiara e concisa di ciò che desideri accada.',
    'gh.steps_to_reproduce'                       => 'Passaggi per riprodurre',
    'gh.submit_btn'                               => 'Invia feedback',
    'gh.submitting'                               => 'Invio in corso...',
    'gh.switch_new'                               => 'Passa a un nuovo problema',
    'gh.title_placeholder'                        => 'Breve riepilogo del problema',
    'gh.title_summary'                            => 'Titolo / Riepilogo',
    'gh.tracking_link_aria'                       => 'Link di tracciamento GitHub per il problema creato',
    'gh.trans_action'                             => 'Cosa stavi facendo su pRD in quel momento?',
    'gh.trans_offending'                          => 'Si prega di copiare e incollare il testo errato:',
    'gh.type_at_least_3'                          => 'Digita almeno 3 caratteri per cercare...',
    'gh.type_bug'                                 => '1. Segnalazione di bug',
    'gh.type_documentation'                       => '3. Richiesta di miglioramento della documentazione',
    'gh.type_enhancement'                         => '2. Estensione o richiesta di funzionalità',
    'gh.type_to_search'                           => 'Inizia a digitare un titolo per cercare...',
    'gh.type_translation'                         => '4. Suggerisci una traduzione',

    // ------------------------------------------------------------------
    // 26. Security Engine & Firewall
    // ------------------------------------------------------------------
    'security_engine.err_access_denied'           => 'Errore di sicurezza: Accesso negato.',
    'security_engine.err_captcha_failed'          => 'Verifica CAPTCHA fallita. Riprova.',
    'security_engine.err_complete_captcha'        => 'Completa il controllo di sicurezza CAPTCHA.',
    'security_engine.err_excessive_links'         => 'Invio bloccato a causa di un numero eccessivo di link.',
    'security_engine.err_rate_limit'              => 'Troppe richieste da questo indirizzo IP. Riprova più tardi.',
    'security_engine.err_suspicious_agent'        => 'Errore di sicurezza: Firma client sospetta.',

    // ------------------------------------------------------------------
    // 27. Installer Wizard
    // ------------------------------------------------------------------
    'install.admin_confirm_password_label'        => 'Conferma Password Amministratore',
    'install.admin_email_label'                   => 'Email Amministratore',
    'install.admin_heading'                       => 'Account Amministratore del Sito',
    'install.admin_password_label'                => 'Password Amministratore (min. 8 caratteri)',
    'install.admin_subheading'                    => 'Questi sono i dati di accesso per questo sito web (non per il database).',
    'install.admin_username_label'                => 'Nome Utente Amministratore',
    'install.back_btn'                            => 'Indietro',
    'install.close_alert'                         => 'Chiudi',
    'install.complete_desc'                       => 'L\'installazione di pRD è stata completata e l\'installatore è stato bloccato per evitare esecuzioni accidentali.',
    'install.complete_heading'                    => 'Congratulazioni!',
    'install.complete_title'                      => 'Installazione Completata',
    'install.continue_btn'                        => 'Continua',
    'install.db_heading'                          => 'Connessione al Database',
    'install.db_hint'                             => 'Utilizza i dettagli MySQL del pannello di controllo del tuo hosting. Questo non è il login dell\'Amministratore del sito web (quello arriverà dopo).',
    'install.db_host_label'                       => 'Host del Database',
    'install.db_name_label'                       => 'Nome del Database',
    'install.db_pass_label'                       => 'Password del Database',
    'install.db_submit_btn'                       => 'Crea Tabelle e Continua',
    'install.db_user_label'                       => 'Utente del Database',
    'install.delete_folder_hint'                  => 'Molti hosting web non consentono all\'installatore di eliminare la propria cartella. Se il pulsante non produce effetti, aprire il file manager dell\'hosting o il client FTP, accedere alla cartella del progetto pRD ed eliminare o rinominare lì la directory di installazione.',
    'install.demo_choose'                         => 'Quali pacchetti desideri installare?',
    'install.demo_continue'                       => 'Aggiungi i pacchetti selezionati e termina',
    'install.demo_heading'                        => 'Pacchetti dimostrativi facoltativi',
    'install.demo_help'                           => 'Puoi aggiungere tabelle iniziali per registri parrocchiali e/o una libreria di libri. Scegli solo tabelle e colonne, o includi alcune righe di esempio fittizie. Puoi saltare questo passaggio e aggiungere o rimuovere i pacchetti in seguito nelle Impostazioni. Rimuovere un pacchetto in un secondo momento eliminerà anche eventuali righe extra aggiunte in quelle tabelle dimostrative.',
    'install.demo_library'                        => 'Demo libreria di libri',
    'install.demo_library_desc'                   => 'Un semplice catalogo di titoli, autori e posizioni sugli scaffali a cui puoi aggiungere elementi.',
    'install.demo_parish'                         => 'Demo registri parrocchiali',
    'install.demo_parish_desc'                    => 'Battesimi, matrimoni e sepolture — Il flusso di lavoro per cui pRD è stato originariamente creato!',
    'install.demo_schema_data'                    => 'Tabelle, colonne e dati di esempio',
    'install.demo_schema_only'                    => 'Solo tabelle e colonne (nessuna riga di dati di esempio)',
    'install.demo_skip'                           => 'Salta per ora',
    'install.demo_what'                           => 'Cosa desideri aggiungere?',
    'install.detail_prefix'                       => 'Dettaglio:',
    'install.done_heading'                        => 'Congratulazioni! pRD è stato installato con successo.',
    'install.done_message'                        => 'Il programma di installazione è ora bloccato. Puoi accedere con l\'account amministratore appena creato.',
    'install.err_admin_required'                  => 'Tutti i campi amministratore sono obbligatori.',
    'install.err_admin_save_failed'               => 'Salvataggio utente amministratore fallito. Controlla la struttura della tabella utenti.',
    'install.err_complete_db_first'               => 'Completa prima la fase del database.',
    'install.err_config_load'                     => 'Impossibile utilizzare la configurazione esistente:',
    'install.err_db_not_empty'                    => 'Questo database non è vuoto. Usa un nuovo database vuoto (o svuota tutte le tabelle) e riprova.',
    'install.err_db_required'                     => 'Il nome del database e l\'utente sono obbligatori.',
    'install.err_install_not_removed'             => 'pRD non è riuscito a rimuovere automaticamente la cartella di installazione (il server non lo ha permesso). Si prega di eliminare o rinominare la cartella di installazione nel file manager. Nessun dato del sito è stato modificato.',
    'install.err_invalid_email'                   => 'Indirizzo email non valido.',
    'install.err_password_length'                 => 'La password deve contenere almeno 8 caratteri.',
    'install.err_passwords_match'                 => 'Le password non coincidono.',
    'install.err_write_permission'                => 'PHP non può creare file in questa cartella del progetto.',
    'install.finish_btn'                          => 'Completa Installazione',
    'install.heading'                             => 'Benvenuto nel sistema di installazione di pRD.',
    'install.hide_password'                       => 'Nascondi',
    'install.home_link'                           => 'Vai al Sito',
    'install.lang_label'                          => 'Lingua',
    'install.login_link'                          => 'Accedi',
    'install.mod_feedback'                        => 'Invio feedback',
    'install.mod_feedback_desc'                   => 'Abilita un modulo di feedback pubblico e un pannello di controllo amministrativo.',
    'install.mod_leaderboard'                     => 'Classifica e gamification',
    'install.mod_leaderboard_desc'                => 'Tabella punteggi opzionale. Richiede che gli account utente siano attivi.',
    'install.mod_maps'                            => 'Mappe',
    'install.mod_maps_desc'                       => 'Campi di posizione e una mappa per tabella. È possibile modificarlo in seguito in Admin → Impostazioni.',
    'install.mod_moderation'                      => 'Flusso di lavoro di moderazione',
    'install.mod_moderation_desc'                 => 'Un sistema di flusso di lavoro di moderazione che consente agli utenti di suggerire modifiche e a un moderatore di approvarle o rifiutarle.',
    'install.mod_users'                           => 'Gestione utenti e accesso multiutente',
    'install.mod_users_desc'                      => 'Consenti alle persone di registrarsi e accedere.',
    'install.mod_volunteers'                      => 'Portale volontari e candidature',
    'install.mod_volunteers_desc'                 => 'Abilita un modulo pubblico di interesse per il volontariato e un pannello di gestione amministrativa.',
    'install.modules_continue'                    => 'Salva e continua',
    'install.modules_heading'                     => 'Quali parti di pRD desideri utilizzare?',
    'install.modules_help'                        => 'Seleziona le funzionalità che desideri fin dal primo giorno. Potrai modificarle in seguito in Admin → Impostazioni sito.',
    'install.modules_later'                       => 'È possibile attivare o disattivare qualsiasi modulo in un secondo momento in Admin → Impostazioni sito.',
    'install.msg_admin_created'                   => 'È stato creato un account amministratore. Puoi aggiungere pacchetti dimostrativi facoltativi o saltare questo passaggio.',
    'install.msg_db_ready'                        => 'Database pronto. Crea il tuo account amministratore per completare.',
    'install.msg_install_removed'                 => 'La cartella di installazione è stata rimossa. Ora puoi effettuare il login.',
    'install.msg_installation_complete'           => 'Installazione completata.',
    'install.msg_modules_saved'                   => 'Le selezioni dei moduli sono state salvate. È possibile aggiungere pacchetti demo opzionali o saltare questo passaggio.',
    'install.msg_schema_imported'                 => 'Database connesso e schema importato. Crea il tuo account amministratore.',
    'install.page_title'                          => 'Installazione — Registro Parrocchiale',
    'install.remove_already_gone'                 => 'La cartella di installazione era già stata rimossa.',
    'install.remove_failed'                       => 'Spiacenti: pRD non è riuscito a eliminare automaticamente la cartella di installazione (spesso i server web non lo consentono). Tramite il file manager dell\'hosting web o via FTP, aprire la cartella di pRD ed eliminare o rinominare la directory di installazione al suo interno.',
    'install.remove_folder_btn'                   => 'Si prega di rimuovere la cartella di installazione!',
    'install.remove_ok'                           => 'La cartella di installazione è stata rimossa.',
    'install.req_all_ok'                          => 'Congratulazioni, tutti i controlli sono stati superati. Sentiti libero di continuare.',
    'install.req_fail_msg'                        => 'Sfortunatamente, non tutti i requisiti di pRD sono soddisfatti sul tuo server. Si prega di risolverli e sentiti libero di riavviare il programma di installazione. Non è stato scritto alcun dato.',
    'install.req_heading'                         => 'pRD ha verificato i propri requisiti sul tuo server.',
    'install.req_logs'                            => 'Cartella log con permessi di scrittura (o cartella del progetto)',
    'install.req_logs_fail'                       => 'Cartella dei log scrivibile — spiacenti, questo controllo non è stato superato.',
    'install.req_logs_ok'                         => 'Cartella dei log scrivibile (o cartella del progetto) — Congratulazioni!',
    'install.req_pdo'                             => 'Estensione PDO MySQL',
    'install.req_pdo_fail'                        => 'Estensione PDO MySQL — spiacenti, questo controllo non è stato superato.',
    'install.req_pdo_ok'                          => 'Estensione PDO MySQL — Congratulazioni!',
    'install.req_php'                             => 'PHP 8.0+ (rilevato %s)',
    'install.req_php_fail'                        => 'pRD richiede PHP 8.0+ — spiacenti, questo controllo non è stato superato.',
    'install.req_php_ok'                          => 'pRD richiede PHP 8.0+ — Congratulazioni!',
    'install.req_probe'                           => 'Capacità di creare file in questa cartella del progetto',
    'install.req_probe_fail'                      => 'Capacità di creare file in questa cartella del progetto — spiacenti, questo controllo non è stato superato.',
    'install.req_probe_ok'                        => 'Puoi creare file in questa cartella del progetto — Congratulazioni!',
    'install.show_password'                       => 'Mostra',
    'install.subheading'                          => 'Assicurati che il database che intendi utilizzare sia vuoto prima di continuare.',

    // ------------------------------------------------------------------
    // 28. Demo Packs
    // ------------------------------------------------------------------
    'demo.already_installed'                      => 'Questa demo è già installata',
    'demo.already_installed_hint'                 => 'Questo pacchetto è già installato, quindi non può essere selezionato di nuovo.',
    'demo.choose_packs'                           => 'Quali pacchetti desideri?',
    'demo.heading'                                => 'Pacchetti dimostrativi',
    'demo.install_btn'                            => 'Aggiungi i pacchetti selezionati',
    'demo.install_heading'                        => 'Aggiungi un pacchetto dimostrativo',
    'demo.install_help'                           => 'Solo tabelle e colonne: struttura vuota che compili tu stesso. Tabelle, colonne e dati di esempio: alcune righe fittizie per poter iniziare subito a cliccare. Puoi rimuovere le righe di esempio in seguito senza eliminare le tabelle.',
    'demo.intro'                                  => 'Tabelle iniziali facoltative per consentirti di provare pRD prima di progettare le tue. Puoi aggiungere un pacchetto o entrambi. Nulla di tutto ciò è obbligatorio per un sito attivo.',
    'demo.none_installed'                         => '???',
    'demo.remove_data_btn'                        => 'Rimuovi solo i dati di esempio',
    'demo.remove_data_help'                       => 'Elimina solo le righe di esempio originali. Tabelle, colonne e qualsiasi riga aggiunta da te vengono mantenute.',
    'demo.remove_heading'                         => 'Rimuovi un pacchetto dimostrativo',
    'demo.remove_pack_btn'                        => 'Rimuovi tabelle, colonne e tutti i dati in questo pacchetto',
    'demo.remove_pack_confirm'                    => 'Questa operazione elimina le tabelle e le colonne dimostrative, le righe di esempio e qualsiasi dato aggiuntivo inserito in tali tabelle dimostrative. Le altre tabelle non vengono toccate. Continuare?',
    'demo.remove_pack_help'                       => 'Elimina le tabelle e le colonne dimostrative, le righe di esempio e qualsiasi dato inserito successivamente in tali tabelle dimostrative. Le tue altre tabelle non saranno interessate.',
    'demo.schema_and_data'                        => 'Tabelle, colonne e dati di esempio',
    'demo.schema_only'                            => 'Solo tabelle e colonne (nessuna riga di esempio)',
    'demo.what_to_add'                            => 'Cosa aggiungere',

    // ------------------------------------------------------------------
    // 29. Leaderboard
    // ------------------------------------------------------------------
    'leaderboard.aria_region'                     => 'Visualizzazione Classifica',
    'leaderboard.default_role'                    => 'Utente',
    'leaderboard.heading'                         => 'Classifica di Partecipazione della Comunità',
    'leaderboard.medal_bronze'                    => 'Medaglia di Bronzo',
    'leaderboard.medal_crown'                     => 'Corona Livello 10',
    'leaderboard.medal_glowing'                   => 'Stella Brillante Livello 9',
    'leaderboard.medal_gold'                      => 'Medaglia d\'Oro',
    'leaderboard.medal_military'                  => 'Medaglia Militare Livello 8',
    'leaderboard.medal_ribbon'                    => 'Nastro Livello 4',
    'leaderboard.medal_rosette'                   => 'Rosetta Livello 5',
    'leaderboard.medal_silver'                    => 'Medaglia d\'Argento',
    'leaderboard.medal_star'                      => 'Stella Livello 7',
    'leaderboard.medal_trophy'                    => 'Trofeo Livello 6',
    'leaderboard.no_users'                        => 'Nessun utente attivo trovato nella classifica.',
    'leaderboard.subheading'                      => 'Un riconoscimento agli sforzi dei membri della nostra comunità che aiutano a raccogliere, trascrivere o gestire i record.',
    'leaderboard.th_contributor'                  => 'Contributore',
    'leaderboard.th_rank'                         => 'Posizione',
    'leaderboard.th_role'                         => 'Ruolo',
    'leaderboard.th_score'                        => 'Punteggio',
    'leaderboard.you_badge'                       => '(Tu)',

    // ------------------------------------------------------------------
    // 30. Site Footer
    // ------------------------------------------------------------------
    'footer.compiled_notice'                      => 'Registri parrocchiali compilati da fonti storiche di pubblico dominio.',
    'footer.rights_reserved'                      => 'Tutti i diritti riservati.',
    'footer.software_notice'                      => 'pRD (V1.18) è open source sotto Licenza MIT - Un ringraziamento a @katherinehuk di GitHub per un secondo paio d\'occhi!',

    // ------------------------------------------------------------------
    // 31. Site Header & Head
    // ------------------------------------------------------------------
    'header.default_title'                        => 'Database dei Registri Parrocchiali',
    'header.skip_to_content'                      => 'Salta al contenuto principale',

    // ------------------------------------------------------------------
    // 32. Notices Banner Module
    // ------------------------------------------------------------------
    'notices_banner.close_title'                  => 'Chiudi avviso',

    // ------------------------------------------------------------------
    // 33. Record History & Audit Trail
    // ------------------------------------------------------------------
    'record_history.actor_label'                  => 'Attore:',
    'record_history.directory_table_label'        => 'Tabella della Directory:',
    'record_history.empty_value'                  => '[Vuoto]',
    'record_history.exit_no_record'               => 'Nessun record specificato.',
    'record_history.exit_not_found'               => 'Record non trovato.',
    'record_history.heading_prefix'               => 'Cronologia e Registro di Audit: Record',
    'record_history.no_history'                   => 'Nessun evento di audit storico registrato specificamente per questo record.',
    'record_history.proposed_value'               => 'Valore Proposto:',
    'record_history.purge_btn'                    => 'Pulisci Registro',
    'record_history.purge_confirm'                => 'Eliminare questa voce specifica del registro di audit?',
    'record_history.reasoning_evidence'           => 'Motivazione / Prova:',
    'record_history.return_btn'                   => 'Indietro',
    'record_history.snapshot_heading'             => 'Istantanea dei valori attivi correnti',
    'record_history.subheading_lifecycle'         => 'Mostra il ciclo di vita sociale delle modifiche, dei suggerimenti e delle prove associate a questo record.',
    'record_history.system_guest'                 => 'Sistema / Ospite',
    'record_history.target_column'                => 'Colonna di Destinazione:',
    'record_history.timeline_heading'             => 'Cronologia degli Eventi e delle Attività',

    // ------------------------------------------------------------------
    // 34. Standalone Update Database Gateway and Settings Database Update Dialogue
    // ------------------------------------------------------------------
    'index.remove_emergency_file'                 => 'Rimuovi file di accesso di emergenza',
    'update_database.backup_notice'               => 'Esegui un backup del database prima di eseguire gli aggiornamenti quando possibile.',
    'update_database.confirm_prompt'              => 'Hai eseguito il backup del database? Premi OK per applicare gli aggiornamenti di schema in sospeso.',
    'update_database.current_version'             => 'Versione schema attuale:',
    'update_database.err_failed'                  => 'Migrazione fallita:',
    'update_database.heading'                     => '⚠️ Aggiornamento di Sistema Necessario',
    'update_database.latest_version'              => 'Ultima versione disponibile:',
    'update_database.msg_success'                 => 'Database aggiornato con successo! Applicate %d migrazioni.',
    'update_database.msg_uptodate'                => 'Il database è già aggiornato.',
    'update_database.page_title'                  => 'Aggiornamento di Sistema Necessario — Registro Parrocchiale',
    'update_database.proceed_login'               => 'Vai alla Pagina di Accesso',
    'update_database.remove_emergency_file'       => 'Rimuovi file di accesso di emergenza',
    'update_database.subheading'                  => 'Lo schema del database dell\'applicazione è obsoleto e richiede un aggiornamento dello schema prima di procedere.',
    'update_database.update_btn'                  => 'Aggiorna Database Ora',

    // ------------------------------------------------------------------
    // 35. User Authentication Action
    // ------------------------------------------------------------------
    'authenticate.err_invalid_credentials'        => 'Credenziali non valide o accesso all\'account limitato.',

    // ------------------------------------------------------------------
    // 36. Save Data Entry Action
    // ------------------------------------------------------------------
    'save_data_entry.audit_created_prefix'        => 'Record creato nella tabella con ID %d.',
    'save_data_entry.err_email'                   => 'Inserisci un\'email valida per %s.',
    'save_data_entry.err_invalid_choice'          => 'Si prega di scegliere un\'opzione elencata per %s.',
    'save_data_entry.err_location'                => 'Scegli un luogo dall’elenco e aggiungi un titolo e un testo breve per %s.',
    'save_data_entry.err_max'                     => '%s supera il massimo.',
    'save_data_entry.err_min'                     => '%s è inferiore al minimo.',
    'save_data_entry.err_not_number'              => '%s deve essere un numero intero.',
    'save_data_entry.err_required_field'          => 'Il campo obbligatorio \'%s\' non può essere vuoto.',
    'save_data_entry.err_url'                     => 'Inserisci un indirizzo web che inizi con http:// o https:// per %s.',
    'save_data_entry.msg_success'                 => 'Record aggiunto con successo!',

    // ------------------------------------------------------------------
    // 37. Save Public Suggestion Action
    // ------------------------------------------------------------------
    'save_public_suggestion.err_failed_submit'    => 'Invio del suggerimento di modifica fallito. Riprova.',
    'save_public_suggestion.err_field_required'   => 'Questo campo è obbligatorio e non può essere inviato vuoto.',
    'save_public_suggestion.err_invalid_column'   => 'Colonna specificata non valida.',
    'save_public_suggestion.err_invalid_params'   => 'Parametri di invio record non validi.',
    'save_public_suggestion.err_spam_detected'    => 'Spam rilevato. Invio rifiutato.',
    'save_public_suggestion.msg_success'          => 'Il tuo suggerimento di modifica è stato inviato con successo alla coda di moderazione. Grazie!',

    // ------------------------------------------------------------------
    // 38. Data Entry Workstation
    // ------------------------------------------------------------------
    'data_entry.active_table_label'               => 'Tabella di inserimento dati attiva:',
    'data_entry.add_entry_summary'                => '➕ Aggiungi nuova immissione dati (Clicca per espandere/comprimere)',
    'data_entry.admin_cols_prompt'                => 'Se sei un Amministratore, vai su Admin → Gestisci Tabelle per gestire le colonne e aggiungere almeno una colonna a questa tabella.',
    'data_entry.admin_tables_prompt'              => 'Se sei un Amministratore, vai su Admin → Gestisci Tabelle per aggiungere una tabella, quindi aggiungi almeno una colonna.',
    'data_entry.apply_filters_btn'                => 'Applica Filtri di Ricerca',
    'data_entry.bool_cross'                       => '✘ (Croce)',
    'data_entry.bool_false'                       => 'Falso',
    'data_entry.bool_female'                      => 'Femmina',
    'data_entry.bool_male'                        => 'Maschio',
    'data_entry.bool_no_false'                    => 'No / Falso',
    'data_entry.bool_tick'                        => '✔ (Spuntato)',
    'data_entry.bool_true'                        => 'Vero',
    'data_entry.bool_yes_true'                    => 'Sì / Vero',
    'data_entry.clipboard_alert'                  => 'Dati della tabella copiati negli appunti! Puoi incollarli in Excel o Google Fogli.',
    'data_entry.contact_admin_cols'               => 'Contatta l\'amministratore per configurare le colonne di questa tabella.',
    'data_entry.contact_admin_tables'             => 'Contatta un amministratore per configurare tabelle e colonne.',
    'data_entry.copy_entire_btn'                  => 'Copia Tabella Completa',
    'data_entry.copy_filtered_btn'                => 'Copia Tabella Filtrata',
    'data_entry.csv_entire_btn'                   => 'Scarica CSV Completo',
    'data_entry.csv_filtered_btn'                 => 'Scarica CSV Filtrato',
    'data_entry.date_format_hint'                 => 'Utilizzare il formato della data mostrato nel campo.',
    'data_entry.date_from_label'                  => 'Da',
    'data_entry.date_placeholder_dmy'             => 'GG/MM/AAAA (o anno parziale)',
    'data_entry.date_placeholder_mdy'             => 'MM/GG/AAAA (o anno parziale)',
    'data_entry.date_placeholder_ymd'             => 'AAAA-MM-GG (o anno parziale)',
    'data_entry.date_title_hint'                  => 'Accetta date complete o parziali (es. 1842 o 1842-05)',
    'data_entry.date_to_label'                    => 'A',
    'data_entry.delete_record_btn'                => 'Elimina',
    'data_entry.delete_record_confirm'            => 'Eliminare definitivamente questo record? I valori, i segnaposti sulla mappa e i relativi suggerimenti verranno rimossi. Questa operazione non può essere annullata.',
    'data_entry.dup_blocked'                      => 'Questo record è troppo simile a uno già salvato, quindi non può essere aggiunto.',
    'data_entry.dup_close_match'                  => 'Ortografia molto simile',
    'data_entry.dup_confirm_btn'                  => 'Sì, conferma e salva duplicato',
    'data_entry.dup_desc'                         => 'Si prega di controllare le schede sottostanti. Puoi comunque salvare se si tratta di un record differente.',
    'data_entry.dup_empty'                        => '(vuoto)',
    'data_entry.dup_flag_btn'                     => 'Salva e avvisa un moderatore',
    'data_entry.dup_heading'                      => '⚠️ Avviso di Possibile Duplicato',
    'data_entry.dup_item_format'                  => 'ID Record: %d — Valore: %s',
    'data_entry.dup_overview'                     => 'Già salvato',
    'data_entry.dup_prompt'                       => 'Desideri procedere e salvare comunque questo duplicato?',
    'data_entry.dup_review_btn'                   => 'Rivedi ciò che ho scritto',
    'data_entry.dup_score_exact'                  => 'Somiglianza calcolata: %s percento.',
    'data_entry.dup_similar'                      => 'Questo appare simile al %s%% rispetto al record #%s',
    'data_entry.edit_record_btn'                  => 'Modifica',
    'data_entry.edit_record_saved'                => 'Il record è stato aggiornato.',
    'data_entry.enter_value_placeholder'          => 'Inserisci valore...',
    'data_entry.error_loading'                    => 'Errore durante il caricamento dei record',
    'data_entry.existing_records_heading'         => 'Tabella dei Record Esistenti',
    'data_entry.filter_all_option'                => '-- Tutti --',
    'data_entry.filter_placeholder'               => 'Filtra...',
    'data_entry.go_manage_tables'                 => 'Vai su Admin → Gestisci Tabelle',
    'data_entry.json_entire_btn'                  => 'Scarica JSON Completo',
    'data_entry.json_filtered_btn'                => 'Scarica JSON Filtrato',
    'data_entry.location_body'                    => 'Testo popup',
    'data_entry.location_busy'                    => 'La ricerca dei luoghi è occupata. Riprova tra un minuto.',
    'data_entry.location_color'                   => 'Colore segnaposto',
    'data_entry.location_help'                    => 'Cerca il luogo così come è noto oggi, seleziona una corrispondenza, quindi puoi etichettarlo con il vecchio nome. Titolo e testo breve sono obbligatori per il popup della mappa.',
    'data_entry.location_label'                   => 'Nome da mostrare (puoi usare un nome storico)',
    'data_entry.location_none'                    => 'Nessun luogo corrispondente. Prova con una città vicina.',
    'data_entry.location_results'                 => 'Forse intendevi',
    'data_entry.location_search'                  => 'Trova luogo',
    'data_entry.location_search_btn'              => 'Cerca',
    'data_entry.location_show_on_map'             => 'Mostra questo luogo sulla mappa',
    'data_entry.location_show_on_map_help'        => 'Deseleziona per mantenere il record nella tabella ma nascondere il segnaposto (es. non ancora aperto).',
    'data_entry.location_title'                   => 'Titolo popup',
    'data_entry.multiselect_hint'                 => 'Tieni premuto Ctrl (o Cmd) per selezionare più di una voce.',
    'data_entry.na_value'                         => 'N/D',
    'data_entry.no_cols_desc'                     => 'Ci sono tabelle nel sistema, ma non è stata definita alcuna colonna di dati per la tabella attiva.',
    'data_entry.no_cols_heading'                  => '⚠️ Nessuna Colonna Configurata',
    'data_entry.no_records'                       => 'Nessun record trovato.',
    'data_entry.no_tables_desc'                   => 'Nessuna tabella del database attiva configurata per l\'inserimento dei dati.',
    'data_entry.no_tables_heading'                => '⚠️ Nessuna Tabella del Database Trovata',
    'data_entry.page_label'                       => 'Pagina:',
    'data_entry.record_deleted'                   => 'Il record è stato eliminato.',
    'data_entry.reset_filter_btn'                 => 'Reimposta Filtro',
    'data_entry.search_summary'                   => '🔍 Cerca e Filtra Record Esistenti (Clicca per espandere/comprimere)',
    'data_entry.shortcuts_tip'                    => '💡 Suggerimenti: Premi Ctrl + Invio per inviare, o Esc per pulire il campo corrente.',
    'data_entry.submit_data_btn'                  => 'Invia Dati',
    'data_entry.th_added_by'                      => 'Aggiunto da',
    'data_entry.th_date_created'                  => 'Data di Creazione',
    'data_entry.time_format_hint'                 => 'Utilizzare il formato dell\'ora mostrato nel campo.',
    'edit_record.heading'                         => 'Modifica record',

    // ------------------------------------------------------------------
    // 39. Forgot Password
    // ------------------------------------------------------------------
    'forgot_password.aria_region'                 => 'Recupero Password',
    'forgot_password.back_login_link'             => 'Torna alla pagina di accesso',
    'forgot_password.email_label'                 => 'Indirizzo Email:',
    'forgot_password.heading'                     => 'Reimposta la tua Password',
    'forgot_password.subheading'                  => 'Inserisci l\'indirizzo email del tuo account qui sotto per ricevere un link sicuro di reset della password.',
    'forgot_password.submit_btn'                  => 'Invia Link di Reset',

    // ------------------------------------------------------------------
    // 40. User Login
    // ------------------------------------------------------------------
    'login.aria_region'                           => 'Accesso Utente',
    'login.err_password_required'                 => 'Inserisci la tua password.',
    'login.err_username_required'                 => 'Inserisci il tuo nome utente.',
    'login.forgot_password_link'                  => 'Hai dimenticato la password?',
    'login.heading'                               => 'Accedi all\'Account',
    'login.password_label'                        => 'Password:',
    'login.submit_btn'                            => 'Accedi',
    'login.username_label'                        => 'Nome utente o email:',

    // ------------------------------------------------------------------
    // 41. User Onboarding Setup Wizard
    // ------------------------------------------------------------------
    'onboarding.attr_anon_text'                   => 'Mostra le iniziali e un numero casuale a tutti.',
    'onboarding.attr_anon_title'                  => 'Anonimo:',
    'onboarding.attr_opt_anon'                    => 'Anonimo (Iniziali e numero casuale)',
    'onboarding.attr_opt_public'                  => 'Pubblico (Mostra nome completo)',
    'onboarding.attr_opt_vol'                     => 'Solo volontari',
    'onboarding.attr_public_text'                 => 'Mostra il tuo nome completo a tutti.',
    'onboarding.attr_public_title'                => 'Pubblico:',
    'onboarding.attr_vol_text'                    => 'Mostra le iniziali al pubblico, ma il tuo nome completo ai volontari, moderatori e amministratori loggati.',
    'onboarding.attr_vol_title'                   => 'Solo Volontari:',
    'onboarding.attribution_desc1'                => 'Controlla come appare il tuo nome nella classifica pubblica e nei record.',
    'onboarding.attribution_label'                => 'Preferenza di visualizzazione in classifica:',
    'onboarding.btn_continue_2fa'                 => 'Continua a 2FA',
    'onboarding.btn_skip_for_now'                 => 'Salta per ora',
    'onboarding.date_fmt_dmy'                     => '23/07/2026 (Stile UK - GG/MM/AAAA)',
    'onboarding.date_fmt_dmy_short'               => '23/07/26 (Anno breve - GG/MM/AA)',
    'onboarding.date_fmt_dots'                    => '23.07.2026 (Punti - GG.MM.AAAA)',
    'onboarding.date_fmt_full'                    => 'Giovedì 23 luglio 2026 (Testo completo)',
    'onboarding.date_fmt_mdy'                     => '07/23/2026 (Stile USA - MM/GG/AAAA)',
    'onboarding.date_format_label'                => 'Formato di Visualizzazione Data:',
    'onboarding.err_names_required'               => 'Nome e cognome sono obbligatori.',
    'onboarding.err_save_failed'                  => 'Salvataggio delle preferenze non riuscito. Riprova.',
    'onboarding.heading'                          => 'Benvenuto nel team!',
    'onboarding.lang_site_default'                => 'Predefinito del sito',
    'onboarding.language_label'                   => 'Lingua',
    'onboarding.msg_welcome'                      => 'Benvenuto a bordo! Le tue preferenze sono state salvate.',
    'onboarding.page_title'                       => 'Benvenuto — Configurazione Account',
    'onboarding.security_hint'                    => 'Puoi modificare le impostazioni personali in qualsiasi momento facendo clic sul tuo nome utente nel menu.',
    'onboarding.subheading'                       => 'Prima di iniziare, prenditi un momento per configurare le preferenze di visualizzazione regionale e la privacy.',
    'onboarding.submit_btn'                       => 'Salva Preferenze e Continua',
    'onboarding.time_12'                          => '12 ore AM/PM (es. 04:07 PM)',
    'onboarding.time_24'                          => '24 ore (es. 16:07)',
    'onboarding.time_format_label'                => 'Formato Orologio (Visualizzazione Ora):',
    'onboarding.time_none'                        => 'Solo Data (Nascondi completamente l\'ora)',
    'onboarding.timezone_label'                   => 'Fuso Orario / Regione:',

    // ------------------------------------------------------------------
    // 42. User Profile & Security Settings
    // ------------------------------------------------------------------
    'profile.aria_new_email'                      => 'Nuovo Indirizzo Email',
    'profile.aria_region'                         => 'Gestione Profilo Utente',
    'profile.backup_codes_heading'                => 'I tuoi nuovi codici di backup di sicurezza',
    'profile.change_email_label'                  => 'Modifica Indirizzo Email:',
    'profile.confirm_password_label'              => 'Conferma Nuova Password:',
    'profile.current_email_label'                 => 'Email attuale:',
    'profile.current_password_label'              => 'Password Attuale:',
    'profile.download_codes_btn'                  => 'Scarica nuovi codici come .txt',
    'profile.email_heading'                       => 'Indirizzo Email',
    'profile.email_unverified'                    => '(Non verificato - Controlla la tua casella di posta)',
    'profile.email_verified'                      => '(Verificato)',
    'profile.err_personal_update'                 => 'Aggiornamento dei dati personali non riuscito.',
    'profile.generate_codes_btn'                  => 'Genera Nuovi Codici di Backup',
    'profile.generate_codes_confirm'              => 'Sei sicuro? Questo invaliderà qualsiasi codice di backup esistente.',
    'profile.heading'                             => 'Profilo Utente e Sicurezza',
    'profile.lang_site_default'                   => 'Predefinita del Sito',
    'profile.language_label'                      => 'Lingua Preferita:',
    'profile.msg_personal_updated'                => 'Dati personali, fuso orario e impostazioni di formato aggiornati con successo!',
    'profile.new_password_label'                  => 'Nuova Password (min. 8 caratteri):',
    'profile.password_heading'                    => 'Modifica Password',
    'profile.personal_details_heading'            => 'Dettagli Personali',
    'profile.setup_tfa_btn'                       => 'Configura 2FA',
    'profile.show_passwords_label'                => 'Mostra password in chiaro',
    'profile.tfa_active_desc'                     => 'Il 2FA sta proteggendo attivamente l\'accesso al tuo account.',
    'profile.tfa_disabled'                        => 'Disattivato',
    'profile.tfa_enabled'                         => 'Attivato',
    'profile.tfa_heading'                         => 'Autenticazione a Due Fattori (2FA)',
    'profile.tfa_status_label'                    => 'Stato:',
    'profile.update_details_btn'                  => 'Aggiorna Dettagli Personali',
    'profile.update_email_btn'                    => 'Aggiorna Email e Verifica',
    'profile.update_password_btn'                 => 'Aggiorna Password',

    // ------------------------------------------------------------------
    // 43. User Registration
    // ------------------------------------------------------------------
    'register.aria_region'                        => 'Registrazione Utente',
    'register.email_label'                        => 'Indirizzo email',
    'register.heading'                            => 'Registra Nuovo Account',
    'register.submit_btn'                         => 'Registrati',
    'register.username_label'                     => 'Nome Utente:',

    // ------------------------------------------------------------------
    // 44. Set Password via Secure Token
    // ------------------------------------------------------------------
    'set_password.aria_region'                    => 'Imposta Password',
    'set_password.confirm_password_label'         => 'Conferma Password:',
    'set_password.exit_expired_token'             => 'Questo link per la password non è valido o è scaduto.',
    'set_password.exit_invalid_token'             => 'Token di configurazione non valido o mancante.',
    'set_password.heading_format'                 => 'Imposta Password per %s',
    'set_password.msg_ready'                      => 'Password configurata con successo! Ora puoi effettuare l’accesso.',
    'set_password.msg_ready_onboarding'           => 'Password salvata. Effettua l’accesso — verrai guidato attraverso una breve configurazione.',
    'set_password.new_password_label'             => 'Nuova Password (min. 8 caratteri):',
    'set_password.proceed_login_btn'              => 'Vai alla Pagina di Accesso',
    'set_password.save_password_btn'              => 'Salva Password',
    'set_password.show_password_label'            => 'Mostra Password',
    'set_password.subheading_format'              => 'Benvenuto nel tuo nuovo account, %s! Scegli la tua password qui sotto.',

    // ------------------------------------------------------------------
    // 45. Setup 2FA Wizard
    // ------------------------------------------------------------------
    'setup_2fa.aria_code_input'                   => 'Codice di verifica a 6 cifre',
    'setup_2fa.aria_region'                       => 'Configurazione Guidata 2FA',
    'setup_2fa.backup_desc'                       => 'Salva questi codici di backup in un luogo sicuro. Ciascun codice può essere utilizzato una sola volta se perdi l\'accesso alla tua app 2FA:',
    'setup_2fa.backup_heading'                    => 'Codici di Recupero Sicurezza di Emergenza',
    'setup_2fa.cancel_link'                       => 'Annulla e Torna al Profilo',
    'setup_2fa.code_label'                        => 'Inserisci il codice a 6 cifre dall\'app per verificare e attivare:',
    'setup_2fa.download_btn'                      => 'Scarica codici come .txt',
    'setup_2fa.heading'                           => 'Configura 2FA',
    'setup_2fa.manual_prompt'                     => 'Oppure inserisci questa chiave segreta manualmente:',
    'setup_2fa.qr_alt'                            => 'Codice QR per configurazione 2FA',
    'setup_2fa.subheading'                        => 'Scansiona il codice QR qui sotto con la tua app di autenticazione.',
    'setup_2fa.submit_btn'                        => 'Verifica e Attiva 2FA',

    // ------------------------------------------------------------------
    // 46. Suggest Edit View
    // ------------------------------------------------------------------
    'suggest_edit.aria_region'                    => 'Suggerisci Modifica',
    'suggest_edit.confirm_prompt'                 => 'Sei sicuro di voler inviare questo suggerimento di modifica per la revisione dell\'amministratore?',
    'suggest_edit.current_values_heading'         => 'Valori Attuali:',
    'suggest_edit.decision_accepted'              => 'Accettato',
    'suggest_edit.decision_not_accepted'          => 'Non accettato',
    'suggest_edit.dup_of_help'                    => 'Opzionale. I moderatori vedranno questa nota.',
    'suggest_edit.dup_of_label'                   => 'Se conosci il numero dell\'altro record, puoi aggiungerlo qui',
    'suggest_edit.dup_please_check'               => 'Questa modifica risulterebbe molto simile a un altro record. Si prega di verificare sotto.',
    'suggest_edit.empty_label'                    => '(vuoto)',
    'suggest_edit.err_notify_email'               => 'Inserisci un indirizzo e-mail se desideri conoscere l\'esito.',
    'suggest_edit.heading_prefix'                 => 'Suggerisci Modifica per il Record',
    'suggest_edit.leave_blank'                    => 'Lascia questo campo vuoto',
    'suggest_edit.no_rationale'                   => 'Non è stata aggiunta alcuna nota extra.',
    'suggest_edit.notify_email_help'              => 'Utilizzato esclusivamente per comunicarti l\'esito. Non viene mostrato sul record pubblico.',
    'suggest_edit.notify_email_label'             => 'Il tuo indirizzo e-mail',
    'suggest_edit.notify_label'                   => 'Inviami un\'e-mail quando un moderatore avrà preso una decisione',
    'suggest_edit.proposed_value_label'           => 'Nuovo valore proposto:',
    'suggest_edit.reasoning_label'                => 'Prova / Motivazione / Note sulla Fonte:',
    'suggest_edit.reasoning_optional'             => 'Facoltativo — prove, fonti o note per i moderatori.',
    'suggest_edit.reasoning_placeholder'          => 'Fornisci contesto, citazione della fonte o ragione per questa modifica...',
    'suggest_edit.report_dup'                     => 'Penso che questo record sia il duplicato di un altro',
    'suggest_edit.return_btn'                     => 'Torna al Record',
    'suggest_edit.select_column_label'            => 'Seleziona colonna da modificare:',
    'suggest_edit.submit_btn'                     => 'Invia Suggerimento per la Revisione',
    'suggest_edit.submit_heading'                 => 'Invia Nuovo Valore Proposto e Prove',
    'suggest_edit.success_msg_suffix'             => 'Puoi inviare un\'altra modifica qui sotto o usare il link di ritorno sopra quando hai finito.',

    // ------------------------------------------------------------------
    // 47. Verify 2FA Login Challenge
    // ------------------------------------------------------------------
    'verify_2fa.aria_code_input'                  => 'Inserisci il codice di verifica o di sicurezza',
    'verify_2fa.aria_region'                      => 'Verifica 2FA',
    'verify_2fa.code_label'                       => 'Codice di Verifica / Codice di Sicurezza:',
    'verify_2fa.heading'                          => 'Autenticazione a Due Fattori',
    'verify_2fa.subheading'                       => 'Inserisci il codice a 6 cifre dalla tua app di autenticazione o un codice di backup di sicurezza.',
    'verify_2fa.submit_btn'                       => 'Verifica e Accedi',

    // ------------------------------------------------------------------
    // 48. Verify Email
    // ------------------------------------------------------------------
    'verify_email.aria_region'                    => 'Stato Verifica Email',
    'verify_email.err_expired_token'              => 'Questo link di verifica è scaduto (limite di 24 ore superato). Registrati di nuovo o richiedi un nuovo link.',
    'verify_email.err_invalid_token'              => 'Token di verifica non valido.',
    'verify_email.err_no_token'                   => 'Nessun token di verifica fornito.',
    'verify_email.err_update_failed'              => 'Si è verificato un errore durante la verifica dell\'email. Riprova.',
    'verify_email.heading'                        => 'Stato Verifica Email',
    'verify_email.login_btn'                      => 'Clicca qui per accedere',
    'verify_email.msg_already_verified'           => 'La tua email è già stata verificata. Puoi accedere.',
    'verify_email.msg_success'                    => 'Email verificata con successo! Il tuo account è ora attivo. Procedi con l\'accesso.',

    // ------------------------------------------------------------------
    // 49. Volunteer Form View
    // ------------------------------------------------------------------
    'index.allocate_unique_username'              => 'Assegnami un nome utente univoco',
    'index.check_availability'                    => 'Verifica disponibilità',
    'volunteer.allocate_unique_username'          => 'Assegnami un nome utente univoco',
    'volunteer.aria_region'                       => 'Modulo Volontario',
    'volunteer.check_availability'                => 'Verifica disponibilità',
    'volunteer.honeypot_label'                    => 'Lascia questo campo vuoto:',
    'volunteer.multi_select_hint'                 => 'Premi Ctrl o Cmd per selezionare più elementi.',
    'volunteer.required_field_title'              => 'Campo Obbligatorio',
    'volunteer.submit_btn'                        => 'Invia Candidatura Volontario',
];
